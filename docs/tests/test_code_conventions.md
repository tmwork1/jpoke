# test_code_conventions

テスト数: 11

- [x] Pokemon_modify_hpが公開APIとして露出していない
- [x] READMEがjudge_winnerのis_None比較を使っていない
- [x] consume_itemの引数名がtargetにリネームされている
- [x] docs_examplesがjpoke_testingモジュールに言及している
- [x] docs_examplesがアイテム操作系APIに言及している
- [x] examplesがactives経由でインデックスアクセスしていない
- [x] examplesがjudge_winnerのis_None比較を使っていない
- [x] examplesがplayer_states経由で内部属性に直接アクセスしていない
- [x] examplesがteam_append_Pokemon経由でチームを構築していない
- [x] examplesがtest_option経由で命中率等を固定していない
- [x] hp直接代入がallowlist外のファイルに存在しない
