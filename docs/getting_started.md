...
jpoke を初めて使う人向けに、インストールから最初の対戦・自作AIの作成までを順に進める
チュートリアル。5分もあれば最後まで動かせる。

より深く知りたくなったら、末尾の「次の一歩」から目的別のサンプル・ドキュメントへ進める。
...
`requires-python = ">=3.11"`。ランタイム依存ライブラリはゼロなので、これだけでネットワーク
接続なしに動作する。
...
（`examples/01_getting_started/01_quick_start.ipynb` 参照）の方が手軽。
...
`RandomPlayer` は合法手からランダムに行動を選ぶ `Player` の既定実装。自作の方策を
