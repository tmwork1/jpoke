...
- （スコープ外・未対応）`Battle.activate_side_field()`は内部で`SideFieldManager`が継承する
  ...
  この差は影響しない）。今回のラウンドのスコープ外のため未対応。対応候補としては、
  `activate_side_field`内でエラー技（壁技かどうか）を判定して`.apply()`に振り分ける、
  または`docstring`に既知の制約として明記する、等が考えられる。次ラウンド以降で検討する。
