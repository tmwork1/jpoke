...
`docs/review/code/tree_search.md` で調査した通り、`Battle`/`Player`/`CommandManager` は
...
`docs/plan/battle_option.md` で提案中の `BattleOption`（`damage_roll="平均"` 等、対戦全体の
...
- `scripts/tree_search/framework.py` を `src/jpoke/search/` へ昇格させる場合は、
  `CLAUDE.md` のアーキテクチャ表に行を追加し、`docs/progress/` 配下に
  進捗ドキュメントを新設するかを別途判断する。
- 本計画は `docs/review/code/tree_search.md` の CRIT-1 / ISSUE-1 の解消を兼ねる。
