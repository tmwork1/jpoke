...
`docs/plan/archives/refactor_event_log_structure.md`（2026-07-04）で緩い辞書
...
同計画は `docs/spec/event_log.md` の新設を「必要であれば追加検討」として
未着手のまま残しており（同ファイル372行目）、また
`docs/plan/archives/battle_log_replay.md:368-369` は
...
### 1. `render()` が LogCode 31種を1つの巨大な `match` 文で個別実装している
...
（`get_log_lines()`）と `scripts/fuzz_common.py:89`（同ロジックの重複）:
...
- **解析用インターフェース**: `EventLog.to_dict()` / `EventLog.payload`
  ...
  `docs/spec/event_log.md` に明文化する。LogCode → Payloadクラス → 各フィールドの
...
- `battle.py` の `get_log_lines()` と `scripts/fuzz_common.py` の
...
### フェーズ2: `docs/spec/event_log.md` の新設

- LogCode（31種、`enums/logcode.py`）× Payloadクラス × フィールド意味の対応表
- 「分析には `to_dict()`/dataclass属性を使う。`render()`の文字列パースは非推奨」
  と明記

### フェーズ3: `render()` 文言の統一（`event_logger.py`）

- LogCodeごとの文体ルールを決定する（例: 文末表現の統一、体言止めの扱い）
- `STAT_CHANGED` 用に `Stat` → 日本語ラベルの対応表を追加し、内部Literalの露出を消す
- 巨大 `match` 文自体の分割・テーブル化は可読性目的の範囲にとどめ、
  機能追加は行わない

### フェーズ4: `TEXT_LOG` の扱いを整理（`handlers/ability.py`, `handlers/volatile.py`）

- 3箇所を確認し、構造化Payloadに置き換え可能なら置き換える
- 置き換えられないものは「意図的な自由記述の例外」であることをコード上に明記する

### フェーズ5: テスト・検証

- LogCode全種が `_get_base_text()` の `match` 文でカバーされていること
  （既存の `case _: raise ValueError(...)` で担保済み、回帰確認のみ）
- `docs/spec/event_log.md` とコードの対応が崩れていないかを継続的に検証する
  手段を検討（自動テスト化するか、レビュー時の目視確認にとどめるかは着手時に判断）
