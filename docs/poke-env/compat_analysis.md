...
- **同期モデル**
  ...
  - jpoke: 同期処理のみで完結
  - 対応方針: jpoke に asyncio を持ち込む必要性は薄い。`battle_against` を追加する場合も同期メソッドとして提供し、非同期を強制しない（学習コストの低さを優先）
...
- **対戦結果の集計**
  - poke-env: `battle_against(opponent, n_battles=n)` 実行後、`n_won_battles` 等が自動更新される
  - jpoke: `Player` にフィールドは存在するが `Battle` からの自動更新は未実装
  - 対応方針: `battle_against` 相当の API を追加する際にまとめて対応する（分類 A-2 の `win_rate` 等と合わせて実装）

- **決定論的テスト機能**
  - poke-env: 実サーバーの乱数に依存するため命中率・追加効果確率を固定する公式手段はない
  - jpoke: `TestOption`（分類D）で命中率・追加効果確率・乱数を固定できる
  - 対応方針: jpoke 独自の強みとして維持。`battle_against` 追加時は「再現可能な対戦を組みたい場合は `TestOption` を使う」という導線をドキュメントに加える
...
| `gender: PokemonGender` | `gender: Gender`（値: `"male" / "female" / ""`） | 値は英語化済みだが型名は `Gender` のまま（`PokemonGender` への改名は未対応） | A-1: 型名を `PokemonGender` に改名 |
...
| `Gender`（型名） | `PokemonGender` | `types/literals.py` | 未対応 |
...
**未対応**: `Gender` → `PokemonGender` の型名改名（値のみ先行して英語化済みで、型名は `types/literals.py` 上でまだ `Gender` のまま。下記「プロパティ・変数の改名」表に追加）。
