...
本早見表も `battle.<manager>.<method>()` のような内部マネージャーへの直接アクセスや、
`_` 始まりの非公開属性は載せない。
...
[APIリファレンス](reference/index.md)を参照。
...
| `calc_move_base_power(attacker, defender, move)` | けたぐり・はきだす等の技固有の変動までを解決した基礎威力を返す。特性・持ち物・天候等の威力補正とテラスタル時の威力60底上げは含まない。威力を持たない技は 0、連続技は1撃目の威力。問い合わせは技の使用として記録されない |
| `calc_move_power(attacker, defender, move)` | 現在の状況における技の最終威力を返す。けたぐり・はきだす等の技固有の変動、特性・持ち物・天候等の補正、テラスタル時の威力60底上げまで含めたダメージ式に入る値で、威力を持たない技（変化技・固定ダメージ技）は 0、連続技は1撃目の威力。問い合わせは技の使用として記録されない |
...
base_power = battle.calc_move_base_power(attacker, defender, "けたぐり")  # カビゴン相手なら120
power = battle.calc_move_power(attacker, defender, "けたぐり")
```
...
| `set_ailment(target, name, count=None, source=None, overwrite=True)` | 状態異常を直接付与する。既定では既存の状態異常があれば上書きするが、タイプ免疫（ほのおタイプへの「やけど」等）やON_BEFORE_APPLY_AILMENT（不眠等の特性による無効化）の判定は通常の付与と同様に行う（これらで阻まれると戻り値`False`で付与されない）。`overwrite=False` にすると既に状態異常があるだけで付与に失敗する |
...
木探索系（`TreeSearchPlayer`/`MinimaxPlayer`）以外に、比較対象・ベースラインとして
すぐ使える `Player` 実装が `jpoke.players` に同梱されている。
...
> シングルバトルにダイマックス・Zワザは存在しないため未実装。`available_commands()` が
...
インデックスから対応するコマンドを組み立てる際に使う。`available_commands()` の結果を
