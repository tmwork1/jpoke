...
1. ログの構造自体（`EventLog`/`Payload` 体系）は既に2回のリファクタ
   （`docs/plan/archives/refactor_event_log_structure.md` 2026-07-04、
   `docs/plan/event_log_render_unification.md` 2026-07-11）を経ており、機械可読性
...
4. **`EventLog` にはターン番号・プレイヤーインデックスはあるがフェーズ・シーケンス番号が
   ...
   （§4）。
5. 設計思想として event_logger のログは「見るためのログ」（`docs/plan/archives/
...
### 位置づけの明文化
`docs/plan/archives/battle_log_replay.md` L368-369:
