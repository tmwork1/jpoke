...
- `docs/spec/moves/` に個別の仕様書は存在しない。100%発動の追加効果（相手の
  『ぼうぎょ』-1）は他の同種の追加効果技（アイアンテール、かみくだく など）
  と同様に `docs/spec/moves/_move_secondary_1.md` の一覧表で管理する方針であり、
...
- **不備を発見し修正した**: `_move_secondary_1.md` 18行目の技名が全角の
  ...
  表記に修正した。
- じゅうりょく状態時の威力1.5倍効果は `docs/spec/fields/じゅうりょく.md` に
...
- `docs/plan/moves/Gのちから.md` は存在しない。同種の単純な追加効果技にも
  個別の計画書は存在せず、慣例に倣い新規作成はしていない。
- じゅうりょく威力補正のハンドラ優先度（ON_CALC_POWER_MODIFIER, priority=100）
  は `docs/spec/fields/じゅうりょく.md` に既に根拠が明記されており、実装も
  それに準拠している。
...
- **不備を発見し修正した**: `src/jpoke/data/move.py` の `"Gのちから"` の
  ...
  `power=90` に修正した。`docs/progress/move.md` の該当行は元々90と正しく
...
- `docs/progress/move.md` のGのちから行を更新:
