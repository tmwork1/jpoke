...
- `Battle.calc_lethal` の連続技（多段ヒット）で、途中ヒットで一部の枝が HP0 になると
  残りのヒットが捨てられ、致死率・打点が過小評価されていた問題を修正
  （例: タネマシンガン5ヒットで確2のところが「乱2 3.36%」になっていた）。
  HP0 の枝は以降のヒット・ハンドラの対象から分離され、`LethalHitResult.damage_dist`
  は「そのヒットを受けた生存枝に対するダメージ分布」を表すようになった。
  `resume_from` に HP0 枝を含む分布を渡した場合に `ON_BEFORE_MOVE` が
  スキップされていた問題も同時に解消
- メガソーラー中のウェザーボールが天候なし扱いでノーマルタイプになっていた問題を修正
  （`ON_SETUP_MOVE` を `ON_MODIFY_MOVE_TYPE` / `ON_MODIFY_MOVE_CATEGORY` より先に発火）

## [0.5.0] - 2026-09-16

### Fixed

- STAB（タイプ一致補正）の元タイプ判定が種族タイプを直接参照していたため、
  へんげんじざい・リベロ・ミラータイプ・みずびたし・へんしん・もりののろい等で
  タイプが変化しても変化後のタイプにタイプ一致補正が乗らなかった問題を修正。
  `Pokemon.types_before_tera`（テラスタルのみ無視した現在タイプ）を追加
- `Battle.calc_damages` / `roll_damage` が技実行フローを経ないため、技データの
  ハンドラ・スキン系のタイプ変換・分類変化・かたやぶり系の特性無効化がダメージ
  計算に反映されていなかった問題を修正
...
- **破壊的変更**: サポートする最小 Python バージョンを 3.10 から 3.11 に引き上げた
  （`requires-python = ">=3.11"`）。CI のテストマトリクスも 3.10/3.12 から
  3.11/3.12 に変更
