...
def build_sorted_content(target: Path) -> str:
    """ABILITIES辞書を五十音順に並び替えた新しいファイル内容を返す（書き込みはしない）。"""
...
def check_abilities(target: Path) -> bool:
...
def main() -> None:
    if "--check" in sys.argv[1:]:
        if check_abilities(ABILITY_PY):
            print(f"OK: 整列済み → {ABILITY_PY}")
        else:
            print(f"未整列: {ABILITY_PY}", file=sys.stderr)
            sys.exit(1)
        return
