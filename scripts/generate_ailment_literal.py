...
使い方:
    python scripts/generate_ailment_literal.py

処理内容:
- src/jpoke/data/ailment.py を AST 解析し、AILMENTS 辞書のトップレベルキーを
  定義順のまま抽出する（実行時 import は行わない）
- src/jpoke/utils/type_defs/type_defs.py 内の `AilmentName = Literal[...]` の行を
...
GENERATED_COMMENT = (
    "# 自動生成: python scripts/generate_ailment_literal.py で更新"
...
def update_type_defs(target: Path, new_line: str) -> None:
    """type_defs.py内のAilmentName定義行を置換する。"""
