...
- **[`core/turn_controller.py`](src/jpoke/core/turn_controller.py)**: `TurnController` - ターン進行管理
  ...
  - `advance_turn()`: ターン進行のエントリポイント
  - `_advance_turn()`: ターン進行の内部処理（コマンド処理、技発動、交代のオーケストレーション）
  - `TOD_score()`: Time of Death スコア計算
  - `winner()`: 勝敗判定
...
- **[`model/pokemon.py`](src/jpoke/model/pokemon.py)**: `Pokemon` クラス - ポケモンのバトル状態と基本データを管理
  ...
  - **場の出入り**:
    - `switch_in()`: イベントハンドラの登録（特性、持ち物、状態異常）
    - `switch_out()`: イベントハンドラの解除
  - **状態管理**:
    - バトル状態: `hp`, `ailment`, `rank`, `is_terastallized` など
...
### データ定義の構造
- **[`data/models.py`](src/jpoke/data/models.py)**: `AbilityData`, `ItemData`, `MoveData`, `AilmentData` などの dataclass 定義
...
- **[`utils/type_defs.py`](src/jpoke/utils/type_defs.py)**: 型エイリアス定義（Literal型）
  - `RoleSpec`, `EffectSource`, `LogPolicy`, `Stat`, `Type`, `Weather`, `Terrain`, `AilmentName` など
...
## 関連ファイル解読順序
1. [`utils/type_defs.py`](src/jpoke/utils/type_defs.py) - RoleSpec, EffectSource, LogPolicy などの型定義
...
5. [`model/pokemon.py`](src/jpoke/model/pokemon.py) - Pokemon クラスとバトル状態管理
6. [`core/event.py`](src/jpoke/core/event.py) - Handler, EventContext, EventManager
7. [`core/move_executor.py`](src/jpoke/core/move_executor.py) - 技実行処理
8. [`core/switch_manager.py`](src/jpoke/core/switch_manager.py) - 交代処理
9. [`core/turn_controller.py`](src/jpoke/core/turn_controller.py) - ターン進行処理
10. [`core/battle.py`](src/jpoke/core/battle.py) - Battle クラスと各マネージャークラスのファサード
11. [`data/models.py`](src/jpoke/data/models.py) - AbilityData, ItemData, MoveData などの構造定義
12. [`handlers/ability.py`](src/jpoke/handlers/ability.py) + [`handlers/item.py`](src/jpoke/handlers/item.py) + [`handlers/ailment.py`](src/jpoke/handlers/ailment.py) - Handler 派生と汎用関数
13. [`handlers/common.py`](src/jpoke/handlers/common.py) - 汎用ハンドラ関数（modify_hp, modify_stat など）
