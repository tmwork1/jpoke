...
設計の詳細・根拠は .internal/plan/archives/tree_search_framework.md を参照。
"""
...
class TreeSearchPlayer(Player):
    ...
    def choose_command(self, battle: Battle) -> Command:
        if self._searching:
            ...
            # （.internal/plan/archives/tree_search_framework.md ISSUE-1 参照）。
    ...
    def _best_command(self, battle: Battle, plies: int) -> tuple[Command, float]:
        ...
        if plies == self.max_plies:
            ...
            # 元々存在しない。.internal/plan/archives/tree_search_framework.md 参照）。
