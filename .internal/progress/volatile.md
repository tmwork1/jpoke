...
テレキネシス	対象外	対象外	対象外	対象外	n/a	n/a	対象外	付与元の技『テレキネシス』が第9世代（スカーレット・バイオレット）で使用不可のため実装対象外（README.mdの対象範囲規定）。一次情報によれば、3ターンの間、相手を『地面にいない』状態にし、一撃必殺技以外の技を必中で受けるようにする（ねをはる状態への付与は失敗、うちおとす状態・じゅうりょく状態で解除、みがわりで防がれる、バトンタッチで引き継ぎ可能等）。
