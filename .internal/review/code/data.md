...
「`lethal_handlers` 二重管理」については、まさに本日 (`.internal/plan/_done/lethal_full_hp_dependent.md`)
...
`.internal/plan/_done/lethal_full_hp_dependent.md`（2026-07-05付、本日完了の計画書）
...
`.internal/plan/_done/lethal_full_hp_dependent.md`（本日付・完了扱い）には次の記述がある。
...
- `.internal/progress/ability.md` / `item.md` の「リーサル実装」列だけでなく、
  ...
  現状はこの整合性を保証する仕組みが「人間が `.internal/progress/*.md` を見比べる」ことに
...
明文化されていない。実際に本日の `.internal/plan/_done/lethal_full_hp_dependent.md` では
