...
**推奨**: `utils/poke_env.py` へ移動するか、`.internal/poke-env/` に対応する新設パッケージ
（例: `src/jpoke/interop/`。実際 `.internal/plan/poke-env/poke_env_battle_converter.md:7` は
...
計画すら存在しなかった）とは異なり、`.internal/plan/poke-env/poke_env_battle_converter.md`
...
`.internal/plan/poke-env/poke_env_battle_converter.md` にある通り「実装が必要になったタイミングで
追加する」方針に倒す。
...
- 新設した「命名の一貫性・妥当性」の観点では、既存12ファイルが確立していた
  ...
  poke-env 互換実装は現在も `.internal/plan/poke-env/` で後続フェーズが計画中であり、
