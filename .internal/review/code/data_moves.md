...
前回の総合レビュー（`.internal/review/code/index.md`、2026-07-05付）は `data/moves/` を
...
（`.internal/spec/moves/パワースワップ.md`）が明記する必中要件に反している疑いが
...
これは単なる命名の不統一にとどまらない。`.internal/spec/moves/パワースワップ.md`
...
基準のPP値」を機械的に洗い出すことが難しい。`# champions基準（.internal/champions/
