...
`.internal/plan/archives/tree_search_framework.md`（実装計画・完了済み）の続編。
...
計画書は `.internal/plan/tree_search_framework.md` から
`.internal/plan/archives/tree_search_framework.md` へ移動済みだが、以下が旧パスのまま:

- `scripts/tree_search/framework.py:7, 81, 115`（docstring・コメント3箇所）
- `tests/test_tree_search_framework.py:3`
- `.internal/review/code/tree_search.md`（CRIT-1・ISSUE-1 の対応記述2箇所）
...
`.internal/plan/tree_search_framework.md` への旧パス参照3箇所と
`.internal/review/code/tree_search.md` の旧パス参照2箇所を
`.internal/plan/archives/tree_search_framework.md` に修正した。
...
5. docstring・コメント内の計画書リンクを `.internal/plan/archives/` の現パスに修正（FW-D4 と同時解消）
...
実際の利用（`scripts/fuzz/fuzz_battle.py --player tree_search` の `TreeSearchFuzzPlayer`）は `TreeSearchPlayer` を
