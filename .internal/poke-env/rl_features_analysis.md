...
3. **報酬シェーピングのヘルパー**（`reward_computing_helper` 相当）— HP割合・瀕死・状態異常・勝敗を
   ...
   スコアで、エピソード単位の報酬シェーピングとは目的が異なる。`players/tree_search_player.py` の
   `total_hp_ratio(battle, target)`（`TreeSearchPlayer.evaluate()` が使うミニマックス葉ノード評価用の
   HP割合合計）と、RL用の報酬ヘルパー（`rl_support.md` の `calc_state_value`）は評価対象こそ似るが、
   評価関数は利用者が重み・実装ごと差し替える前提のものなので、`calc_state_value` 側は
   `total_hp_ratio` を import せず独立に実装する（詳細は `rl_support.md` 論点4）。
...
B案を `src/jpoke/rl.py` として実装済み。`ACTION_COMMANDS`/`command_to_action`/
`action_to_command`/`get_action_mask`（行動空間・合法手マスク）、`RewardWeights`/
`calc_state_value`（`reward_computing_helper`相当の評価関数、独立実装は論点4参照）、
`embed_battle_basic`（観測ベクトル化の最小参考実装）、`RLBattleEnv`（`reset()`/`step()`
形式の環境ラッパー）を提供する。テストは `tests/test_rl.py`、外部ライブラリなしの
Q学習例は `examples/04_others/rl_q_learning.ipynb` を参照。

実装時に判明した設計上の補足（`rl_support.md` の試案コードからの変更点）:

- `Battle.available_commands()` は `battle.phase` が `"action"`/`"switch"` の
  文脈でしか呼べない実装のため、`get_action_mask()` は内部で
  `battle.phase_context("action")` を使って一時的にフェーズを切り替えてから
  問い合わせる（`jpoke.testing.reserve_command()` と同じ手法）
- `RLBattleEnv.step()` は `resolve_command("action")`（両プレイヤー分）を呼んで
  `state.required_command_type` を都度 `"any"` に更新してから、学習対象の
  コマンドだけをRL行動で上書きする。前のターンに学習対象が強制交代を挟んだ場合、
  更新を省略すると `required_command_type` が `"switch"` のまま残り、次のターンの
  通常コマンドが `validate_command()` に弾かれる
- `terminated = battle.finished` は `calc_state_value()` より必ず先に呼ぶ。
  `finished` は `judge_winner()` 経由で `battle.winner` を確定させる副作用を持ち、
  これを呼ぶ前に `calc_state_value()` の `won()`/`lost()` 判定を行うと、決着した
  当のターンでも `victory` 分の報酬が反映されない
