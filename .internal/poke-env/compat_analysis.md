...
追記日: 2026-07-04（`docs/compat.md` の「使い勝手の違い」観点を統合）
再清書日: 2026-07-04（`.internal/plan/poke_env_compat.md` を `.internal/poke-env/compat_plan.md` へ移動。
実装コードと再照合し、`Gender` 型名が `PokemonGender` に未改名のまま残っていた誤記を修正）

レビュー日: 2026-07-07（`.internal/poke-env/api_reference/` の poke-env 公式ドキュメントと照合。
`accuracy` は 0〜1 の float に変更されている・戦績は `n_tied_battles`・`won`/`lost` は `bool | None` の
プロパティ、等の仕様誤りを修正。`ailment.turn_count` → `ailment.count` の実装名誤り、
`effort`（Champions 形式 0〜32）と poke-env `evs`（0〜252）のスケール差を追記。
なお最新の poke-env ではモジュールパスが `poke_env.environment` から `poke_env.battle` に変更されている）

追記日: 2026-07-07（`battle_against` の提供が `compat_plan.md` Phase 5 として確定したため、
§0 の条件付き表現「追加する場合は」を計画参照に更新）

更新日: 2026-07-11（Pokemonの一時状態をmemoryスコープに統一するリファクタ（`active_turn` フィールド廃止、
`acted_since_switch_in` プロパティへの置換）と、まもる系連続使用チェックの再実装（`protect_chain_move` 撤廃、
`executed_move` + `failed_or_immobile_last_turn` の決定的チェックへ変更）を反映し、
`first_turn` の実装案と `protect_counter`（`Volatile.counter` は実在しない）の記述を修正。
その他 `executed_move`/`indiv`/`effort`/`rank`/`terastallized`/`base_ability_name`（Pokemon）、
`power`/`critical_rank`（Move）、`name`/`n_won`/`n_game`（Player）、`opponent()`（Battle、対応済み）は
現行コードと一致していることを確認済み）

## 0. 使い勝手の違い（アーキテクチャレベル）

以降の章はプロパティ・命名レベルの差異一覧だが、その前提として **属性を合わせるだけでは埋まらない
アーキテクチャ上の違い** を先に整理する。`battle_against` のような使用感の再現を検討する際は、
どこを模倣し、どこは意図的に違えるかをここで切り分ける。

- **実行基盤**
  - poke-env: Pokémon Showdown サーバー（Node.js）に接続するクライアント。事前にサーバー起動が必要
  - jpoke: 外部プロセス不要。`Battle(players)` を作るだけで対戦できる純粋な Python ライブラリ
  - 対応方針: 差異はドキュメントで明示するのみ。jpoke の「サーバー不要ですぐ動く」点は参入障壁を下げる最大の強みなので解消（＝サーバー要求の追加）はしない

- **通信方式**
  - poke-env: WebSocket 経由のプロトコルメッセージ送受信
  - jpoke: プロセス内の関数呼び出しのみ
  - 対応方針: 対象外（分類C の「Showdown サーバー接続機能」と同一）

- **同期モデル**
  - poke-env: ネットワーク I/O が本質のため `battle_against` 等は `asyncio` 前提（`await`, `asyncio.run`）
  - jpoke: 同期処理のみで完結
  - 対応方針: jpoke に asyncio を持ち込む必要性は薄い。`battle_against`（`compat_plan.md` Phase 5）も同期メソッドとして提供し、非同期を強制しない（学習コストの低さを優先）

- **情報の可観測性**
  - poke-env: `Battle` は実プロトコルメッセージから構築されるため部分観測（自分から見える情報のみ。相手の非公開ステータス等は不可視）
  - jpoke: `Battle` は完全情報（両プレイヤーの `Pokemon` オブジェクトに直接アクセス可能）
  - 対応方針: 互換化の対象外。用途がバトルシミュレーション・検証である以上、完全情報が前提でよい。ただし `build_observation(observer)`（分類D）で部分観測ビューを明示的に得られることをドキュメントに明記し、poke-env 由来のユーザーの誤解を防ぐ

- **チームの定義方法**
  - poke-env: Showdown の「パックドチーム」文字列（Teambuilder 経由 or エクスポート）を渡す
  - jpoke: `Pokemon(...)` オブジェクトを Python コードで直接組み立てる
  - 対応方針: 将来検討事項。パックド形式のインポート機能は資産の使い回しという点で参入障壁低減効果が大きいが、実装コスト（フォーマットパース、日本語⇔英語の名称対応）が高いため優先度は低。分類 B の名前マッピング（B-2）が前提になる

- **フォーマット（対戦ルール）**
  - poke-env: `battle_format="gen9ou"` 等の文字列で多数のルールに対応
  - jpoke: ポケモンチャンピオンズのシングルバトル専用。フォーマット概念自体が存在しない
  - 対応方針: 対象外（分類C の「バトルフォーマット」と同一）

- **対戦結果の集計**
  - poke-env: `battle_against(*opponents, n_battles=1)` 実行後、`n_won_battles` 等が自動更新される
  - jpoke: `Player` にフィールドは存在するが `Battle` からの自動更新は未実装
  - 対応方針: `compat_plan.md` Phase 5 の `battle_against` 内で戦績を更新する（`Battle` 側に自動更新は追加しない。分類 A-2 の `win_rate` 等とセットで実装）

- **決定論的テスト機能**
  - poke-env: 実サーバーの乱数に依存するため命中率・追加効果確率を固定する公式手段はない
  - jpoke: `TestOption`（分類D）で命中率・追加効果確率・乱数を固定できる
  - 対応方針: jpoke 独自の強みとして維持。`battle_against`（`compat_plan.md` Phase 5）のドキュメントに「再現可能な対戦を組みたい場合は `TestOption` を使う」という導線を加える

- **ログ形式**
  - poke-env: プロトコルの生ログ行が `battle.observations` に蓄積される表示用テキスト
  - jpoke: `EventLogger` / `LogCode` による構造化ログ（`battle.print_logs()`）
  - 対応方針: 対象外。jpoke の構造化ログの方が高機能なので現状維持
...
| `won / lost` | `bool \| None` | 勝敗フラグ（プロパティ。未終了時は `None`） |
| `tied` | `bool` | 引き分けフラグ |
...
| `status_counter` | `int` | 状態異常の経過ターン数（TOX・SLP のみカウント） |
...
| `fainted` | `bool` | 瀕死か |
| `first_turn` | `bool` | 交代で場に出てから最初の行動か |
...
| `base_power` | `int` | 威力 |
| `accuracy` | `float` | 命中率（0〜1 スケール。旧版の `int \| bool` から変更され、必中も 1.0 になる） |
| `priority` | `int` | 優先度 |
| `current_pp` | `int` | 現在 PP |
| `max_pp` | `int` | 最大 PP |
| `crit_ratio` | `int` | 急所ランク補正 |
| `n_hit` | `tuple[int, int]` | （最小, 最大）ヒット数 |
| `expected_hits` | `float` | 期待ヒット数（2〜5回技は 35/35/15/15 分布の期待値 3.1。トリプルキック系は特別扱い） |
| `target` | `str` | ターゲット文字列 |
...
| メソッド / プロパティ | 説明 |
|---|---|
| `choose_move(battle) -> BattleOrder \| Awaitable[BattleOrder]` | **唯一の抽象メソッド**。行動選択 |
| `teampreview(battle) -> str` | チームプレビューでの選出文字列 |
| `create_order(order, mega, z_move, dynamax, terastallize, move_target) -> SingleBattleOrder` | オーダー生成ヘルパー（テラスタルは `terastallize: bool` 指定。タイプは指定しない） |
| `username: str` | ユーザー名 |
| `n_won_battles / n_lost_battles / n_tied_battles / n_finished_battles` | 戦績 |
| `win_rate: float` | 勝率 |
...
| `status_counter: int` | `ailment.count` | アクセスパス違い（poke-env 側は TOX・SLP のみカウントする点にも注意） | B-4 |
...
| `evs: Dict[str, int]` | `effort: list[int]` | 変数名・コンテナ型違い・**スケール違い**（poke-env: 0〜252、jpoke: Champions 形式 0〜32） | A-1: `effort` → `evs` に改名（docstring にスケール差を明記）+ B-3: インポート時に `evs_from_poke_env` で変換 |
...
| `first_turn: bool` | （`not acted_since_switch_in` で代替。`active_turn` フィールドは
  「Pokemonの一時状態をmemoryスコープに統一」リファクタで廃止済み） | 直接プロパティなし | A-2: property 追加 |
| `must_recharge: bool` | （`has_volatile("リチャージ")` で代替） | 直接プロパティなし | B-4 |
| `protect_counter: int` | （専用の数値カウンターは存在しない。まもる系連続使用の成否は
  `executed_move`（protectフラグの有無）と `failed_or_immobile_last_turn` の組み合わせで
  都度判定する決定的な二値チェックに再実装済み。`handlers/move_status.py` の
  `まもる系_連続使用失敗チェック` 参照） | poke-envの数値カウンター概念自体が存在しない | B-4（ただし変換元の数値がないため実質対象外） |
| `gender: PokemonGender` | `gender: Gender`（値: `"male" / "female" / ""`） | 型名のみ違い（値は英語化済みで互換） | C: 型名 `Gender` は改名しない（ユーザー判断） |
...
| `accuracy: float`（0〜1） | `accuracy: int \| None` | スケール違い（poke-env は 0〜1 の float。必中も 1.0 になり命中 100 と区別できない） | B-4 |
...
| `expected_hits: float` | （なし） | 直接プロパティなし | A-2: property 追加（poke-env の分布期待値に合わせる。`(min + max) / 2` は poke-env の値と一致しないため不採用） |
...
| `won / lost: bool \| None`（プロパティ） | `winner == self_player` | 直接フラグなし。poke-env は未終了時 `None` | A-2: `Player` を引数に取るメソッドとして追加（視点固定しない意図的な差異。未終了時は `False`） |
| `tied: bool` | （なし） | jpoke に引き分けは存在しない | C: 対象外 |
...
| `n_lost_battles` | （なし） | 直接プロパティなし | A-2: property 追加。あわせて `n_tied_battles`（jpoke に引き分けがないため常に 0）も追加 |
...
| 変更前（jpoke 現在） | 変更後 | 対象 | 状態（2026-07-04 再確認） |
|---|---|---|---|
...
型名 `Gender` → `PokemonGender` の改名は **行わない**（ユーザー判断。値は英語化済みで互換のため実害なし）。

**Literal 値の英語化**（`src/jpoke/types/literals.py`） — **値は対応済み**（2026-07-02 時点で確認。当初の想定より先行して実施されていた）
...
| `first_turn` | `not acted_since_switch_in` property（旧案の `active_turn == 0` は
  `active_turn` フィールド廃止に伴い変更） | `Pokemon` |
| `current_pp` | `pp` のエイリアス | `Move` |
| `max_pp` | `data.pp` のエイリアス（ユーザー判断） | `Move` |
| `expected_hits` | poke-env 準拠の分布期待値 property（固定回数はその値、2〜5回技は 35/35/15/15 分布で 3.1） | `Move` |
| `finished` | `winner is not None` property | `Battle` |
| `won` / `lost` | `winner` から計算する。poke-env と異なり `Player` を引数に取るメソッド（視点固定しない意図的差異） | `Battle` |
...
| `n_tied_battles` | 常に 0 を返す property（jpoke に引き分けなし。poke-env に `n_draw_battles` という名前は存在しない） | `Player` |
| `n_lost_battles` | `n_finished_battles - n_won_battles - n_tied_battles` property | `Player` |
| `win_rate` | `n_won_battles / n_finished_battles` property（ゼロ除算ガード付き。jpoke 独自判断） | `Player` |
...
| `fields` | `Field` Enum | 日本語 Literal（地形 4 種は `TerrainName`、じゅうりょく・トリックルーム等は `GlobalFieldName` に分離済み） |
| `side_conditions` / `opponent_side_conditions` のキー | `SideCondition` Enum | 日本語 Literal（`SideFieldName`。いやしのねがい等の slot condition は poke-env 側に対応なし） |
...
| `ivs` のインポート | `Dict[str, int]`（英語キー、0〜31） | `list[int]`（0〜31） | キー → インデックス変換のみ（スケール同一） |
| `evs` のインポート | `Dict[str, int]`（英語キー、0〜252） | `list[int]`（Champions 形式、0〜32） | キー → インデックス変換 + **スケール変換**（`evs_from_poke_env`。Champions 形式は 0 または 8x-4 相当のみ表現可能で非可逆） |
...
| 対象プロパティ | 差異 | 変換方法 |
|---|---|---|
| Move `accuracy` | 0〜1 の float（poke-env）vs 1〜100 の int / None = 必中（jpoke） | `round(accuracy * 100)`。poke-env 側では必中（1.0）と命中 100 が区別できないため、技 ID から jpoke の `MOVES` データを引ける場合はそちらを正とする |
| `status_counter` | `ailment.count`（アクセスパス違い） | インポート時に `Ailment.count` に設定（poke-env 側は TOX・SLP のみカウント） |
| `must_recharge` | `has_volatile("リチャージ")` で代替 | 揮発性状態名で判定 |
| `protect_counter` | 専用カウンターなし（`executed_move` + `failed_or_immobile_last_turn` の
  決定的チェックに置換済み。`Volatile.counter` という属性は存在しない） | 変換対象の数値がないため実質対象外。
  poke-env 側の逓減確率（1/3, 1/9…）を再現したい場合は別途カウンターの新設が必要 |
...
| `acted_since_switch_in: bool` | 場に出てから一度でも行動したか（`active_turn` フィールドは
  memoryスコープ統一リファクタで廃止され、本プロパティに置き換わった） |
...
src/jpoke/
  ...
  types/
    literals.py        ← 対応済み: Stat / MoveCategory / Gender の Literal 値英語化（型名 Gender は維持）
...
- `Stat` / `MoveCategory` / `Gender` の Literal 値英語化（A-1）は既に完了しているため、
...
詳細な実装計画は `.internal/poke-env/compat_plan.md` を参照。
