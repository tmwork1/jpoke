...
仕様差分の出典: `.internal/champions/changes_from_sv.md`, `.internal/spec/moves/_all_moves.md`
...
### 仕様変更
全技のPPが8・12・16・20の4段階に統一。`.internal/spec/moves/_all_moves.md` に全技の正しいPP値が記載されている。

### 対象ファイル
- `src/jpoke/data/move.py`（各 `MoveData` の `pp=` 引数を更新）
- `.internal/spec/moves/_all_moves.md` を参照して一括更新
...
進捗テーブル更新: `.internal/progress/ailment.md` の「まひ」行を `12.5%` に修正。
