...
など。`.internal/spec/abilities/マルチスケイル.md` 16行目に明記）で誤った結果になる。既存テスト
...
登録されておらず、リーサル計算では完全に無視されている（`.internal/progress/ability.md` /
...
def がんじょう_survive_lethal(battle: Battle, ctx: LethalContext, hp_dist: StateDist) -> StateDist:
    ...
    何度でも発動する。.internal/spec/abilities/がんじょう.md 20行目参照。
    """
...
きあいのタスキは消費されない」という仕様(`.internal/spec/items/きあいのタスキ.md` 26行目)を
...
| `.internal/progress/ability.md` / `.internal/progress/item.md` | がんじょう・きあいのタスキ・マルチスケイル・ファントムガード・テラスシェルの `リーサル実装` / `リーサルテスト` 列を `n/a` → `x` に更新 |
