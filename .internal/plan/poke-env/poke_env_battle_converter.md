...
- 方針: poke-env の `Battle` オブジェクト（進行中の対戦状態）を渡すと、jpoke の `Battle`
  インスタンスを生成する変換機構を作る。既存の poke-env 互換実装（`.internal/poke-env/compat_plan.md`
  Phase 1〜5、mainにマージ済み）はプロパティ名・値レベルの互換に留まっていたが、本計画は
  battle レベルのファクトリを新設する点で毛色が異なる。

## 前提

- `.internal/poke-env/compat_plan.md`（Phase 1〜5）・`.internal/poke-env/compat_analysis.md` は
...
### 調査で判明した重要事項（`.internal/poke-env/api_reference/` の poke-env 公式ドキュメントより）
...
- `.internal/poke-env/compat_plan.md` に「Phase 6: Battle変換機構」として本計画の要約を追記し、
  `.internal/poke-env/compat_analysis.md` にも battle-level converter の存在を追記する
