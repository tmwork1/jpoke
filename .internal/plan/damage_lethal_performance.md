...
なお上表は §4 Phase 1 + 2-1〜2-3 を適用した時点の数値で、
後から追加検討した **2-4（確殺枝の畳み込み）は含まない**。
2-4 は致命打が出るケースでさらに −3〜10 ポイント上乗せされる（§2-4 の表を参照）。
...
#### 2-4. 確殺枝の畳み込み（HP=0 ショートカット）

`src/jpoke/utils/lethal_dist.py`（2-3 の `_combine` に組み込む）

**前提の確認**: 「HP=0 が現れたら計算を打ち切る」という粗い短絡は **すでに実装済み**。
`fainted()`（`lethal.py:144`）が以下の3か所で効いている。

| 位置 | 効果 |
|---|---|
| `_lethal_loop`（1ヒット後 / ターン終了後） | HP=0 が1枝でも出たら `return results` |
| `_emit`（イベント入口） | HP=0 なら `_get_handlers` ごとスキップ |
| `_apply_handlers`（ハンドラごと） | HP=0 になった時点で `break` |

したがって「死んだ枝を抱えたまま次の攻撃を計算し続ける」無駄は元々発生しない。
残っているのは **HP=0 に落ちる枝そのものの展開コスト**である。

`subtract_dist(hp_dist, damage_dist, minimum=0)` は、`hp - min(damage) <= 0` の枝
（＝どの乱数でも確実に落ちる枝）についても律儀に16通りを展開し、
16回とも `State(0, ...)` へ加算している。この枝は
**`freq × sum(damage_dist.values())` を `State(0, ...)` に1回加算するだけで等価**。

```python
        # _combine 内、sign == -1 かつ minimum is not None かつ plain のとき
        if vx - min_y <= minimum:
            # どの乱数でも下限に張り付く枝 → len(y) 通りの展開を省く
            result[State(minimum, ax, ix)] += fx * total_y
            continue
```

`min_y = min(k.value for k in y)` と `total_y = sum(y.values())` はループ外で1回求める。
`plain`（`y` 側のフラグが全て `True`）でないと `State` のフラグを畳めないので、
2-3 で導入する `plain` 判定を再利用し、`plain` が偽のときは通常経路に落とすこと。

**実測効果**（2-3 の1パス融合を入れた状態からの追加分。出力は全ケース完全一致）:

| ケース | main | 2-3 のみ | 2-3 + 2-4 |
|---|---:|---:|---:|
| ピカチュウ→ハピナス でんこうせっか ×10（8ヒット） | 4.241 ms | 3.129 ms (0.74) | **3.022 ms (0.71)** |
| ガブリアス→カイリュー（マルスケ+オボン）げきりん（2ヒット） | 1.312 ms | 1.191 ms (0.91) | **1.060 ms (0.81)** |
| ガブリアス→ハバタクカミ じしん（確1） | 0.715 ms | 0.737 ms (1.03) | **0.692 ms (0.97)** |
| ハバタクカミ→ハピナス シャドーボール ×10（致死せず） | 1.833 ms | 1.869 ms (1.02) | 1.766 ms (0.96) |
| ハピナス→ハピナス たいあたり ×10（致死せず） | 2.979 ms | 2.794 ms (0.94) | 2.798 ms (0.94) |

実際に致命打が出るケース（上2行）で **追加 −3〜10ポイント**。
致死しないケースでは短絡が発火しないので誤差の範囲。
数行で済むうえ `_combine` の内側なので、**2-3 と同じ PR に含めること**。

> **試して不採用にした案**: 逆方向の「誰も死なない枝（`vx - max_y > minimum`）は
> クランプ判定を省いた高速ループに流す」短絡も実装して測ったが、
> 分岐追加のコストが `if hp < minimum` の削減を上回り、
> でんこうせっか×10 で 0.71 → 0.76、じしん（確1）で 0.97 → 1.16 と
> **むしろ悪化した**。採用しないこと。
...
#### 3-3. `fainted` / `_update_hp` の重複走査の削減

`fainted()` は `any(state.value == 0 for state in dist)`、
`_update_hp()`（`lethal.py:616`）は `min(state.value for state in hp_dist)` で、
**どちらも分布全体を毎回線形走査する**。呼び出し回数を実測すると:

| ケース | `fainted` 呼び出し / 走査 State 数 | `_update_hp` 呼び出し / 走査 State 数 |
|---|---|---|
| ピカチュウ→ハピナス でんこうせっか ×10 | 46 回 / 1,258 | 68 回 / 1,850 |
| ガブリアス→カイリュー じしん ×10 | 100 回 / 100 | 90 回 / 90 |

`_emit` は入口で `fainted` を呼び、その直後 `_apply_handlers` が
**同じ分布に対してもう一度** `fainted` を呼ぶ（重複）。
また `_emit` はハンドラが1つも無い場合でも `_update_hp` を2回呼び、
`min()` の全走査を無駄に行う（分布が変わっていないので不要）。

**改善案**: `_emit` から重複する `fainted` を除き、
`handlers` / `every_event_handlers` が空なら対応する `_update_hp` ごとスキップする
（`defender.hp == min(hp_dist)` の不変条件は直前の `_update_hp` で保たれているため安全）。

**実測効果**: でんこうせっか×10 で 4.036 → **3.597 ms（−11%）**。
他の4ケースは 0.98〜1.04 倍で**誤差レベル**。
cProfile 上でも `fainted` + `_update_hp` の合計は全体の約 3% にとどまる。

分布メタデータ（`has_zero` / `min_hp`）を `_combine` の副産物として持ち回れば
これらを O(1) にできるが、`StateDist = dict[State, int]` という型を
`handlers/lethal.py` の全ハンドラが直接扱っているため**戻り値型の変更が広範囲に波及する**。
費用対効果が合わないので、上記の「重複を削るだけ」の範囲に留めること。
...
| **PR5** | Phase 2-3 + 2-4（`add_dist`/`subtract_dist` の1パス化・確殺枝の畳み込み + 死んだヘルパー削除） | 中 | A, C |
