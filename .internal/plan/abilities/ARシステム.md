...
- `.internal/spec/abilities/ARシステム.md`
...
- `.internal/spec/turn.md` に ARシステム/マルチタイプ 個別の記載はないため、同種の
...
- へんしん（Transform）は `.internal/progress/move.md` で実装保留のため、
...
- マルチタイプ（プレート）側にも同様の「相手の道具が交換対象なら失敗する」抜けが
  ...
  `is_exchange` 判定を追加して修正済み（`.internal/review/abilities/マルチタイプ.md` 参照）。
