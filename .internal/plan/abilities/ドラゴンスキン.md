...
|---------|---------|-------------|--------|
| `ON_MODIFY_MOVE_TYPE` | 100 | `attacker:self` | `ドラゴンスキン_modify_move_type` |
| `ON_CALC_POWER_MODIFIER` | 100 | `attacker:self` | `ドラゴンスキン_modify_power` |

`ON_MODIFY_MOVE_TYPE` / `ON_CALC_POWER_MODIFIER` はいずれも攻撃フェーズ（`AttackContext`）のイベントのため、
`subject_spec` は CLAUDE.md の約束事に従い `attacker:self` を使う（`source:self` は非攻撃フェーズ用のため誤り）。
