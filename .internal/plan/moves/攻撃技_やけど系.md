...
`.internal/spec/moves/_move_secondary_1.md` の「やけど」列（13列目）が非0の技が対象。
命中時に確率でやけど状態を相手に付与する。

- イベント: `ON_HIT`
- priority: 100（`.internal/spec/turn.md` の `Event.ON_DAMAGE` セクション「追加効果（特殊なもの除く）」priority=10 に記載されているが、`ON_HIT` は別イベントで priority=100 が既存の類似ハンドラ（`でんじほう`、`ほうでん` 等）と一致）
...
- **ダークファイア**: 第三世代『ポケモンXD 闇の旋風ダーク・ルギア』のみに登場するダークタイプの専用技（ファイヤー(XD)専用）。
  ...
  一次情報（`.internal/wiki/moves/ダークファイア.html`）では タイプ=あく、分類=特殊、威力=75、命中率=100、PP=-（不明）、
...
`.internal/spec/turn.md` の `Event.ON_HIT` セクション（行474-490）には「追加効果」の記載なし。
...
**ブレイズキック**（critical_rank=1 を保持。個別レビューは .internal/plan/moves/ブレイズキック.md 参照。
...
### やけど無効条件
- `.internal/spec/turn.md` の `Event.ON_TRY_MOVE_2` セクション（行345）に「ほのおタイプ: やけど無効」がある。
