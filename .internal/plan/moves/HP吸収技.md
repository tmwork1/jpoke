...
`.internal/spec/moves/_move_secondary_2.md` の「HP吸収(与ダメ比)」列が非0の技をすべて対象とする。
...
> `.internal/plan/moves/むねんのつるぎ.md` を参照。本ファイルの以下の記載
...
> `.internal/wiki/moves/ウッドホーン.html`（および ギガドレイン・きゅうけつ 等の同種Wikiページ）の
...
> 詳細は `.internal/plan/moves/ウッドホーン.md` を参照。
