...
| 実装済み       |  207 | handlers 193 件（実装対象外との重複 1 件除く 192 件）+ 実装対象外 15 件           |
| 未実装         |   93 | 300 - 207                                                                        |
| コード実装済み |  193 | `src/jpoke/data/ability.py` で `AbilityData(handlers={...})` を明示した特性      |
...
| テスト実装済み |  149 | `tests/test_ability.py` に専用テストあり                                         |


## 特性一覧（実装状況付き）

- 実装対象総数: 282件（✓ 184件 / - 98件）
