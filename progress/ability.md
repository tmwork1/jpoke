# 特性実装進捗

更新日: 2026-05-05（不適切なかたやぶりテスト削除・進捗表整合）

## 実装状況サマリ（進捗管理基準）

| 項目           | 件数 | 根拠                                                                             |
| -------------- | ---: | -------------------------------------------------------------------------------- |
| 特性総数       |  300 | `len(ABILITIES)`                                                                 |
| 実装済み       |  213 | handlers 199 件（実装対象外との重複 1 件除く 198 件）+ 実装対象外 15 件           |
| 未実装         |   87 | 300 - 213                                                                        |
| コード実装済み |  199 | `src/jpoke/data/ability.py` で `AbilityData(handlers={...})` を明示した特性      |
| 実装対象外     |   15 | ダブル専用 12 件 + 対戦に影響なし 3 件                                           |
| 仕様書あり     |  291 | `spec/ability/` にファイルあり                                                   |
| 仕様具体化済み |  291 | `spec/ability/` 内で `要確認` を含まない仕様書ファイル数                         |
| 要確認残件     |    0 | `spec/ability/` 内の `要確認` ヒット件数                                         |
| テスト実装済み |  187 | `tests/test_ability.py` に専用テストあり（かたやぶり相互作用テスト含む）          |


## 特性一覧（実装状況付き）

- 実装対象総数: 282件（✓ 190件 / - 92件）
- 方針: 実装済み/未実装を分離せず、単一テーブルの 実装 列を記号（✓/-）で管理する

- 凡例: ✓ = 実装済み/テスト済み, - = 未実装/未テスト

| 特性             | 実装 | 優先度 | 仕様書 | テスト | かがくへんかガステスト | かたやぶりテスト | 備考                         |
| ---------------- | ---- | ------ | ------ | ------ | ------------ | ---------- | ---------------------------- |
| あめふらし       | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 入場時にあめ展開             |
| ありじごく       | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| いかく           | ✓    | -      | ✓      | ✓      | ✓        | N/A  | かがくへんかガス_登場中はいかくが不発で確認済み |
| おうごんのからだ | ✓    | -      | ✓      | ✓      | ✓        | ✓      |                              |
| おやこあい       | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 2ヒット化（2ヒット目減衰）   |
| かがくへんかガス | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| かげふみ         | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| かたやぶり       | ✓    | -      | ✓      | -      | -        | N/A  |                              |
| カブトアーマー   | ✓    | -      | ✓      | ✓      | ✓        | ✓      | 急所ランクを0に補正          |
| かちき           | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| ぎゃくじょう     | ✓    | -      | ✓      | ✓      | ✓        | N/A  | HP半分跨ぎで特攻+1           |
| ききかいひ       | ✓    | -      | ✓      | -      | -        | N/A  | HP半分跳びで緑急交代         |
| きんちょうかん   | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| クリアボディ     | ✓    | -      | ✓      | ✓      | ✓        | ✓      | 能力ランク低下を無効化       |
| こだいかっせい   | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| じゅうなん       | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| じりょく         | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| しゅうかく       | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| すいすい         | ✓    | -      | ✓      | ✓      | ✓        | N/A  | あめ中に素早さ2倍            |
| すなかき         | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| すなおこし       | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 入場時にすなあらし展開       |
| ぜったいねむり   | ✓    | -      | ✓      | ✓      | N/A      | N/A  |                              |
| ちからもち       | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 物理技の攻撃補正2倍          |
| てつのこぶし     | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| テクニシャン     | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 威力60以下の技を1.5倍        |
| てんねん         | ✓    | -      | ✓      | ✓      | ✓        | ✓      |                              |
| どんかん         | ✓    | -      | ✓      | ✓      | ✓        | ✓      |                              |
| にげごし         | ✓    | -      | ✓      | ✓      | -        | N/A  | HP半分跏ぎで緑急交代         |
| ねんちゃく       | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| はらぺこスイッチ | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| ばけのかわ       | ✓    | -      | ✓      | ✓      | N/A      | ✓      |                              |
| バトルスイッチ   | ✓    | -      | ✓      | ✓      | N/A      | N/A  |                              |
| ひでり           | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 入場時にはれ展開             |
| ふゆう           | ✓    | -      | ✓      | ✓      | ✓        | ✓      | 浮遊状態を付与               |
| ふしょく         | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 有効時のみ毒・鋼へ付与可     |
| ふみん           | ✓    | -      | ✓      | -      | -        | ✓      |                              |
| ぶきよう         | ✓    | -      | ✓      | ✓      | -        | N/A  | 道具効果を無効化             |
| へんげんじざい   | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 場で1回のみ、交代リセット    |
| めんえき         | ✓    | -      | ✓      | -      | -        | ✓      |                              |
| みずのベール     | ✓    | -      | ✓      | -      | -        | ✓      |                              |
| やるき           | ✓    | -      | ✓      | -      | -        | ✓      |                              |
| ゆきふらし       | ✓    | -      | ✓      | ✓      | ✓        | N/A  | 入場時にゆき展開             |
| ようりょくそ     | ✓    | -      | ✓      | ✓      | ✓        | N/A  | はれ中に素早さ2倍            |
| ヨガパワー       | ✓    | -      | ✓      | ✓      | ✓        | N/A  | ちからもちと同仕様           |
| わるいてぐせ     | ✓    | -      | ✓      | -      | -        | N/A  |                              |
| クォークチャージ | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| エレキメイカー   | ✓    | -      | ✓      | ✓      | -        | N/A  | 入場時にエレキフィールド展開 |
| グラスメイカー   | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| サイコメイカー   | ✓    | -      | ✓      | ✓      | -        | N/A  | 入場時にサイコフィールド展開 |
| ターボブレイズ   | ✓    | -      | ✓      | -      | -        | N/A      |                              |
| テラボルテージ | ✓    | -      | ✓      | -      | -        | N/A      |                              |
| トレース         | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| マイティチェンジ | ✓    | -      | ✓      | ✓      | N/A      | N/A  | フォルムチェンジ（交代時）   |
| マイペース       | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| マグマのよろい   | ✓    | -      | ✓      | -      | -        | ✓      |                              |
| マジックガード   | ✓    | -      | ✓      | ✓      | -        | N/A  | 間接ダメージ無効化           |
| マジックミラー   | ✓    | -      | ✓      | ✓      | ✓        | ✓      | 変化技反射                   |
| マジシャン       | ✓    | -      | ✓      | -      | -        | N/A      |                              |
| ミストメイカー   | ✓    | -      | ✓      | ✓      | -        | N/A  | 入場時にミストフィールド展開 |
| ミラーアーマー   | ✓    | -      | ✓      | ✓      | -        | ✓      | 能力ランク低下反射           |
| ノーガード       | ✓    | -      | ✓      | ✓      | -        | N/A  | 命中判定を必中化             |
| リベロ           | ✓    | -      | ✓      | ✓      | -        | N/A  | へんげんじざいと同仕様       |
| アナライズ       | ✓    | -      | ✓      | -      | -        | N/A  |                              |
| いろめがね       | ✓    | -      | ✓      | ✓      | ✓        | N/A  |                              |
| いわはこび       | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| かたいツメ       | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| きよめのしお     | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| くさのけがわ     | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| こおりのりんぷん | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| こんじょう       | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| しんりょく       | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| すいほう         | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| すなのちから     | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| スナイパー       | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| スロースタート   | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| たいねつ         | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| トランジスタ     | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| ねつぼうそう     | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| はがねつかい     | ✓    | -      | ✓      | -      | -        | N/A  |                              |
| はがねのせいしん | ✓    | -      | ✓      | -      | -        | N/A  |                              |
| はりこみ         | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| ひひいろのこどう | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| ふしぎなうろこ   | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| ファーコート     | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| ファントムガード | ✓    | -      | ✓      | ✓      | -        | N/A    |                              |
| フィルター       | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| ブレインフォース | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| もうか           | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| もふもふ         | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| マルチスケイル   | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| メガランチャー   | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| よわき           | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| りゅうのあぎと   | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| わざわいのおふだ | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| わざわいのうつわ | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| わざわいのたま   | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| わざわいのつるぎ | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| がんじょうあご   | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| げきりゅう       | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| ごりむちゅう     | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| ハードロック     | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| ハドロンエンジン | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| パンクロック     | ✓    | -      | ✓      | ✓      | -        | ✓      | 音技威力1.3倍＋被ダメ0.5倍   |
| プリズムアーマー | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| むしのしらせ     | ✓    | -      | ✓      | ✓      | -        | N/A  |                              |
| あついしぼう     | ✓    | -      | ✓      | -      | -        | ✓      |                              |
| あまのじゃく     | ✓    | -      | ✓      | -      | -        | ✓      | 能力ランク低下を攻撃ランク上昇に変換     |
| いたずらごころ   | ✓    | -      | ✓      | ✓      | -        | N/A  | 変化技の優先度+1、あくタイプ無効         |
| ちょすい         | ✓    | -      | ✓      | ✓      | ✓        | ✓      | みず技無効化、HP1/4回復                 |
| どくぼうそう     | ✓    | -      | ✓      | -      | -        | N/A  |                                          |
| どしょく         | ✓    | -      | ✓      | ✓      | ✓        | ✓      | くさ技無効化、HP1/4回復                 |
| ひらいしん       | ✓    | -      | ✓      | ✓      | ✓        | ✓      | でんき技誘導・無効化、特攻+1             |
| もらいび         | ✓    | -      | ✓      | ✓      | ✓        | ✓      | ほのお技無効化、強化時ほのお技1.5倍      |
| よびみず         | ✓    | -      | ✓      | ✓      | ✓        | ✓      | みず技誘導・無効化、特攻+1               |
| スキルリンク     | ✓    | -      | ✓      | -      | -        | N/A  | 連続技のヒット数を最大に固定             |
| あくしゅう       | -   | L      | ✓      | -      | -        | N/A  |                              |
| あとだし         | ✓   | -      | ✓      | ✓      | -        | N/A      |                              |
| あめうけざら     | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| いかりのこうら   | -   | L      | ✓      | -      | -        | N/A  |                              |
| いかりのつぼ     | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| いしあたま       | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| うのミサイル     | ✓   | L      | ✓      | -      | N/A      | N/A  |                              |
| うるおいボイス   | -   | L      | ✓      | -      | -        | N/A  |                              |
| うるおいボディ   | -   | L      | ✓      | -      | -        | N/A  |                              |
| えんかく         | -   | L      | ✓      | -      | -        | N/A  |                              |
| おどりこ         | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| おみとおし       | -   | L      | ✓      | -      | -        | N/A  |                              |
| おもかげやどし   | -   | L      | ✓      | -      | -        | N/A  |                              |
| おわりのだいち   | ✓   | -      | ✓      | ✓      | -      | N/A  | 入場時におおひでり展開       |
| かいりきバサミ   | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| かぜのり         | -   | L      | ✓      | -      | -        | -  |                              |
| かそく           | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| かるわざ         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                             |
| かんそうはだ     | ✓   | -      | ✓      | -      | -        | ✓      |                              |
| かんろなミツ     | -   | L      | ✓      | -      | -        | N/A  |                              |
| がんじょう       | ✓   | -      | ✓      | -      | -        | ✓  |                              |
| きけんよち       | -   | L      | ✓      | -      | -        | N/A  |                              |
| きもったま       | -   | L      | ✓      | -      | -        | N/A  |                              |
| きゅうばん       | ✓   | -      | ✓      | ✓      | -        | ✓  |                              |
| きょううん       | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| きれあじ         | ✓   | -      | ✓      | ✓      | -        | N/A  |                              |
| きんしのちから   | -   | L      | ✓      | -      | -        | N/A  |                              |
| ぎたい           | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| くいしんぼう     | -   | L      | ✓      | -      | -        | N/A  |                              |
| くだけるよろい   | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| くろのいななき   | -   | L      | ✓      | -      | -        | N/A  |                              |
| こぼれダネ       | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| こんがりボディ   | -   | L      | ✓      | -      | -        | -  |                              |
| さいせいりょく   | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| さまようたましい | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| さめはだ         | ✓   | L      | ✓      | ✓      | -      | N/A  |                              |
| しぜんかいふく   | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| しめりけ         | ✓   | -      | ✓      | ✓      | ✓        | ✓      |                              |
| しょうりのほし   | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| しろいけむり     | ✓   | -      | ✓      | ✓      | ✓        | ✓      |                              |
| しろのいななき   | -   | L      | ✓      | -      | -        | N/A  |                              |
| しんがん         | -   | L      | ✓      | -      | -        | -  |                              |
| じきゅうりょく   | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| じしんかじょう   | ✓   | -      | ✓      | ✓      | -        | N/A  |                              |
| じゅくせい       | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| じょうききかん   | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| じょおうのいげん | -   | L      | ✓      | -      | -        | -      |                              |
| じんばいったい   | -   | L      | ✓      | -      | -      | N/A  |                              |
| すじがねいり     | -   | L      | ✓      | -      | -        | N/A  |                              |
| すてみ           | -   | L      | ✓      | -      | -        | N/A  |                              |
| すながくれ       | -   | L      | ✓      | -      | -        | -  |                              |
| すなはき         | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| すりぬけ         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| するどいめ       | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| せいぎのこころ   | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| せいしんりょく   | ✓   | -      | ✓      | ✓      | ✓        | ✓      | ひるみ防止・いかく無効       |
| せいでんき       | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| そうしょく       | ✓   | L      | ✓      | ✓      | -        | ✓      |                              |
| そうだいしょう   | -   | L      | ✓      | -      | -        | N/A  |                              |
| たんじゅん       | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| だっぴ           | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| ちからずく       | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ちくでん         | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| ちどりあし       | -   | L      | ✓      | -      | -        | -      |                              |
| てきおうりょく   | ✓   | -      | ✓      | ✓      | -        | N/A  |                              |
| てつのトゲ       | ✓   | -      | ✓      | ✓      | -      | N/A  |                              |
| てんきや         | -   | L      | ✓      | -      | -        | N/A  |                              |
| てんのめぐみ     | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| でんきにかえる   | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| でんきエンジン   | ✓    | -      | ✓      | ✓      | -        | ✓      |                              |
| とうそうしん     | -   | L      | ✓      | -      | -        | N/A  |                              |
| とれないにおい   | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| どくくぐつ       | -   | L      | ✓      | -      | -        | N/A  |                              |
| どくげしょう     | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| どくしゅ         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| どくのくさり     | -   | L      | ✓      | -      | -        | N/A  |                              |
| どくのトゲ       | ✓   | -      | ✓      | ✓      | -      | N/A  |                              |
| なまけ           | -   | L      | ✓      | -      | -        | N/A  |                              |
| ぬめぬめ         | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ねつこうかん   | ✓   | -      | ✓      | ✓      | ✓        | ✓      |                              |
| のろわれぜぼぱて   | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| はじまりのうみ   | ✓   | -      | ✓      | ✓      | -      | N/A  | 入場時におおあめ展開         |
| はんすう         | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| はっこう         | -   | L      | ✓      | -      | -        | -      |                              |
| はとむね         | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| はやあし         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| はやおき         | -   | L      | ✓      | -      | -        | N/A  |                              |
| はやてのつばさ   | -   | L      | ✓      | -      | -        | N/A  |                              |
| はりきり         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| ばんけん         | -   | L      | ✓      | -      | -        | -      |                              |
| ひとでなし       | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| びびり           | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| びんじょう       | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| ふうりょくでんき | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ふかしのこぶし   | -   | L      | ✓      | -      | -        | N/A  |                              |
| ふくがん         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| ふくつのこころ   | -   | L      | ✓      | -      | -        | N/A  |                              |
| ふくつのたて     | -   | L      | ✓      | -      | -        | N/A  |                              |
| ふとうのけん     | -   | L      | ✓      | -      | -        | N/A  |                              |
| へんしょく       | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| ほうし           | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| ほおぶくろ       | -   | L      | ✓      | -      | -        | N/A  |                              |
| ほのおのからだ   | ✓   | -      | ✓      | ✓      | -      | N/A  |                              |
| ほろびのボディ   | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| ぼうおん         | ✓   | -      | ✓      | ✓      | ✓        | ✓      |                              |
| ぼうじん         | ✓   | -      | ✓      | ✓      | ✓        | ✓      | 粉技無効・砂嵐免除           |
| ぼうだん         | ✓   | -      | ✓      | ✓      | ✓        | ✓      | 弾技無効                     |
| まけんき         | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| みずがため       | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| ものひろい       | -   | L      | ✓      | -      | -        | N/A  |                              |
| ゆうばく         | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| ゆきかき         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| ゆきがくれ       | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| よちむ           | -   | L      | ✓      | -      | -        | N/A  |                              |
| りんぷん         | ✓   | -      | ✓      | ✓      | ✓        | ✓      |                              |
| わたげ           | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| アイスフェイス   | -   | L      | ✓      | -      | N/A      | -      |                              |
| アイスボディ     | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| アロマベール     | -   | L      | ✓      | -      | -        | -      |                              |
| エアロック       | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| オーラブレイク   | -   | L      | ✓      | -      | -        | -      |                              |
| カーリーヘアー   | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| クイックドロウ   | -   | L      | ✓      | -      | -        | N/A  |                              |
| サンパワー       | ✓   | -      | ✓      | ✓      | ✓        | ✓      |                              |
| サーフテール     | -   | L      | ✓      | -      | -        | N/A  |                              |
| シェルアーマー   | -   | L      | ✓      | -      | -        | -      |                              |
| シンクロ         | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| スイートベール   | -   | L      | ✓      | -      | -        | -      |                              |
| スカイスキン     | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| スワームチェンジ | -   | L      | ✓      | -      | N/A      | N/A  |                              |
| ゼロフォーミング | -   | L      | ✓      | -      | -        | N/A  |                              |
| ソウルハート     | -   | L      | ✓      | -      | -        | N/A  |                              |
| ダウンロード     | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| ダークオーラ     | ✓   | L      | ✓      | -      | -        | -      |                              |
| テイルアーマー   | -   | L      | ✓      | -      | -        | -      |                              |
| テラスシェル     | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| テラスチェンジ   | -   | L      | ✓      | -      | N/A      | N/A  |                              |
| デルタストリーム | ✓   | -      | ✓      | ✓      | -      | N/A  | 入場時にらんきりゅう展開     |
| ナイトメア       | -   | L      | ✓      | -      | -        | N/A  |                              |
| ノーてんき       | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ノーマルスキン   | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| バリアフリー     | -   | L      | ✓      | -      | -        | N/A  |                              |
| パステルベール   | -   | L      | ✓      | -      | -        | -      |                              |
| パワースポット   | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| ヒーリングシフト | -   | L      | ✓      | -      | -        | N/A  |                              |
| ビビッドボディ   | -   | L      | ✓      | -      | -        | -      |                              |
| ビーストブースト | -   | L      | ✓      | -      | -        | N/A  |                              |
| フェアリーオーラ | ✓   | L      | ✓      | -      | -        | -      |                              |
| フェアリースキン | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| フラワーギフト   | -   | L      | ✓      | -      | -        | -      |                              |
| フラワーベール   | -   | L      | ✓      | -      | -        | -      |                              |
| フリーズスキン | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| フレンドガード   | -   | L      | ✓      | -      | -        | -      |                              |
| プレッシャー     | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ヘドロえき       | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ヘヴィメタル     | -   | L      | ✓      | -      | -        | -      |                              |
| ポイズンヒール   | ✓   | -      | ✓      | ✓      | -        | N/A  |                              |
| マルチタイプ     | ✓   | -      | ✓      | -      | N/A      | N/A  |                              |
| ミイラ           | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ミラクルスキン   | -   | L      | ✓      | -      | -        | -      |                              |
| ムラっけ         | ✓   | -      | ✓      | ✓      | ✓        | N/A  |                              |
| メタルプロテクト | ✓   | L      | ✓      | -      | -        | N/A  |                              |
| メロメロボディ   | ✓   | -      | ✓      | -      | -        | N/A  |                              |
| ライトメタル     | -   | L      | ✓      | -      | -        | -      |                              |
| リミットシールド | ✓   | L      | ✓      | -      | N/A      | N/A  |                              |
| リーフガード     | ✓   | -      | ✓      | ✓      | -        | ✓      |                              |
| ARシステム       | ✓   | -      | ✓      | -      | N/A      | N/A  |                              |
## 実装保留（1件）

| 特性           | 理由           |
| イリュージョン  | 処理が複雑     |
| かわりもの      | 処理が複雑     |

## リファクタリング（2026-05-01）

### コード品質改善: ハンドラモジュール統廃合

◆ **実施内容**
- 重複するユーティリティ関数を集約: `src/jpoke/handlers/common.py`
  - `is_berry_item()` - アイテム名から木の実判定 (ability.py, move.py から統一)
  - `apply_modifier()` - 4096基準の補正値計算 (ability.py で15+ 呼び出し統一)
  - `crossed_half_hp()` - HP 50% 閾値踏破判定 (ability.py で2箇所統一)

◆ **修正対象**
- `src/jpoke/handlers/move.py`
  - `_is_berry()` 削除 → `common.is_berry_item()` に統一
  - 2 呼び出し箇所更新 (ついばむ, やきつくす)

- `src/jpoke/handlers/ability.py`
  - `_is_berry_item()`, `_apply_modifier()`, `_crossed_half_hp()` 削除
  - 45 呼び出し箇所を `common.xxx` に統一
  - 特に `_apply_modifier()` は28箇所、`_crossed_half_hp()` は2箇所、`_is_berry_item()` は1箇所

◆ **テスト検証**
- ✓ `tests/test_ability.py`: 192/192 テスト合格
- ✓ リグレッション: なし

◆ **将来の最適化候補**
- item.py の `modify_power_by_type()`, `modify_super_effective_damage()` の common.py 整理検討
  - 現在: item.py 内で 30+ 呼び出し (partial でデータの定義で使用)
  - 複数モジュール再利用の可能性あり（優先度: 低）

## 実装対象外特性（15件）

| 特性             | 理由           |
| ---------------- | -------------- |
| いやしのこころ　| ダブル専用     |
| おもてなし　　　| ダブル専用     |
| きみょうなくすり| ダブル専用     |
| きょうえん　　　| ダブル専用     |
| きょうせい　　　| ダブル専用     |
| しれいとう　　　| ダブル専用     |
| スクリューおびれ| ダブル専用     |
| たまひろい　　　| 対戦に影響なし |
| にげあし　　　　| 対戦に影響なし |
| みつあつめ　　　| 対戦に影響なし |
| テレパシー　　　| ダブル専用     |
| バッテリー　　　| ダブル専用     |
| プラス　　　　　| ダブル専用     |
| マイナス　　　　| ダブル専用     |
| レシーバー　　　| ダブル専用     |
