---
name: planner
description: jpoke 向け計画書作成エージェント。仕様書を読み込み、handlers/・data/ の実装コードを含む計画書を .internal/plan/ に出力する。impl エージェントに渡す前段として使う。
tools:
  - Read
  - Write
  - Glob
...
- `.internal/spec/` の仕様書と `.internal/spec/turn.md` を読んで設計を行う
- ハンドラ構成・subject_spec・priority・実装コードを含む計画書を `.internal/plan/{category}/{entry}.md` に出力する
- コードの書き込みは行わない（実装は `impl` エージェントが担当）

## 手順

1. **仕様を読む**
   - `.internal/spec/` 以下の対応仕様書を探して読む（moves/, abilities/, items/, volatiles/, side_fields/, global_fields/ も確認）
   - `.internal/spec/turn.md` を読み、対象イベントの priority を確認する
...
3. **計画書を書く**
   ...
   ### Priority 根拠
   - `.internal/spec/turn.md` に記載があれば引用、なければ類似ハンドラを根拠として明記
...
- priority は `.internal/spec/turn.md` を必ず確認する
