...
`isolate_reruns=100, extra_fullsuite_reruns=5, verify_reruns=100, total_runs=0` で新規作成）。
...
結果確認までに長時間待たされることを避けるため、フルスイートは**1回だけ**実行する（複数回を
1ターンにまとめて連続実行しない）:
...
`total_runs += 1`（成功・失敗を問わず今回実行した1回分としてカウントする）。

### 3. 今回成功の場合

状態を保存して §共通7 に従いターンを終える（「run_{n}: flaky test無し（累計{total_runs}回）」と
報告）。次回の起動（ユーザーの `/loop flaky` 再実行、または背景エージェント完了通知）で手順1に戻り、
手順2で次の1回を実行する。これを繰り返すことで「1回実行 → 問題があれば手順4で修正 → 次の1回実行 →
…」を無限に継続する。
...
5. 変更をすべてコミットする（作業は `loop/flaky` ブランチ上で行う）:
   git add -A
...
再開する）。再開時は手順1から入り、手順2で次の1回のフルスイート実行に進む。
...
main への反映が完了した後、§共通16 の判断基準を適用する（`completed` 台帳の `summary` を見て、
本質的に同じ原因パターン——モック漏れの箇所違い・同種の状態汚染の再発等——が繰り返されていないか
確認する）。体系的対応が必要と判断した場合は §共通16 の手順で `general-purpose` サブエージェントに
横断リファクタを委譲し、成功したら §共通16 の後処理（テスト全体実行・進捗更新・コミット・
main反映）に従う。単発修正で十分な場合は何もせず手順5（次の調査対象へ）に進む。
