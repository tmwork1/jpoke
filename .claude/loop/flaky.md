...
マージする**（flaky test は他の CI すべてに影響するため、§共通6 の「ユーザーが任意のタイミングで
実行」は待たない）。
...
**flaky test は他の CI すべてに影響するため、他フローと異なり §共通6 の「ユーザーが任意のタイミングで
実行」は適用しない。1件の修正が review-test で成功・コミットされるたびに、ディスパッチャーが
その場で即座に main へ反映する**（`{branch}` = `loop/flaky`）:

```bash
# jpoke/ で:
git switch main
git merge --ff-only loop/flaky
```

main が独自に進んでいて FF 不可な場合や、main のワークツリーに未解決のマージ・コンフリクト等
既存の異常な状態がある場合は、自動解決せずユーザーに報告してこの手順を止める（§共通8 相当）。
この場合でも `loop/flaky` 側のコミット自体は失われないので、ループは通常どおり次の調査対象へ進めてよい。
...
エージェント呼び出しが API セッション制限で失敗した場合 → §共通12 に従う。
