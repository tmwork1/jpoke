...
```bash
git -C "{worktree}" push origin loop/flaky
gh pr create --base main --head loop/flaky --fill
gh pr merge loop/flaky --merge --delete-branch=false
```

`gh pr merge` が失敗した場合（コンフリクト・ブランチ保護等）は自動解決せず、PR を open のまま
ユーザーに報告してこの手順を止める（§共通8 相当）。この場合でも `loop/flaky` 側のコミット自体は
