...
3エージェントとも「指摘なし」（または全指摘が誤検知・重複）だったラウンドが**2回連続**した
...
開発者エージェント・初心者エージェント・AI開発者エージェントを **同一メッセージ内で並列起動**する
...
`{"id", "source": "developer"|"beginner"|"ai_developer", "summary", "detail"}` として積む。

**`pending_findings` に1件も積まれなかった場合**（3エージェントとも「指摘なし」、または
...
{finding.source == "developer" ? "開発者視点" : finding.source == "ai_developer" ? "AI開発者視点" : "初心者視点"}のレビューで挙がった指摘:
{finding.detail}
