...
- ローカルテスト（各フローの review-test 等）を通過した単位（単一ブランチ方式は1件の修正
  コミットごと、統合ブランチ方式は §共通5 のバッチ整形コミットごと。ただし `lethal` は例外で
  10件ごと）で、ディスパッチャーが **都度自動的に GitHub PR 経由で main へ反映する**（§共通6）。
  ローカルテストの通過が承認条件であり、人間のレビュー待ち・PR コメント対応は行わない。
...
タイミングは各フロー指示書の「main への反映」節で規定する（単一ブランチ方式は1件の修正コミット
ごと、統合ブランチ方式は §共通5 のバッチ整形コミットごと。`lethal` のみ例外で10件ごと）。
ローカルテスト（各フローの review-test 等で既に実行済み）を通過済みの内容であることが反映条件で
あり、ユーザーの都度承認や人間レビューは待たない（承認条件はローカルテストの通過そのもの）。
...
# 複数コミットをまとめて反映する場合（統合ブランチ方式のバッチ整形・lethal の10件ごと等）で
