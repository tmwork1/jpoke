...
9. `python scripts/generate_test_list.py` を実行する
10. `python -m pytest tests/ -v` を実行し、全件パスすることを確認する（今回と無関係な既存テストが
    flaky と判明した場合は §共通13 に従いその場で修正する）
11. 該当する `docs/progress/*.md` に変更があれば更新する
12. 変更を意味のある単位でコミットする（`{branch}` ブランチ上で行う。1コミットにまとめても、
