...
全フロー共通。§共通7 の方針と一貫させ、セッション制限からの自動待機・自動再開の仕組みは持たない。
制限を検知したら**その場で止まり、ユーザーに報告してターンを終える**。再開はユーザーが次に
`/loop {flow}` を実行したときに行う。

状態ファイルに以下の1フィールドを持つ（フロー側のスキーマに追加する。参考情報としての記録のみで、
待機時間の計算には使わない）:
- `rate_limit_reset_at`: 次にAPIセッション制限がリセットされる時刻（文字列。ユーザー申告、または
  直近のセッション制限エラーメッセージから読み取った値）。未確認なら `null`。

1. **ループ初回起動時にリセット時刻を確認する**: ユーザーが `/loop {flow}` でループを新規に起動した
   とき（`.loop/{flow}_state.json` が存在しない、または存在していても `rate_limit_reset_at` が
   未設定の場合）のみ、通常の手順に入る前にユーザーへ「次のAPIセッション制限のリセット時刻を
   教えてください」と確認し、回答を `rate_limit_reset_at` に記録してから手順を進める。既に記録済み
   なら再確認しない。
2. **セッション制限エラーを検知する**: foreground/background 問わず、エージェント（ディスパッチャー
   ...
   再試行しない）。エラーメッセージにリセット時刻が含まれていれば `rate_limit_reset_at` をその値で
   上書きする。
3. **完了済みの分は失わない**: 制限失敗は一部のエージェントのみに起きることがある（例: 4並列中1件
   だけ成功）。ターンを終える前に、既に成功・コミット済みの分は通常どおりマージ・ハーベストする。
4. **状態を保存してターンを終える**: 未処理の entry（失敗したエージェントに対応する分）は
   `in_progress` やキューから無理に取り除かず、実態に合わせて state を保存する。「◯件がセッション
   制限で失敗しました（参考リセット時刻: ХХ:ХХ）。ユーザーが `/loop {flow}` を再実行すると再開
   します」と報告してターンを終える。
5. **次回起動時の再開**: ユーザーが `/loop {flow}` を再度実行したときに再開する。前回失敗していた
