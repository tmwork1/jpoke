...
main への反映が完了した後、§共通16 の判断基準を適用する（`completed_bugs`/`failed_bugs` 台帳は
両モード共通の1リストなので、モードをまたいだ突き合わせもここで行う）。体系的対応が必要と
判断した場合は §共通16 の手順で `general-purpose` サブエージェントに横断リファクタを委譲し、
成功したら §共通16 の後処理（テスト全体実行・進捗更新・コミット・main反映）に従う。単発修正で
十分な場合は何もせず手順5（次のバグへ）に進む。
