...
- `pending_failures`: 直近のバッチ実行で見つかった未処理の失敗のキュー（`fuzz_log.md` の
  `pending_anomalies` と同じ形）。`scripts/fuzz_battle.py --search` は打ち切りをせず `batch_size` 件を
...
自動選択される。明示指定したい場合のみ `--workers N` を追加する）。exit code 0 = 全件成功、
exit code 1 = 失敗あり（stdout に `FAIL: seed=... signature=...` と `report: {絶対パス}` の組が
失敗した件数分、seed 昇順で出力される）。
...
（例: PYTHONPATH=src python scripts/fuzz_battle.py --seed {seed} --player {mode} --max-turns {modes[mode].max_turns} --n-pokemon {modes[mode].n_pokemon}[ --max-plies {modes[mode].max_plies}]）
