...
`scripts/fuzz/fuzz_log_battle.py` で完全ランダムな対戦を1バッチ `batch_size × workers` 件実行し、
成否を問わず全ターン分のイベントログをレポートファイルに書き出す。既存の `fuzz` ループ（未捕捉例外を検出）・
...
1バッチ分のログを **1グループ10件（固定）** に分割し、**Explore エージェント（読み取り専用）に
分担させる**（1レポート1エージェントの粒度までは分割しない。過度な並列化によるコスト増を避け
つつ、1エージェントあたりの読み込み量を一定に保つため、グループサイズは `workers`（並列度）とは
独立した固定値10とする）。`workers`（状態ファイルの設定値、既定1）はグループの**同時実行数**の
みを制御する: `workers` が2以上ならその件数までのグループを1つのメッセージ内で並列起動し、
グループ総数が `workers` を超える場合は先行ラウンドの結果が揃ってから次ラウンドを起動する。
`workers=1` なら1グループずつ順番に（前のグループの結果を受け取ってから次を起動）読ませる。
各エージェントには
**整合性の観点のみ**（推奨判定基準。可読性・情報の親切さといった主観的な観点は対象外）で
異常の有無を報告させる。判定にあたっては、ログに登場する特性・アイテム・技の効果を
`.internal/progress/` の効果概要（該当名でGrepして参照する。全文は読まない）で確認したうえで
...
`workers` 体分の Explore エージェントの報告をすべて合算した上で、**ディスパッチャー自身
（このループの実行主体）** が確認し、
...
ディスパッチャーは指摘をそのまま鵜呑みにせず、関連する `.internal/spec/` や実装コードを見て
...
- `next_seed` / `total_battles` / `batch_size`: 探索済みシード範囲の管理（`fuzz`/`replay_fuzz` と同じ意味）。
  1回のバッチで実際に実行する件数は `batch_size × workers`。
- `workers`: 1回のバッチ実行の並列度（既定1）。以下の2つの役割を1つの値で兼ねる:
  (a) `fuzz_log_battle.py` の `--workers` に渡され、対戦のマルチプロセス並列実行数を制御する。
  (b) `N` に設定すると1バッチあたりの対戦数が `batch_size × N` 件に増え、Explore エージェントに
  よるレビューの**同時起動数**が最大 `N` になる（詳細は手順2・手順3）。レビューの1グループ
  あたりの件数は `workers` の値に関わらず常に10件固定（後述）。
  ユーザーから「workersをNに」等の指示があった場合、この値を更新して保存する。値を上げるほど
  1バッチあたりの探索スループットが上がるが、CPU負荷（他の並行セッション・ループとの共有）と
  Explore エージェント呼び出しのトークンコストも比例して増えるため、状況を見て調整する。
- Explore レビューのグループサイズ（1エージェントあたりの担当件数）は常に **10件固定**
  （`workers` の値とは独立。`batch_size` を変更してもこのグループサイズには影響しない）。
...
- `verification_agent_id`: 手順4.1で使う裏取りサブエージェントのID（同一セッション内で
  使い回すため、起動できたら記録する）。セッションを跨ぐと無効になる可能性が高いフィールド
  （Agent呼び出しのIDは会話セッションに紐づき、新しいセッションでは復元できない）ため、
  参考情報として保存するのみで、実際に有効かどうかは手順4.1の指示に従い都度確認する。
...
初期値で新規作成する: `next_seed=0`, `total_battles=0`, `batch_size=20`, `workers=1`,
`max_turns=20`, `n_pokemon=3`, `report_dir="fuzz_log_reports"`, `pending_crashes=[]`,
`pending_anomalies=[]`, `current_failure=null`, `completed_bugs=[]`, `failed_bugs=[]`）。
既存の状態ファイルに `workers` フィールドが無い場合は `1`（従来どおり単一Exploreエージェント・
単一プロセス）として扱う。
...
cd "{worktree}" && PYTHONPATH=src python scripts/fuzz/fuzz_log_battle.py \
  --start-seed {next_seed} --count {batch_size * workers} --workers {workers} \
  --max-turns {max_turns} --n-pokemon {n_pokemon} --effect-bias 0.5
```

`--count` は `batch_size × workers`（`workers` は状態ファイルの設定値、既定1）。1回のスクリプト
実行にまとめることで、`--workers` によるプロセス並列化の恩恵をバッチ全体で受けられる
（`workers` 分だけスクリプトを複数回起動するのではない）。
...
`count` 件は worker プロセスに分散して並列実行される。`--workers {workers}` はこの値が Explore
エージェントの並列度・バッチ倍率も兼ねるため、状態ファイルの `workers` を上げると対戦の
プロセス並列実行数と探索スループットが連動して増える（省略時のスクリプト既定は CPU数と count の
小さい方だが、`fuzz_log` は他の並行セッション・ループ（review/impl 等）と共有する CPU 資源を
考慮しつつ明示的に指定する）。
...
worktree 配下の絶対パス）。`next_seed += batch_size * workers`、
`total_battles += batch_size * workers` を更新する。
...
手順3.5で impl に回す）。`crashed=False` の行だけを seed 昇順に並べ、先頭から **10件ずつ固定**
でグループに分割する（`workers` の値に関わらずグループサイズは常に10。最終グループはクラッシュの
分だけ10件未満になり得る）。このグループ分けを次の手順3で使う。状態を保存する。

### 3. `crashed=False` の分を Explore エージェント（10件/グループ、最大 `workers` 体・並列）に分担させて読ませる

手順2で作ったグループ（各グループ最大10件）ごとに、**1体の Explore エージェントに、そのグループの
対象ファイル一覧を並べて一括で**読ませる（1レポート1エージェントの粒度までは分割しない。過度な
並列化によるコスト・レビュー負荷の増大を避けるため）。グループ総数が `workers` 以下ならすべてを
1つのメッセージ内で同時起動する。グループ総数が `workers` を超える場合は、先頭から `workers` 件分を
1ラウンド目として同時起動し、その結果がすべて揃ってから次の `workers` 件分を2ラウンド目として
起動する、という形でラウンドを繰り返す（`workers=1` なら常に1グループずつ順番に起動）。プロンプトは
グループごとに同一のテンプレートを使い、`{seed_list}`・`{n}`・読むファイル一覧だけをグループの
内容に差し替える:
...
数値変化に関わっていそうなものについて `.internal/progress/{ability,item,move,ailment,volatile,
...
1ラウンドあたり最大 `workers` 体の Explore エージェント呼び出しを **1回のメッセージ内に複数の
Agent ツール呼び出しとしてまとめ、同時に起動する**（`run_in_background: false` を指定し、その
ラウンドの全エージェントの結果を同一ターン内で受け取ってから次のラウンドに進む。他のグループの
結果を待たずに先行して次の処理へ進むことはしない）。`workers=1` なら常に1回の Agent 呼び出し
のみを起動し、結果を受け取ってから次のグループに進む（＝グループ数だけ順次繰り返す）。

全グループの結果がすべて「異常なし」の場合、`pending_anomalies` は空のまま次に進む:
...
#### 4.1 裏取りサブエージェントによる検証

**ディスパッチャー（このループの実行主体）は進行管理と最終判断に徹し、仕様書・実装コードを
自ら読み込んで裏取りする作業そのものは行わない。** 手順3で「異常なし」以外の報告があった場合、
それぞれについて専用の裏取りサブエージェント（読み取り専用）に検証させる。

- **エージェントの起動・使い回し**: このループセッション中に裏取りサブエージェントをまだ
  起動していない場合、`Agent`（`subagent_type="Explore"`）で新規に1体起動し、`agentId` を
  `verification_agent_id` として状態ファイルに記録する。起動時のプロンプトには、これまでの
  fuzz_log 運用で判明した誤検知パターンの知識を必ず含める（最低限、以下を含める。今後
  新たな誤検知パターンが判明した場合は追記していく）:
  - タイプ相性表の0倍（無効）の正規8ペア（ノーマル→ゴースト、かくとう→ゴースト、
    ゴースト→ノーマル、でんき→じめん、どく→はがね、じめん→ひこう、ドラゴン→フェアリー、
    エスパー→あく）。非対称であること自体はバグの根拠にならない
  - HPクランプ（回復要求量が最大HPを超える場合、実際の適用量だけが記録される。ダメージ
    直後の回復が要求量どおりに見えない場合はまずクランプを疑う）
  - `Event.ON_TURN_END` は `stop_if_winner_determined=True` で発火するため、速度順処理中に
    勝敗が確定すると、まだ処理されていない他個体（自分・相手を問わない）の同ターン分の
    残り処理（毒/やけど/ほろびのうた等）がスキップされる
  - バインド系状態は「付与した側が場からいなくなる（交代・瀕死問わず）」で即座に解除され、
    解除ターンに追加ダメージは発生しない
  - 瀕死交代で新規設置された天候・フィールド等は、そのターンのON_TURN_END（カウントダウン）
    が既に完了した後に設置されるため同一ターンにはtickせず、次のターン終了時から効果が
    始まる（PR#266で確認済み）
  - 攻撃側の特性（ノーマルスキン等）による技タイプ変換は、変換後のタイプで無効・効果抜群を
    判定する。防御側の免疫特性（どんかん＝こんらん無効等）や、対象の実際のタイプ・浮遊
    状態・テラスタル状態も確認してから矛盾と判断する
  - 指摘を報告する際は、引用するログ行を実際のファイルから一字一句正確にコピーする
    （数値やログ文言を記憶・推測で書き換えて誤った指摘を行った過去の実例がある）
  - 以降のバッチでは、`SendMessage` で `verification_agent_id` 宛にExplore報告を送り、
    同じエージェントを継続利用する（知識を蓄積させ、都度の指示を簡潔にできる）。
- **検証の依頼**: Explore報告で「異常なし」以外だった指摘それぞれについて、seed・指摘内容・
  対象ログの絶対パスを裏取りサブエージェントに渡し、関連する `.internal/spec/` や実装コード
  （CLAUDE.md の実装時参照順を参照）を確認したうえで、「真のバグか誤検知か」を、根拠
  （該当する仕様書の行・実装コードの箇所・実際のログの該当行）とともに判定して返すよう
  依頼する。
- **ディスパッチャーによる最終判断**: 裏取りサブエージェントの結論をそのまま鵜呑みにせず、
  提示された根拠がログ・コードの記述と実際に整合しているかをディスパッチャー自身も
  要点だけ確認したうえで、採否を決定する（詳細な一次調査はサブエージェントに委ねるが、
  最終責任はディスパッチャーにある）。
...
main への反映が完了した後、§共通16 の判断基準を適用する（クラッシュ由来・整合性指摘由来を問わず、
`completed_bugs`/`failed_bugs` 台帳は共通の1リストなので合わせて突き合わせる）。体系的対応が
必要と判断した場合は §共通16 の手順で `general-purpose` サブエージェントに横断リファクタを委譲し、
成功したら §共通16 の後処理（テスト全体実行・進捗更新・コミット・main反映）に従う。単発修正で
十分な場合は何もせず3.5冒頭または手順4.2（未処理分の続き）に進む。
