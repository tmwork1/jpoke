...
```json
{
  "worktree": "C:\\Users\\tmtmp\\Documents\\pokemon\\jpoke-loop\\fuzz_log",
  "next_seed": 0,
  "total_battles": 0,
  "batch_size": 10,
  "max_turns": 30,
...
### 3. 10件（`crashed=False` の分）を Explore エージェントに並列で読ませる

`crashed=False` の各レポートについて、**1レポートにつき1 Explore エージェントを同一メッセージ内で
並列起動**する（`crashed=True` を除いた件数分。通常10件、クラッシュがあれば10件未満）:

```
jpoke fuzz_log 整合性チェック: seed={seed}

以下のバトルログ（対戦のイベントログ全文）を読み、内容の「整合性」だけを確認してほしい。
...
出力形式:
- 矛盾が見つかった場合: 該当のターン番号・ログ行を引用し、何がどう矛盾しているかを一言で説明する
  （複数あれば箇条書き）
- 矛盾が見つからなかった場合: 「異常なし」とだけ答える
```
