...
1バッチ分のログを `batch_size` 件ずつのグループに分割し、`workers`（状態ファイルの設定値、
既定1）体の **Explore エージェント（読み取り専用）に分担させ、1つのメッセージ内で並列起動する**
（1レポート1エージェントの粒度までは分割しない。過度な並列化によるコスト増を避けるため、
分割の粒度は常に `batch_size` 件単位で固定する。`workers=1` なら従来どおり1体に一括で
読ませる動作と等価）。各エージェントには
...
- `workers`: 1回のバッチ実行・Explore レビューの並列度（既定1）。以下の2つの役割を1つの値で兼ねる:
  (a) `fuzz_log_battle.py` の `--workers` に渡され、対戦のマルチプロセス並列実行数を制御する。
  (b) `N` に設定すると1バッチあたりの対戦数が `batch_size × N` 件に増え、`crashed=False` の
  レポートを `batch_size` 件ずつ `N` グループに分割して `N` 体の Explore エージェントを
  1つのメッセージ内で並列起動する（詳細は手順2・手順3）。
...
手順3.5で impl に回す）。`crashed=False` の行だけを seed 昇順に並べ、先頭から `batch_size` 件ずつ
`workers` グループに分割する（最終グループはクラッシュの分だけ `batch_size` 未満になり得る。
`workers=1` なら1グループのみ）。このグループ分けを次の手順3で使う。状態を保存する。

### 3. `crashed=False` の分を Explore エージェント（`workers` 体・並列）に分担させて読ませる

手順2で作った `workers` グループ（各グループ最大 `batch_size` 件）ごとに、**1体の Explore
エージェントに、そのグループの対象ファイル一覧を並べて一括で**読ませる（1レポート1エージェントの
粒度までは分割しない。過度な並列化によるコスト・レビュー負荷の増大を避けるため）。プロンプトは
...
`workers >= 2` の場合、`workers` 体の Explore エージェント呼び出しを **1回のメッセージ内に
複数の Agent ツール呼び出しとしてまとめ、同時に起動する**（`run_in_background: false` を指定し、
全エージェントの結果を同一ターン内で受け取ってから次に進む。他のグループの結果を待たずに
先行して次の処理へ進むことはしない）。`workers=1` なら従来どおり1回の Agent 呼び出しのみで良い。
