...
`scripts/fuzz/fuzz_log_battle.py` で完全ランダムな対戦を1バッチ `batch_size × parallel` 件実行し、
...
1バッチ分のログを `batch_size` 件ずつのグループに分割し、`parallel`（状態ファイルの設定値、
既定1）体の **Explore エージェント（読み取り専用）に分担させ、1つのメッセージ内で並列起動する**
（1レポート1エージェントの粒度までは分割しない。過度な並列化によるコスト増を避けるため、
分割の粒度は常に `batch_size` 件単位で固定する。`parallel=1` なら従来どおり1体に一括で
読ませる動作と等価）。各エージェントには
...
`parallel` 体分の Explore エージェントの報告をすべて合算した上で、**ディスパッチャー自身
（このループの実行主体）** が確認し、
...
- `next_seed` / `total_battles` / `batch_size`: 探索済みシード範囲の管理（`fuzz`/`replay_fuzz` と同じ意味）。
  1回のバッチで実際に実行する件数は `batch_size × parallel`。
- `parallel`: 1回のバッチ実行・Explore レビューの並列度（既定1）。`N` に設定すると、1バッチあたりの
  対戦数が `batch_size × N` 件に増え、`crashed=False` のレポートを `batch_size` 件ずつ `N` グループに
  分割して `N` 体の Explore エージェントを1つのメッセージ内で並列起動する（詳細は手順2・手順3）。
  ユーザーから「parallelをNに」等の指示があった場合、この値を更新して保存する。値を上げるほど
  1ターンあたりの探索スループットが上がるが、Explore エージェント呼び出しのトークンコストも
  比例して増える。
...
初期値で新規作成する: `next_seed=0`, `total_battles=0`, `batch_size=20`, `parallel=1`,
...
既存の状態ファイルに `parallel` フィールドが無い場合は `1`（従来どおり単一Exploreエージェント）
として扱う。
...
cd "{worktree}" && PYTHONPATH=src python scripts/fuzz/fuzz_log_battle.py \
  --start-seed {next_seed} --count {batch_size * parallel} --workers {workers} \
  --max-turns {max_turns} --n-pokemon {n_pokemon} --effect-bias 0.5
```

`--count` は `batch_size × parallel`（`parallel` は状態ファイルの設定値、既定1）。1回のスクリプト
実行にまとめることで、`--workers` によるプロセス並列化の恩恵をバッチ全体で受けられる
（`parallel` 分だけスクリプトを複数回起動するのではない）。
...
worktree 配下の絶対パス）。`next_seed += batch_size * parallel`、
`total_battles += batch_size * parallel` を更新する。
...
`parallel` グループに分割する（最終グループはクラッシュの分だけ `batch_size` 未満になり得る。
`parallel=1` なら1グループのみ）。このグループ分けを次の手順3で使う。状態を保存する。

### 3. `crashed=False` の分を Explore エージェント（`parallel` 体・並列）に分担させて読ませる

手順2で作った `parallel` グループ（各グループ最大 `batch_size` 件）ごとに、**1体の Explore
...
`parallel >= 2` の場合、`parallel` 体の Explore エージェント呼び出しを **1回のメッセージ内に
複数の Agent ツール呼び出しとしてまとめ、同時に起動する**（`run_in_background: false` を指定し、
全エージェントの結果を同一ターン内で受け取ってから次に進む。他のグループの結果を待たずに
先行して次の処理へ進むことはしない）。`parallel=1` なら従来どおり1回の Agent 呼び出しのみで良い。
