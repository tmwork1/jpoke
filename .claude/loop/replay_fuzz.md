...
main への反映が完了した後、§共通16 の判断基準を適用する。体系的対応が必要と判断した場合は
§共通16 の手順で `general-purpose` サブエージェントに横断リファクタを委譲し、成功したら §共通16
の後処理（テスト全体実行・進捗更新・コミット・main反映）に従う。単発修正で十分な場合は何もせず
手順5（次のバグへ）に進む。
