...
`04_janken_nash_fictitious_play.py` は「毎ターン同じ確率で技を選び続ける」固定の混合戦略の
...
（04と同じ既定値・同じ制約。詳細は04のモジュールdocstring「題材」を参照）。
...
def main() -> None:
    ...
    # * 04_janken_nash_fictitious_play.py の固定混合戦略の結果と比較し、「own（自分HP）が
