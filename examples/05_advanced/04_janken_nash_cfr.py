"""CFR風の自己対戦学習で「状態に応じて手を変える」読み合いAIを作る。

## ゲーム理論の用語ミニ解説（対戦知識はあるがゲーム理論は初めての人向け）

「混合戦略」「Nash均衡」「fictitious play」「exploitability」は
`03_janken_nash_fictitious_play.py` の冒頭で解説済み（未読ならまず
そちらを参照）。このファイルではさらに以下の用語が出てくる。

* **information set（状態）**: このサンプルでは「自分と相手のHP割合が
  それぞれどのバケットにいるか」の組。対人戦で「今このHP状況なら
  こう選ぶ」という局面ごとの判断基準を、数値化した「状態」として
  扱っているだけ、と捉えるとよい（詳細は次の節「状態の定義」参照）
* **regret（後悔）**: 「実際に選んだ手」と「もし別の手を選んでいたら」の
  勝率の差。対人戦の反省会で「あそこは別の技の方が勝ちやすかった」と
  振り返る感覚に相当する
* **regret matching**: 後悔が大きく蓄積している手ほど次に選ばれやすくする
  更新ルール。「反省が多い選択肢ほど今後は多めに試す」という素朴な
  発想を確率分布に変換する手法
* **CFR（Counterfactual Regret Minimization、反実仮想後悔最小化）**:
  状態（information set）ごとにregret matchingを行い、Nash均衡に
  近づけていく手法群の総称。本来は「もし別の手を選んでいたら」を
  ゲーム木上で厳密に計算するが、このサンプルでは後述の通り
  `battle.copy()` によるロールアウトで近似する

## 03との違い

`03_janken_nash_fictitious_play.py` は「毎ターン同じ確率で技を選び続ける」固定の混合戦略の
...
学習する。対戦するポケモン・技3つはファイル先頭の `MATCHUP` で指定する
（03と同じ既定値・同じ制約。詳細は03のモジュールdocstring「題材」を参照）。

## 状態（information set）の定義

状態は `(自分のHP割合バケット, 相手のHP割合バケット)` の組で表す。HP満タン
（fraction=1.0）は常に専用の最上位バケットとして切り出し、残りの (0, 1) 区間を
`HP_BUCKETS` 個に等分する（バケット総数は `HP_BUCKETS + 1` になる。既定の
`HP_BUCKETS=3` なら「満タン/多い/中/少ない」の4段階。詳しくは `hp_bucket()` 参照）。
ターン数は状態に含めない単純化をしている。この状態ごとに「MATCHUP.moves の3つの技を
それぞれ何%選ぶか」という戦略テーブルを学習する。
...
以下のように近似する（`.internal/plan/archives/janken_nash_equilibrium_methods.md`
の「案4」も参照）:
...
2. 実際に選んだ手を確定する前に、`battle.copy()` で盤面を複製し、
   ...
   決着まで進めた場合の勝率を評価する（`rollout_value()`）
3. その3通りの勝率と、実際に選んだ確率分布での期待勝率との差を後悔として
   蓄積し（regret matching、`update_regret()`）、次にこの状態を再訪した
   ときの戦略に反映する
...
（`MinimaxPlayer` が使っているのと同じ `Battle.copy()` の仕組みを流用している）。
...
def hp_bucket(mon: Pokemon, n_buckets: int = HP_BUCKETS) -> int:
    """HP割合を0〜n_bucketsの段階に離散化する（大きいほどHPが多い）。

    HP割合（fraction）は1から始まるため、fraction==1.0（HP満タン）は常に専用の
    最上位バケット（n_buckets）として切り出し、残りの (0, 1) 区間を n_buckets 個に
    等分する。そのためバケット総数は n_buckets+1 になる（例: n_buckets=2 なら
    「1（満タン）」「1〜0.5」「0.5〜0」の3段階）。
...
def sample_command(strategy: tuple[float, float, float], rng):
...
def update_regret(regrets: dict[tuple[int, int], list[float]],
                   info: tuple[int, int],
                   strategy: tuple[float, float, float],
                   values: list[float]) -> None:
    """`info`（状態）の累積後悔を、実際にサンプルされた戦略の期待勝率を基準に更新する。

    `values[i]` は「その状態でi番目の技を選んでいたら」の反実仮想価値。基準
    （baseline、実際の戦略分布での期待勝率）より高かった手だけ後悔が積み上がる。
    """
    baseline = sum(s * v for s, v in zip(strategy, values))
    for i in range(3):
        regrets[info][i] += values[i] - baseline
...
def play_training_episode(regrets: dict[tuple[int, int], list[float]],
    ...
    while not battle.finished and battle.turn <= max_turns:
        ...
        update_regret(regrets, info0, strategy0, values0)
        ...
        update_regret(regrets, info1, strategy1, values1)
...
def main() -> None:
    ...
    strategy_sums: dict[tuple[int, int], tuple[float, float, float]] = defaultdict(
        lambda: (0.0, 0.0, 0.0),
    )
    visit_counts: dict[tuple[int, int], int] = defaultdict(int)

    print(f"=== {MATCHUP.species_name} Janken CFR-style Self-play ({'/'.join(MATCHUP.moves)}) ===")
    ...
    # HPの多い順（own_bucket降順、同点はopp_bucket降順）に表示する
    for info in sorted(visit_counts, reverse=True):
        own_bucket, opp_bucket = info
        n = visit_counts[info]
        avg = tuple(s / n for s in strategy_sums[info])
        print(f"  own={own_bucket} opp={opp_bucket} (訪問{n:4d}回): {MATCHUP.strategy_text(avg)}")

    # 試してみよう:
    # * HP_BUCKETS を増やす、または状態にターン数を加えると、より細かい読み合いを
    #   学習できるが、その分エピソード数(N_EPISODES)を増やさないと学習が安定しない
    # * 03_janken_nash_fictitious_play.py の固定混合戦略の結果と比較し、「own（自分HP）が
