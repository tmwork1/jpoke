...
## 基本方針
- `Battle`、`EventManager`、`EventContext`、`Handler` を中心としたイベント駆動設計を維持する。
...
- `data/*.py` では `partial(...)` の用途を「定数束縛」（タイプ名、倍率、固定ターン数など）に留める。
- 固有効果のロジックは `handlers/*` に名前付き関数で実装し、`data/*.py` からはその関数を登録する。
...
- テスト項目を変更したら、`scripts/generate_test_list.py` を実行して `docs/test/` の一覧を更新する。
