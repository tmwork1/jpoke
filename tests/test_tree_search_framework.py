...
def test_max_plies2でネストしたsimでも全階層のseedが重複しない():
    """r7-7回帰: `_worst_case_over_opponent` が `battle.copy(reseed=True)` に
    変更されたことで、max_plies=2 でネストして `_worst_case_over_opponent` が
    再帰的に呼ばれる場合も、各階層の sim = battle.copy(reseed=True) は
    複製元 sim の `_reseed_count` を基準に派生シードを生成するため、
    トップレベルの兄弟ノードだけでなく、そこからさらに再帰した2手目の
    各分岐まで含めて全ての sim が一意な seed を持つこと（Battle.copy() の
    docstring 参照）。
    """
    class SeedTrackingPlayer(TreeSearchPlayer):
        def __init__(self, username: str, max_plies: int = 1):
            super().__init__(username=username, max_plies=max_plies)
...
def test_reseedTrueにより兄弟ノード間でsimの乱数系列が独立する():
    """r7-7回帰: `_worst_case_over_opponent` 内の `sim = battle.copy()` が
    ...
    """
    class RandomStateTrackingPlayer(TreeSearchPlayer):
        def __init__(self, username: str, max_plies: int = 1):
            super().__init__(username=username, max_plies=max_plies)
