...
def test_configure_simが各分岐でsim_step実行前に呼ばれる():
    ...
    player1 = TrackingPlayer(name="SearchPlayer")
    player1.team = [
...
def test_evaluate関数が例外を投げてもsearchingフラグは解除される():
    ...
    player1 = BrokenEvaluatePlayer(name="SearchPlayer")
...
def test_fallbackに独自関数を指定するとそれが使われる():
    ...
    class CustomFallbackPlayer(TreeSearchPlayer):
        def __init__(self, name: str, max_plies: int = 1):
    ...
    player1 = CustomFallbackPlayer(name="SearchPlayer", max_plies=2)
    player1.team = [
...
def test_max_nodesでノード数上限が機能する():
    ...
    """
    player1 = TreeSearchPlayer(name="SearchPlayer", max_plies=2, max_nodes=3)
    player1.team = [
...
def test_max_plies2で相手の未公開技が2手目の分岐にも現れない():
    ...
    player1 = InspectSimPlayer(name="SearchPlayer", max_plies=2)
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
...
def test_opponent_estimatorを指定すると推定情報から探索が継続される():
    ...
    player1 = EstimatorPlayer(name="SearchPlayer")
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
...
def test_相手の合法手が未公開の場合fallbackに委譲される():
    ...
    player1 = TrackingFallbackPlayer(name="SearchPlayer")
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
