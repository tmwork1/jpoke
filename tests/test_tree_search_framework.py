"""jpoke.players の木探索フレームワーク（TreeSearchPlayer/MinimaxPlayer）の単体テスト。
...
フレームワーク自体（max_nodes・fallback・estimate_opponent・configure_sim等）の
テストは、具体的なスコア計算アルゴリズムを持つ `MinimaxPlayer` を使って行う
（`TreeSearchPlayer` 単体は `_score_command` 未実装のためインスタンス化はできても
探索を実行できない）。
"""
import pytest
...
from jpoke.players import MinimaxPlayer, TreeSearchPlayer
...
def test_configure_simが各分岐でsim_step実行前に呼ばれる():
    ...
    """
    class TrackingPlayer(MinimaxPlayer):
...
def test_estimate_opponent_selectionの返り値が公開済み選出とマージされる():
    """TODO1回帰: estimate_opponent_selection が返した相手の推定選出インデックスは、
    フレームワーク（estimate_opponentの既定実装）側で公開済みの selected_indexes と
    マージされ（公開済み分は消えず）、相手の交代コマンドが探索の相手候補として
    拡張されること。
    """
    class SelectionEstimatorPlayer(MinimaxPlayer):
        def __init__(self, username: str):
            super().__init__(username=username)
            self.merged_selected_indexes: list[list[int]] = []

        def estimate_opponent_selection(self, battle: Battle) -> list[int] | None:
            return [1]  # ベンチの未公開個体(カメックス)を選出済みと推定する

        def estimate_opponent(self, battle: Battle) -> None:
            super().estimate_opponent(battle)
            opponent = battle.opponent(self)
            state = battle.player_states[opponent]
            self.merged_selected_indexes.append(list(state.selected_indexes))

    player1 = SelectionEstimatorPlayer(username="SearchPlayer")
    player1.team = [
    ...
    # カメックスは未公開のまま(交代先候補としては観測されない)

    battle = Battle(player1, player2, n_selected=2, seed=1)
    battle.test_option.accuracy = 100
    battle.start()
    battle.player_states[player2].team[0].moves[0].revealed = True  # 相手の技を公開

    battle.step()

    assert player1.merged_selected_indexes, "estimate_opponentが一度も呼ばれていない"
    merged = player1.merged_selected_indexes[0]
    assert 0 in merged, "公開済みの選出インデックスが消えている"
    assert 1 in merged, "推定した選出インデックスがマージされていない"
    # 自分の合法手3（たいあたり・たいあたりのテラスタル・交代）×
    # 相手候補3（公開技1＋そのテラスタル1＋推定交代1）＝9ノード。
    assert player1.nodes_expanded == 9


def test_estimate_opponent_teamのみのオーバーライドで推定が機能する():
    """TODO1回帰: estimate_opponent（umbrella）ではなく estimate_opponent_team のみを
    オーバーライドしても、フレームワークのテンプレートメソッドが委譲するため、
    相手の合法手が未公開で空の盤面から探索が継続されること。
    """
    class TeamOnlyEstimatorPlayer(MinimaxPlayer):
        def __init__(self, username: str):
            super().__init__(username=username)
    ...
    player1 = TeamOnlyEstimatorPlayer(username="SearchPlayer")
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
    ...
    assert player1.evaluate_calls, "estimate_opponent_teamのみの指定でもevaluateが呼ばれ探索が行われるはず"


def test_estimate_opponentが部分公開でも毎ターン呼ばれ相手候補が拡張される():
    """TODO2回帰: 相手の技が一部だけ公開されている盤面（探索の相手候補が非空）でも、
    estimate_opponent_team がオーバーライドされていれば毎回呼ばれ、未公開スロットの
    推定によって相手候補（探索の分岐）が拡張されること（実対戦の型推定に相当）。
    従来は相手候補が非空だと estimate_opponent は一切呼ばれなかった。
    """
    class TeamEstimatorPlayer(MinimaxPlayer):
        def __init__(self, username: str):
            super().__init__(username=username)
            self.estimate_calls: list[int] = []

        def estimate_opponent_team(self, battle: Battle) -> None:
            self.estimate_calls.append(1)
            opponent = battle.opponent(self)
            active = battle.get_active(opponent)
            # 公開済みの技（たいあたり）は上書きせず、未公開分のみ補う。
            if len(active.moves) < 2:
                active.moves = list(active.moves) + [Move("みずでっぽう")]

    player1 = TeamEstimatorPlayer(username="SearchPlayer")
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
    ...
    battle.test_option.accuracy = 100
    battle.start()
    battle.player_states[player2].team[0].moves[0].revealed = True  # たいあたりのみ公開

    battle.step()

    assert player1.estimate_calls, "相手候補が非空でもestimate_opponent_teamが呼ばれるはず"
    # 自分の合法手4（たいあたり・10まんボルトの通常/テラスタル各2）×
    # 相手候補4（公開1＋推定1の通常/テラスタル各2）＝16ノード。
    assert player1.nodes_expanded == 16

    # 比較用: 推定なしのプレイヤーでは相手候補が公開済みの1技のみになり展開が半分になる。
    baseline_player1 = MinimaxPlayer(username="SearchPlayer")
    baseline_player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
    baseline_player2 = Player(username="RandomPlayer")
    baseline_player2.team = [Pokemon("ゼニガメ", item_name="", move_names=["たいあたり", "みずでっぽう"])]
    baseline_battle = Battle(baseline_player1, baseline_player2, n_selected=1, seed=1)
    baseline_battle.test_option.accuracy = 100
    baseline_battle.start()
    baseline_battle.player_states[baseline_player2].team[0].moves[0].revealed = True

    baseline_battle.step()

    assert player1.nodes_expanded > baseline_player1.nodes_expanded
...
def test_estimate_opponentを指定すると推定情報から探索が継続される():
    ...
    """
    class EstimatorPlayer(MinimaxPlayer):
        def __init__(self, username: str):
            super().__init__(username=username)
            self.evaluate_calls: list[int] = []
...
def test_evaluate_commandsがmax_nodesを無視して全合法手を評価する():
    ...
    確認する。
    """
    player1 = MinimaxPlayer(username="SearchPlayer", max_nodes=1)
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
...
def test_evaluate関数が例外を投げてもsearchingフラグは解除される():
    ...
    """
    class BrokenEvaluatePlayer(MinimaxPlayer):
        def evaluate(self, battle: Battle) -> float:
...
def test_fallbackに独自関数を指定するとそれが使われる():
    ...
    使われること。"""
    class CustomFallbackPlayer(MinimaxPlayer):
...
def test_max_nodesでノード数上限が機能する():
    ...
    """
    player1 = MinimaxPlayer(username="SearchPlayer", max_plies=2, max_nodes=3)
    player1.team = [
...
def test_max_plies2でネストしたsimでも全階層のseedが重複しない():
    """r7-7回帰: `MinimaxPlayer._score_command`（旧 `_worst_case_over_opponent`）が
    `battle.copy(reseed=True)` に変更されたことで、max_plies=2 でネストして
    `_score_command` が再帰的に呼ばれる場合も、各階層の sim = battle.copy(reseed=True) は
    複製元 sim の `_reseed_count` を基準に派生シードを生成するため、
    トップレベルの兄弟ノードだけでなく、そこからさらに再帰した2手目の
    各分岐まで含めて全ての sim が一意な seed を持つこと（Battle.copy() の
    docstring 参照）。
    """
    class SeedTrackingPlayer(MinimaxPlayer):
        def __init__(self, username: str, max_plies: int = 1):
            super().__init__(username=username, max_plies=max_plies)
            self.observed_seeds: list[int] = []

        def configure_sim(self, sim: Battle) -> None:
            self.observed_seeds.append(sim.seed)

    player1 = SeedTrackingPlayer(username="SearchPlayer", max_plies=2)
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
    ...
    # トップレベル（1手目）の分岐に加え、そこから再帰した2手目の分岐も
    # configure_sim で観測されるため、単純な my_commands×opponent_commands の
    # 1階層分（本テストの盤面では4×4=16件）よりずっと多くのsimが記録される
    assert len(player1.observed_seeds) == player1.nodes_expanded
    assert len(player1.observed_seeds) > 16, (
        "2手目以降の再帰分岐がconfigure_simに現れていない（テスト設計を要確認）"
    )
    assert len(set(player1.observed_seeds)) == len(player1.observed_seeds), (
        "階層をまたいでsimのseedが重複している"
        "（親simのreseed_countを基準にした派生シード生成が効いていない）"
    )
...
def test_max_plies2で相手の未公開技が2手目の分岐にも現れない():
    ...
    sim を覗き見る）。
    """
    class InspectSimPlayer(MinimaxPlayer):
        def __init__(self, username: str, max_plies: int = 1):
            super().__init__(username=username, max_plies=max_plies)
    ...
    battle.test_option.accuracy = 100
    battle.start()
    battle.player_states[player2].team[0].moves[0].revealed = True  # みずでっぽうは未公開のまま
...
def test_reseedTrueにより兄弟ノード間でsimの乱数系列が独立する():
    """r7-7回帰: `MinimaxPlayer._score_command`（旧 `_worst_case_over_opponent`）内の
    `sim = battle.copy()` が
    `sim = battle.copy(reseed=True)` に変更される前は、同じ my_cmd に対する
    各 opp_cmd 分岐・各 my_cmd 分岐が複製元 battle の random/decision_random の
    状態をそのまま共有し、探索木の兄弟ノード間で乱数系列が相関していた
    （configure_sim で命中判定等の確率的要素を固定しない場合に評価値が歪む
    原因）。configure_sim で確率的要素を一切固定しない状態
    （battle.test_option.accuracy 未設定）で、各分岐の sim.random /
    sim.decision_random の内部状態（getstate()）がすべて異なることを確認する。
    """
    class RandomStateTrackingPlayer(MinimaxPlayer):
        def __init__(self, username: str, max_plies: int = 1):
            super().__init__(username=username, max_plies=max_plies)
            self.observed_states: list[tuple] = []

        def configure_sim(self, sim: Battle) -> None:
            self.observed_states.append(
                (sim.random.getstate(), sim.decision_random.getstate())
            )

    player1 = RandomStateTrackingPlayer(username="SearchPlayer")
    player1.team = [
        Pokemon("ヒトカゲ", item_name="", move_names=["たいあたり", "10まんボルト"]),
    ]
    ...
    assert len(player1.observed_states) == player1.nodes_expanded >= 4
    assert len(set(player1.observed_states)) == len(player1.observed_states), (
        "分岐間でsimの乱数系列（random/decision_random）の初期状態が重複している"
        "（reseed=Trueによる兄弟ノード独立化が効いていない）"
    )


def test_score_command未実装のTreeSearchPlayerはNotImplementedErrorになる():
    """TreeSearchPlayer はスコア計算アルゴリズムを持たない抽象度の高い探索
    フレームワーク基底のため、_score_command をオーバーライドしないまま
    choose_command() を呼ぶと NotImplementedError になること
    （MinimaxPlayer 等の具体的なサブクラスが必須であることの確認）。
    """
...
def test_相手の合法手が未公開の場合fallbackに委譲される():
    ...
    """
    class TrackingFallbackPlayer(MinimaxPlayer):
        def __init__(self, username: str):
            super().__init__(username=username)
            self.fallback_calls: list[int] = []
