...
def test_かいがらのすず_反動で瀕死になった場合は回復しない():
    ...
    ならない（.internal/spec/items/かいがらのすず.md「反動技や、ひんしになる攻撃技などの効果で攻撃側の
...
def test_くっつきバリ_致命打でも発動する():
    """くっつきバリ: 所持者がひんしになった場合も攻撃した相手にくっつく
    （fuzz_log横断監査。.internal/spec/items/くっつきバリ.md）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", item_name="くっつきバリ")],
        team1=[Pokemon("カビゴン", move_names=["たいあたり"])],
        accuracy=100,
    )
...
def test_くろいヘドロ_同ターンに瀕死になったポケモンは回復しない():
    """くろいヘドロ: 同ターン中の攻撃で先にHPが0になったどくタイプのポケモンは、
    ターン終了時のくろいヘドロ回復を受けずに瀕死のままとなる"""
    battle = t.start_battle(
        team0=[Pokemon("ゲンガー", item_name="くろいヘドロ"), Pokemon("コラッタ")],
...
def test_こだわり系_がんじょうで無効化された一撃必殺技でも技ロック():
    """こだわり系アイテム: ON_TRY_MOVE_2（がんじょうによる一撃必殺技無効化）で
    技が不発した場合も、こだわりロックがかかること。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", item_name="こだわりメガネ", move_names=["じわれ"])],
    ...
    assert mon.volatiles["こだわり"].move_name == "じわれ"


def test_こだわり系_くさタイプの粉技無効化でも技ロック():
    """こだわり系アイテム: くさタイプへの粉技無効化で技が不発した場合も、
    こだわりロックがかかること。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", item_name="こだわりメガネ", move_names=["キノコのほうし"])],
        team1=[Pokemon("フシギダネ")],
        accuracy=100,
    )
    t.run_move(battle, 0)
    mon = battle.actives[0]
    assert mon.has_volatile("こだわり")
    assert mon.volatiles["こだわり"].move_name == "キノコのほうし"


def test_こだわり系_タイプ相性で無効化された技でも技ロック():
    """こだわり系アイテム: PP消費後にタイプ相性で技が無効化された場合も、
    実機同様PP消費済みの不発として扱われこだわりロックがかかること。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", item_name="こだわりメガネ", move_names=["10まんボルト"])],
        team1=[Pokemon("ヌオー")],
        accuracy=100,
    )
    t.run_move(battle, 0)
    mon = battle.actives[0]
    assert mon.has_volatile("こだわり")
    assert mon.volatiles["こだわり"].move_name == "10まんボルト"
...
def test_ゴツゴツメット_致命打でも発動する():
    """ゴツゴツメット: 持つポケモンがひんしになったときも発動する
    （fuzz_log横断監査。.internal/spec/items/ゴツゴツメット.md）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", item_name="ゴツゴツメット")],
        team1=[Pokemon("カビゴン", move_names=["たいあたり"])],
        accuracy=100,
    )
    defender, attacker = battle.actives
