...
def test_たべのこし_同ターンに瀕死になったポケモンは回復しない():
    """たべのこし: 同ターン中の攻撃で先にHPが0になったポケモンは、
    ターン終了時のたべのこし回復を受けずに瀕死のままとなる"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", item_name="たべのこし"), Pokemon("コラッタ")],
...
def test_だっしゅつパック_すばやさが一番高いポケモンのみ発動する():
    ...
    assert state1.team[0].boosts["atk"] == -1
...
def test_だっしゅつパック_ミラーアーマーで跳ね返しても発動する():
    ...
    assert ejected_mon.item.revealed
    assert not ejected_mon.has_item()
    assert ejected_mon.boosts["atk"] == 0


def test_だっしゅつパック_ランクが下限に達しなくても発動する():
    """だっしゅつパック: +2から+1になるなど、ランクがマイナスにならなかった場合でも発動する

    急所に当たり大ダメージで相手が瀕死になると、防御側の能力ランク低下自体が
    適用されず（実機仕様、瀕死時は追加効果が発動しない）だっしゅつパックも
    発動しなくなり結果が不安定になるため、急所を固定回避する（flaky対策）。
    """
    battle = t.start_battle(
...
def test_だっしゅつパック_わるいてぐせに奪われた場合は発動しない():
    ...
    # 急所が発生しない乱数固定（急所だとピカチュウが1発で瀕死になり、
    # わるいてぐせが発動条件（被弾側が生存）を満たせずflakyになるため）
...
def test_だっしゅつボタン_ちからずくの効果が発動した技を受けたときは発動しない():
    ...
    # 急所が発生しない乱数固定（急所だとピカチュウが1発で瀕死になり、
    # 瀕死による強制交代でactive_indexが変わりflakyになるため）
    battle.random.random = lambda: 0.9
    battle.step()
    state1 = battle._player_states[1]
...
def test_とくせいガード_いえきによる特性無効化をブロック():
    """とくせいガード: 所持者に対するいえきは失敗し「とくせいなし」状態にならない"""
    battle = t.start_battle(
...
def test_とつげきチョッキ_アンコールで変化技を強制されたターンは使用できる():
    ...
    assert mon.last_move.name == "なきごえ"
...
def test_どくどくだま_ふしょく持ちはどくタイプとはがねタイプにも発動する():
    """どくどくだま: 持ち主がふしょくの場合、自身がどくタイプ・はがねタイプでも
    ターン終了時にもうどくが付与される（ふしょくは自身が特性所持者としてどくを
    付与する経路にも適用される）"""
    battle = t.start_battle(
        team0=[
            Pokemon("フシギダネ", ability_name="ふしょく", item_name="どくどくだま"),  # くさ/どくタイプ
            Pokemon("コイル", ability_name="ふしょく", item_name="どくどくだま"),  # でんき/はがねタイプ
        ],
        team1=[Pokemon("ピカチュウ")],
    )
    mon = battle.actives[0]
