...
def test_じゃくてんほけん_マジシャンより先に発動して奪われない():
    ...
    # 急所が発生しない乱数固定（急所だとフシギダネが1発で瀕死になり、
    # じゃくてんほけんの発動条件（被弾側が生存）を満たせずflakyになるため）
...
def test_じゃくてんほけん_瀕死になったときは発動しない():
    """じゃくてんほけん: 効果抜群の攻撃でHPが0(ひんし)になったときは発動せず、アイテムも消費されない"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["でんきショック"])],
        team1=[Pokemon("ゼニガメ", item_name="じゃくてんほけん")],
        accuracy=100,
...
def test_じゅうでんち_瀕死になったときは発動しない():
    """じゅうでんち: でんき技でHPが0(ひんし)になったときは発動せず、アイテムも消費されない"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["でんきショック"])],
        team1=[Pokemon("カビゴン", item_name="じゅうでんち")],
        accuracy=100,
