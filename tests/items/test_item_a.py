...
def test_アッキのみ_あまのじゃくによる下降はしろいきりで防げない():
    ...
    foe = battle.actives[1]
    t.run_move(battle, 0)
    assert foe.boosts["def"] == -1
    assert not foe.has_item()


def test_アッキのみ_きんちょうかんの相手がいると発動しない():
    """アッキのみ: 相手が特性きんちょうかんを持つときは発動しない"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="きんちょうかん", move_names=["たいあたり"])],
...
def test_イバンのみ_瀕死になったときはフラグが立たない():
    """イバンのみ: ダメージでHPが0(ひんし)になったときはフラグが立たず消費もされない"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", item_name="イバンのみ")],
        team1=[Pokemon("カビゴン")],
    )
...
def test_オッカのみ_きんちょうかんの相手がいると発動しない():
    """オッカのみ: 相手が特性きんちょうかんを持つときは発動しない"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", ability_name="きんちょうかん", move_names=["だいもんじ"])],
