...
def test_アンコール_3ターン後に解除():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"アンコール": 3})
    mon = battle.actives[0]
    for _ in range(3):
        battle.events.emit(Event.ON_TURN_END_3)
...
def test_かなしばり_4ターン後に解除():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"かなしばり": 4})
    mon = battle.actives[0]
...
def test_きゅうしょアップ_他補正と合算():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"きゅうしょアップ": 2})
    attacker, defender = battle.actives
    rank = battle.events.emit(
        Event.ON_CALC_CRITICAL_RANK,
        BattleContext(attacker=attacker, defender=defender),
...
def test_こんらん_カウント満了で解除():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"こんらん": 1})
    battle.test_option.trigger_volatile = False
    assert t.check_event_result(battle, Event.ON_CHECK_ACTION)
...
def test_じごくづき_2ターン後に解除():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"じごくづき": 2})
    mon = battle.actives[0]
    for _ in range(2):
        battle.events.emit(Event.ON_TURN_END_3)
...
def test_ちょうはつ_3ターン後に解除():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"ちょうはつ": 3})
    mon = battle.actives[0]
    for _ in range(3):
        battle.events.emit(Event.ON_TURN_END_3)
...
def test_でんじふゆう_5ターン継続():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"でんじふゆう": 5})
    mon = battle.actives[0]
    for _ in range(4):
        battle.events.emit(Event.ON_TURN_END_3)
...
def test_のろい_ダメージ():
    """のろい: ターン終了時ダメージ"""
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"のろい": 1})
    mon = battle.actives[0]
    battle.events.emit(Event.ON_TURN_END_3)
...
def test_ひるみ():
    """ひるみ: 行動不能（1ターン）"""
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"ひるみ": 1})
    attacker, defender = battle.actives
    battle.events.emit(
...
def test_ほろびのうた():
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"ほろびのうた": 1})
    mon = battle.actives[0]
    battle.events.emit(Event.ON_TURN_END_3)
    assert mon.hp == 0
    assert t.log_contains(battle, LogCode.HP_CHANGED)
...
def test_ほろびのうた_カウント進行():
    ...
    battle = t.start_battle(foe=[Pokemon("ピカチュウ")], ally=[Pokemon("ピカチュウ")], ally_volatile={"ほろびのうた": 3})
    mon = battle.actives[0]
    battle.events.emit(Event.ON_TURN_END_3)
