...
def test_どろぼう_ねんちゃく持ちから奪えない():
    """どろぼう: defenderがねんちゃく持ちのとき奪取が失敗する。"""
...
def test_どろぼう_攻撃者がアイテムを得て防御者がアイテムを失う():
    ...
    # 奪われた道具は場に存在したまま相手に渡っているため、
    # リサイクル・しゅうかく・ものひろいの復元/拾得対象にならない
    assert defender.last_lost_item_name == ""
...
def test_ハイドロカノン_まもるで防がれた場合はリチャージ状態にならない():
    """ハイドロカノン: まもるで防がれた場合はリチャージ揮発状態が付与されない。"""
    battle = t.start_battle(
...
def test_ハイドロカノン_交代するとリチャージ状態が解除される():
    """ハイドロカノン: リチャージ状態中に交代するとリチャージ揮発状態が解除される。"""
    battle = t.start_battle(
        team0=[Pokemon("カメックス", move_names=["ハイドロカノン"]), Pokemon("ピカチュウ")],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_ハイドロカノン_外れた場合はリチャージ状態にならない():
    """ハイドロカノン: 外れた場合はリチャージ揮発状態が付与されない。"""
    battle = t.start_battle(
        team0=[Pokemon("カメックス", move_names=["ハイドロカノン"])],
        team1=[Pokemon("カビゴン")],
        accuracy=0,
...
def test_ハイドロカノン_相手をひんしにしてもリチャージ状態が付与される():
    """ハイドロカノン: 相手をひんしにした場合でもリチャージ揮発状態が付与される。"""
    battle = t.start_battle(
        team0=[Pokemon("カメックス", move_names=["ハイドロカノン"])],
        team1=[Pokemon("ピッピ")],
        accuracy=100,
...
def test_ハイドロスチーム_こおり状態で使うと解凍されて攻撃できる():
    """ハイドロスチーム: こおり状態でも使用でき、使うと解凍される。"""
    battle = t.start_battle(
...
def test_ハイドロスチーム_こおり状態の相手に当てると解凍する():
    """ハイドロスチーム: みずタイプだが、被弾した相手のこおりを解凍する。"""
    battle = t.start_battle(
        team0=[Pokemon("カメックス", move_names=["ハイドロスチーム"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_ハイドロスチーム_両者ばんのうがさのとき晴れでも補正なし():
    """ハイドロスチーム: 攻撃側・防御側の両方がばんのうがさを持つ場合、晴れ中でも補正は等倍（4096）。

    攻撃側のばんのうがさによりハイドロスチーム固有の1.5倍化は発動せず、
    防御側のばんのうがさにより通常のみず技の0.5倍弱体化も発動しないため、結果1.0倍になる。
    """
    battle = t.start_battle(
...
def test_ハイパーボイス_相手にダメージを与える():
    """ハイパーボイス: 追加効果なしの特殊ノーマル技で相手にダメージを与える。"""
    battle = t.start_battle(
        team0=[Pokemon("ニンフィア", move_names=["ハイパーボイス"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_はいよるいちげき_secondary_effectフラグを持つ():
    """はいよるいちげき: ちからずくとの相互作用のためsecondary_effectフラグを持つこと。"""
    move_data = MOVES["はいよるいちげき"]
    assert "secondary_effect" in move_data.flags


def test_はいよるいちげき_とくこう1段階低下が発動する():
    """はいよるいちげき: 100%の確率で相手のとくこうを1段階下げる。"""
    battle = t.start_battle(
        team0=[Pokemon("メガヤンマ", move_names=["はいよるいちげき"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    t.run_move(battle, 0)
...
def test_はかいこうせん_相手をひんしにしてもリチャージ状態が付与される():
    """はかいこうせん: 相手をひんしにした場合でもリチャージ揮発状態が付与される。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["はかいこうせん"])],
        team1=[Pokemon("ピッピ")],
        accuracy=100,
...
def test_はきだす_おやこあいで2ヒットしてもランク低下は1回のみ():
    ...
    attacker.boosts["def"] = 2
    attacker.boosts["spd"] = 2
    t.run_move(battle, 0)
...
def test_はきだす_ゴーストタイプに無効でもランクが低下する():
    """はきだす: 効果がないゴーストタイプに使用した場合でも、
    たくわえるで上がった分のぼうぎょ・とくぼうランクは低下する。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["はきだす"])],
        team1=[Pokemon("ムウマ")],
        volatile0={"たくわえる": 2},
...
def test_はきだす_外れてもランクが低下する():
    """はきだす: 技を外した場合でも、たくわえるで上がった分のランクは低下する。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["はきだす"])],
        team1=[Pokemon("カビゴン")],
        volatile0={"たくわえる": 2},
...
def test_ハサミギロチン_ゴーストタイプに無効():
    """ハサミギロチン: ノーマルタイプ技のため、タイプ相性によりゴーストタイプの相手には無効。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["ハサミギロチン"])],
        team1=[Pokemon("ゲンガー")],
        accuracy=100,
    )
    defender = battle.actives[1]
...
def test_ハサミギロチン_タイプ分類命中率PPが仕様通り():
    """ハサミギロチン: ノーマルタイプの物理接触の一撃必殺技で、威力なし・命中30・PP8を持つ。"""
...
def test_ハサミギロチン_命中時は相手を一撃で倒す():
    """ハサミギロチン: 一撃必殺技であり、命中すると相手のHPを問わず即座にひんしにする。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["ハサミギロチン"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_はたきおとす_相手のアイテムがないとき威力補正なし():
    ...
    # 急所判定のブレで威力差（1.3倍）が逆転しないよう、両バトルとも急所なしに固定する
    t.fix_random(battle_no_item, 0.9)
    t.fix_random(battle_with_item, 0.9)
...
def test_はっぱカッター_急所ランクが1():
    """はっぱカッター: 急所ランク+1のため乱数0で急所が発生する。"""
    battle = t.start_battle(
        team0=[Pokemon("フシギバナ", move_names=["はっぱカッター"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
...
def test_はっぱカッター_急所ランクが1_乱数大で急所なし():
    """はっぱカッター: 乱数が急所閾値以上のとき急所にならない。"""
    battle = t.start_battle(
        team0=[Pokemon("フシギバナ", move_names=["はっぱカッター"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_はやてがえし_はやてのつばさが発動しない場合は失敗する():
    """はやてがえし: 相手がはやてのつばさを持っていてもHPが満タンでなければ優先度が上がらず失敗する。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["はやてがえし"])],
        team1=[Pokemon("ピジョット", ability_name="はやてのつばさ", move_names=["つつく"])],
        accuracy=100,
    )
    defender = battle.actives[1]
    battle.modify_hp(defender, v=-1)
    before_foe_hp = defender.hp
...
def test_はやてがえし_はやてのつばさによる優先度上昇技にも成功する():
    """はやてがえし: 特性はやてのつばさによりひこう技の優先度が+1になった場合でも成功する。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["はやてがえし"])],
        team1=[Pokemon("ピジョット", ability_name="はやてのつばさ", move_names=["つつく"])],
        accuracy=100,
...
def test_ハードプラント_まもるで防がれた場合はリチャージ状態にならない():
    """ハードプラント: まもるで防がれた場合はリチャージ揮発状態が付与されない。"""
    battle = t.start_battle(
...
def test_ハードプラント_交代するとリチャージ状態が解除される():
    """ハードプラント: リチャージ状態中に交代するとリチャージ揮発状態が解除される。"""
    battle = t.start_battle(
        team0=[Pokemon("フシギバナ", move_names=["ハードプラント"]), Pokemon("ピカチュウ")],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_ハードプラント_外れた場合はリチャージ状態にならない():
    """ハードプラント: 外れた場合はリチャージ揮発状態が付与されない。"""
    battle = t.start_battle(
        team0=[Pokemon("フシギバナ", move_names=["ハードプラント"])],
        team1=[Pokemon("カビゴン")],
        accuracy=0,
...
def test_ハードプラント_相手をひんしにしてもリチャージ状態が付与される():
    """ハードプラント: 相手をひんしにした場合でもリチャージ揮発状態が付与される。"""
    battle = t.start_battle(
        team0=[Pokemon("フシギバナ", move_names=["ハードプラント"])],
        team1=[Pokemon("ピッピ")],
        accuracy=100,
...
def test_ばくおんぱ_相手にダメージを与える():
    """ばくおんぱ: 追加効果なしの特殊ノーマル技で相手にダメージを与える。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["ばくおんぱ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_ひけんちえなみ_ちからずくで威力上昇しまきびしは設置されない():
...
def test_ひけんちえなみ_まきびし3層のとき設置されず攻撃は成功する():
...
def test_ひけんちえなみ_命中後まきびしが1層設置される():
...
def test_ひっかく_追加効果が発生しない():
    ...
    assert defender.boosts == {stat: 0 for stat in defender.boosts}
...
def test_ブラストバーン_まもるで防がれた場合はリチャージ状態にならない():
    """ブラストバーン: まもるで防がれた場合はリチャージ揮発状態が付与されない。"""
    battle = t.start_battle(
...
def test_ブラストバーン_交代するとリチャージ状態が解除される():
    """ブラストバーン: リチャージ状態中に交代するとリチャージ揮発状態が解除される。"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", move_names=["ブラストバーン"]), Pokemon("ピカチュウ")],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_ブラストバーン_外れた場合はリチャージ状態にならない():
    """ブラストバーン: 外れた場合はリチャージ揮発状態が付与されない。"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", move_names=["ブラストバーン"])],
        team1=[Pokemon("カビゴン")],
        accuracy=0,
...
def test_ブラストバーン_相手をひんしにしてもリチャージ状態が付与される():
    """ブラストバーン: 相手をひんしにした場合でもリチャージ揮発状態が付与される。"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", move_names=["ブラストバーン"])],
        team1=[Pokemon("ピッピ")],
        accuracy=100,
...
def test_ブラッドムーン_2ターン後は再び選択可能():
    ...
    move_names = [attacker.moves[cmd.index].name for cmd in commands if cmd.is_move]
...
def test_ブレイククロー_ぼうぎょ1段階低下が発動しない():
    ...
    t.run_move(battle, 0)
    assert battle.actives[1].boosts["def"] == 0
...
def test_ほのおのキバ_やけどが発動する():
    """ほのおのキバ: 10%でやけどを付与する（ひるみとは独立判定）。"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", move_names=["ほのおのキバ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_ほのおのキバ_やけどとひるみが同時に発動する():
    """ほのおのキバ: 2つの追加効果はそれぞれ独立に判定されるため、同時に発動しうる。"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", move_names=["ほのおのキバ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
