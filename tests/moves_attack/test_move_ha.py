...
def test_はやてがえし_ひるみが発動する():
    ...
    ひるみは付与された同ターンに消費されるため、advance_turn 後は既に解除されている。
...
def test_ボディプレス_防御ランク上昇でダメージ増加する():
    ...
    battle1.modify_stats(attacker1, {"B": 2}, source=attacker1)
...
def test_ボディプレス_防御実数値を攻撃として計算する():
    ...
    battle.random.random = lambda: 0.9
    t.run_move(battle, 0)
    assert battle.damage_calculator.final_attack == attacker.stats["B"]
