...
def test_だいばくはつ_タイプ無効で不発でも攻撃者がひんしになる():
    ...
    必ずひんしになる（.internal/spec/moves/じばく.md「威力を除けば`だいばくはつ`と全く同じ
    効果を持つ」を参照）。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["だいばくはつ"])],
...
def test_ダメおし_相手が受けたさめはだの反撃ダメージも同ターンのダメージとして扱われる():
    """ダメおし: 対象が同ターン中にさめはだ所持者への接触で反撃ダメージを受けていれば威力2倍になる。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["たいあたり"])],
        team1=[Pokemon("カビゴン", ability_name="さめはだ", move_names=["ダメおし"])],
        accuracy=100,
    )
    # team0がteam1（さめはだ持ち）に接触技を当て、反撃ダメージを受ける
    t.run_move(battle, 0)
    battle.random.random = lambda: 0.9
...
def test_でんこうそうげき_でんきタイプでないポケモンが使うと技が失敗する():
    ...
    でんきタイプの判定は Event.ON_TRY_ACTION（へんげんじざい等のタイプ変化より前）
    で行われるため、失敗は action_success に反映される。
    """
    battle = t.start_battle(
...
def test_でんじほう_タイプと分類と威力と命中率とppが正しい():
    """でんじほう: でんきタイプ・特殊・威力120・命中50・PP8であることを確認する。"""
...
def test_とびひざげり_あなをほるで姿を隠している相手に外れても失敗反動ダメージを受ける():
    """とびひざげり: あなをほるで姿を隠している相手に外れても同様に反動ダメージを受ける。"""
    battle = t.start_battle(
...
def test_とびひざげり_タイプ相性で無効化されても反動ダメージは受けない():
    """とびひざげり: ゴーストタイプによる無効化時の反動欠落は既知の実装制約であり、
    姿を隠している相手への対応（本修正）の影響を受けないことを確認する回帰テスト
    （.internal/spec/moves/とびひざげり.md「実装上の制約」）。
    """
    battle = t.start_battle(
        team0=[Pokemon("カイリキー", move_names=["とびひざげり"])],
        team1=[Pokemon("ゲンガー")],
...
def test_とびひざげり_まもるでブロックされても反動ダメージは受けない():
    """とびひざげり: まもるにブロックされた場合の反動欠落は既知の実装制約であり、
    姿を隠している相手への対応（本修正）の影響を受けないことを確認する回帰テスト
    （.internal/spec/moves/とびひざげり.md「実装上の制約」）。
    """
    battle = t.start_battle(
...
def test_とびひざげり_姿を隠している相手にいしあたまでも失敗反動ダメージを受ける():
    """とびひざげり: そらをとぶで姿を隠している相手に外れても、いしあたまでは防げない。"""
    battle = t.start_battle(
        team0=[Pokemon("ゴンベ", ability_name="いしあたま", move_names=["とびひざげり"])],
        team1=[Pokemon("ピジョット")],
        accuracy=100,
...
def test_とびひざげり_姿を隠している相手にマジックガードでは失敗反動ダメージを受けない():
    """とびひざげり: そらをとぶで姿を隠している相手に外れても、マジックガードでは反動を防げる。"""
    battle = t.start_battle(
        team0=[Pokemon("ラッキー", ability_name="マジックガード", move_names=["とびひざげり"])],
        team1=[Pokemon("ピジョット")],
        accuracy=100,
...
def test_とびひざげり_姿を隠している相手に外れても失敗反動ダメージを受ける():
    """とびひざげり: そらをとぶ等で姿を隠している相手に対応していない技で外れた場合も、
    通常の外れと同様に自分の最大HPの1/2の反動ダメージを受ける
    （.internal/spec/moves/とびひざげり.md「姿を隠している相手（そらをとぶ等）に使用して
    外れた場合も、通常の『外れ』と同様に反動ダメージを受ける」）。
    """
    battle = t.start_battle(
        team0=[Pokemon("カイリキー", move_names=["とびひざげり"])],
        team1=[Pokemon("ピジョット")],
...
def test_ともえなげ_PPは12():
    """ともえなげ: チャンピオンズでのPPは12（.internal/champions/move_list.txt準拠）。"""
    assert MOVES["ともえなげ"].pp == 12
...
def test_トライアタック_PPは12():
    """トライアタック: チャンピオンズでのPPは12（.internal/champions/move_list.txt準拠。Gen9本家は10）。"""
...
def test_どくばり_キングシールドで防がれると使用者のこうげきが1段階下がる():
    ...
    発動していなかった（.internal/spec/moves/キングシールド.md）。
    """
    battle = t.start_battle(
...
def test_ドラゴンテール_PPは12():
    """ドラゴンテール: チャンピオンズでのPPは12（.internal/champions/move_list.txt準拠）。"""
    assert MOVES["ドラゴンテール"].pp == 12
...
def test_ドラゴンテール_ダメージを与える():
    """ドラゴンテール: ドラゴンタイプの物理技で相手にダメージを与える。"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", move_names=["ドラゴンテール"])],
        team1=[Pokemon("カビゴン"), Pokemon("ヤドン")],
        accuracy=100,
...
def test_ドラゴンテール_ばんけん持ちの相手には強制交代が無効():
    ...
    ダメージだけ受けて特性バーは表れない（きゅうばんと異なりメッセージも流れない）。"""
    battle = t.start_battle(
        team0=[Pokemon("リザードン", move_names=["ドラゴンテール"])],
    ...
    t.run_move(battle, 0)
    assert battle.actives[1] is defender_before
    assert defender_before.hp < defender_before.max_hp
    assert not defender_before.ability.revealed
