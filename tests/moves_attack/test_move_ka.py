...
def test_かかとおとし_こんらんが発動する():
    ...
    追加効果は相手を瀕死にした場合は発動しないため、こんらん付与を確認できるよう
    ダメージを固定して相手を生存させる。
    """
    battle = t.start_battle(
...
def test_かかとおとし_対象がすでに全員ひんしのときは失敗反動ダメージを受けない():
    """かかとおとし: 相手の場に対象がすでに全員ひんしで技自体が不発になった場合は
    ON_MISSが発火しないため失敗反動を受けない。
    ...
    assert attacker.hp == hp_before  # 対象不在の不発では反動を受けない
...
def test_がんせきアックス_ちからずくで威力上昇しステルスロックは設置されない():
    """がんせきアックス: ちからずく使用時は威力が1.3倍になる代わりにステルスロックが設置されない。"""
    battle = t.start_battle(
        team0=[Pokemon("イシヘンジン", ability_name="ちからずく", move_names=["がんせきアックス"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    ...
    assert not side.fields["ステルスロック"].is_active
...
def test_くちばしキャノン_使用者がひんしになってもやけど効果は発動する():
    """くちばしキャノン: 加熱中に接触技を受けてひんしになった場合でも、
    やけど効果はダメージ適用直後（Event.ON_DAMAGE_HIT）に判定されるため、
    使用者がその後行動できなくても攻撃者はやけど状態になる。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ", move_names=["くちばしキャノン"])],
...
def test_くちばしキャノン_接触技を受けると攻撃者がやけどになる():
    ...
    加熱中（Event.ON_DAMAGE_HIT）にやけどが付与される。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ", move_names=["くちばしキャノン"])],
...
def test_くちばしキャノン_非接触技を受けてもやけどにならない():
    ...
    相手が非接触技（10まんボルト）を使う場合は加熱中でもやけどが付与されない。
    """
    battle = t.start_battle(
...
def test_くらいつく_ちからずくでも威力上昇せず効果は発動する():
    ...
    t.fix_random(battle, 0.999)  # 急所で相手がひんしになると使用者側は付与されないため固定
    t.run_move(battle, 0)
    assert battle.damage_calculator.power_modifier == 4096
