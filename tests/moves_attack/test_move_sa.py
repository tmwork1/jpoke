...
def test_サイコショック_相手のとくぼうランク変化の影響を受けない():
    """サイコショック: 相手の『とくぼう』ランクが上がっていてもダメージが変わらない。"""
...
def test_サイコショック_相手のぼうぎょランク変化の影響を受ける():
    """サイコショック: 相手の『ぼうぎょ』ランクが上がっているとダメージが減る。"""
    battle1 = t.start_battle(
        team0=[Pokemon("フーディン", move_names=["サイコショック"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_サイコファング_オーロラベールを解除する():
    """サイコファング: 命中時に相手側のオーロラベールを解除する。"""
    battle = t.start_battle(
...
def test_サイコファング_ひかりのかべを解除する():
    """サイコファング: 命中時に相手側のひかりのかべを解除する。"""
    battle = t.start_battle(
        team0=[Pokemon("フーディン", move_names=["サイコファング"])],
        team1=[Pokemon("カビゴン")],
        side1={"ひかりのかべ": 1},
...
def test_サイコファング_リフレクターがあってもダメージは軽減されない():
    """サイコファング: 壁はダメージ計算前に解除されるため、このターンのダメージ自体は軽減されない。"""
    battle = t.start_battle(
        team0=[Pokemon("フーディン", move_names=["サイコファング"])],
        team1=[Pokemon("カビゴン")],
        side1={"リフレクター": 1},
...
def test_サイコファング_命中が外れたとき壁を解除しない():
    """サイコファング: 命中判定に外れた場合はリフレクターを解除しない。"""
    battle = t.start_battle(
        team0=[Pokemon("フーディン", move_names=["サイコファング"])],
        team1=[Pokemon("カビゴン")],
        side1={"リフレクター": 1},
...
def test_サイコファング_壁3種を同時に解除する():
    """サイコファング: 命中時にリフレクター・ひかりのかべ・オーロラベールを同時に解除する。"""
    battle = t.start_battle(
        team0=[Pokemon("フーディン", move_names=["サイコファング"])],
        team1=[Pokemon("カビゴン")],
...
def test_サイコファング_壁がなくても動作する():
    """サイコファング: 相手側に壁が張られていない場合もエラーなく動作する。"""
    battle = t.start_battle(
        team0=[Pokemon("フーディン", move_names=["サイコファング"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_サンダーダイブ_recoilフラグを持つ():
    """サンダーダイブ: flagsに"recoil"が含まれること（失敗反動技もすてみの対象であるため）。"""
    move_data = MOVES["サンダーダイブ"]
    assert "recoil" in move_data.flags


def test_サンダーダイブ_いしあたまでも失敗反動ダメージを受ける():
    """サンダーダイブ: reason=fixed_recoilのためいしあたまでも防げない。"""
    battle = t.start_battle(
        team0=[Pokemon("ゴンベ", ability_name="いしあたま", move_names=["サンダーダイブ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=0,
    )
    attacker = battle.actives[0]
...
def test_サンダーダイブ_マジックガードでは失敗反動ダメージを受けない():
    """サンダーダイブ: reason=fixed_recoilのためマジックガードでは防げる。"""
    battle = t.start_battle(
        team0=[Pokemon("ラッキー", ability_name="マジックガード", move_names=["サンダーダイブ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=0,
    )
    attacker = battle.actives[0]
...
def test_サンダーダイブ_威力命中率PPが仕様通り():
    """サンダーダイブ: 威力100・命中率95・PP16（champions基準の値）であること。"""
    move_data = MOVES["サンダーダイブ"]
...
def test_シャカシャカほう_こおり状態で使うと解凍されて攻撃できる():
    """シャカシャカほう: こおり状態でも使用でき、使うと解凍される。"""
    battle = t.start_battle(
...
def test_シャカシャカほう_こおり状態の相手に当てると解凍する():
    """シャカシャカほう: くさタイプだが、被弾した相手のこおりを解凍する。"""
    battle = t.start_battle(
        team0=[Pokemon("カイリキー", move_names=["シャカシャカほう"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_シャドークロー_PPは16():
    """シャドークロー: チャンピオンズでのPPは16（.internal/champions/move_list.txt準拠）。"""
...
def test_シャドーボール_とくぼう1段階低下が発動する():
    ...
    急所ヒットでこのダメージが相手を瀕死にすると、実機仕様により追加効果の
    ランク低下は発動しない（`modify_defender_stats` の瀕死ガード）ため、
    ダメージを固定して瀕死にならないようにする。
    """
    battle = t.start_battle(
...
def test_じばく_タイプ無効で不発でも攻撃者がひんしになる():
    ...
    消費されなかった（.internal/spec/moves/じばく.md「命中判定・タイプ相性・まもるなどにより
...
def test_すてみタックル_反動ダメージが与ダメの3分の1になる():
    """すてみタックル: 反動量は max(1, round_half_down(与ダメ / 3)) で計算される。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["すてみタックル"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    ...
    hp_before = attacker.hp
    t.run_move(battle, 0)
    # max(1, round_half_down(100 * 1/3)) = max(1, round_half_down(33.33...)) = 33
    assert attacker.hp == hp_before - 33


def test_すてみタックル_反動ダメージの端数が0_5を超える場合は切り上げられる():
    """すてみタックル: 端数処理はround_half_downのため、端数が0.5を超える場合は切り捨てず繰り上がる。

    101 * 1/3 = 33.66... のため、単純な切り捨て(int())では33になるが、
    round_half_down（五捨五超入）では34になる。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["すてみタックル"])],
        team1=[Pokemon("カビゴン")],
    ...
    hp_before = attacker.hp
    t.run_move(battle, 0)
    assert attacker.hp == hp_before - 34


def test_ストーンエッジ_急所ランクが1():
    """ストーンエッジ: 急所ランク+1のため乱数0で急所が発生する。"""
    battle = t.start_battle(
        team0=[Pokemon("イワーク", move_names=["ストーンエッジ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    t.fix_random(battle, 0.0)
...
def test_ストーンエッジ_急所ランクが1_乱数大で急所なし():
    """ストーンエッジ: 乱数が急所閾値以上のとき急所にならない。"""
    battle = t.start_battle(
        team0=[Pokemon("イワーク", move_names=["ストーンエッジ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_ストーンエッジ_相手にダメージを与える():
    """ストーンエッジ: 追加効果なしの物理いわ技で相手にダメージを与える。"""
    battle = t.start_battle(
        team0=[Pokemon("イワーク", move_names=["ストーンエッジ"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_スピードスター_相手の回避率が高くても必ず命中する():
    """スピードスター: 自分の命中率、相手の回避率に関係なく必ず命中する。"""
    battle = t.start_battle(
...
def test_スモッグ_どくが発動しない():
    """スモッグ: secondary_chanceが0のときどくを付与しない。"""
    battle = t.start_battle(
...
def test_ずつき_ひるみが発動する():
    ...
    防御側はカビゴン（高耐久）にする。防御側がピカチュウ等の低耐久だと、まれに発生する
    急所判定でその一撃により瀕死になり、ひるみが付与されず flaky になるため。
    """
    battle = t.start_battle(
...
def test_せいなるつるぎ_相手の回避率ランクを無視する():
    """せいなるつるぎ: 相手の回避率ランクが上がっていても命中率が変わらない。"""
    battle = t.start_battle(
        team0=[Pokemon("ルカリオ", move_names=["せいなるつるぎ"])],
        team1=[Pokemon("カビゴン")],
    )
...
def test_せいなるほのお_こおり状態で使うと解凍されて攻撃できる():
    """せいなるほのお: こおり状態でも使用でき、使うと解凍される。"""
    battle = t.start_battle(
...
def test_そらをとぶ_PPは1ターン目のみ消費され2ターン目は消費しない():
    """そらをとぶ: 1ターン目にPPを1消費し、強制続行される2ターン目はPPを消費しない
    （2ターン合計でPP消費は1のみ）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["そらをとぶ"])],
        team1=[Pokemon("カビゴン")],
...
def test_ソーラービーム_PPは1ターン目のみ消費され2ターン目は消費しない():
    """ソーラービーム: 1ターン目にPPを1消費し、強制続行される2ターン目はPPを消費しない
    （2ターン合計でPP消費は1のみ）。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", move_names=["ソーラービーム"])],
        team1=[Pokemon("カビゴン")],
    ...
    move = attacker.moves[0]
    pp_before = move.pp
    t.run_move(battle, 0)  # 1ターン目: 溜め（PPを1消費）
    assert move.pp == pp_before - 1
    t.run_move(battle, 0)  # 2ターン目: 攻撃（PPは消費しない）
    assert move.pp == pp_before - 1
...
def test_ソーラービーム_あめ_おおあめ_すなあらし_ゆき下では威力が半減する(weather_name):
    ...
    def damage_with_weather(weather):
        ...
        t.fix_random(battle, 0.9)  # 急所判定・ダメージロールを固定し、威力差だけを見る
        defender = battle.actives[1]
        hp_before = defender.hp
        t.run_move(battle, 0)  # 1ターン目: 溜め
        t.run_move(battle, 0)  # 2ターン目: 攻撃
...
def test_ソーラービーム_はれ_おおひでり下では溜めずに1ターンで攻撃する(weather_name):
    ...
    assert not attacker.has_volatile("ソーラービーム")
    assert defender.hp < hp_before
    # 天候により溜めをスキップして単ターンで発動した場合もPP消費は1のみ
    assert move.pp == pp_before - 1
