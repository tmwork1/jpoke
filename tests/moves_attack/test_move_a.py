...
def test_うちおとす_おんみつマントでも揮発状態が付与される():
    ...
    battle.random.random = lambda: 0.99  # 急所が発生しない乱数（急所で瀕死になり付与判定が変わるのを防ぐ。1.0だとaccuracy=100の命中判定も外れるため使わない）
    t.run_move(battle, 0)
...
def test_うちおとす_ひこうタイプの浮遊が無効化される():
    ...
    battle.random.random = lambda: 0.99  # 急所が発生しない乱数（急所で瀕死になり判定が変わるのを防ぐ。1.0だとaccuracy=100の命中判定も外れるため使わない）
    defender = battle.actives[1]
