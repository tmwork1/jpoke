...
def test_うちおとす_おんみつマントでも揮発状態が付与される():
    ...
    ポッポが瀕死になると揮発状態が付与されず判定できなくなるため、
    ダメージを固定して撃墜されないようにする（急所やダメージ乱数によるflaky化防止）。
    """
    battle = t.start_battle(
...
def test_オーラぐるま_すばやさ1段階上昇が発動する():
    ...
    battle = t.start_battle(
        team0=[Pokemon("モルペコ(まんぷく)", move_names=["オーラぐるま"])],
        team1=[Pokemon("ピカチュウ")],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_オーラぐるま_ちからずくで威力上昇しすばやさ上昇は発動しないがタイプ変更は発動する():
    ...
    """
    battle = t.start_battle(
        team0=[Pokemon("モルペコ(まんぷく)", ability_name="ちからずく", move_names=["オーラぐるま"])],
        team1=[Pokemon("ピカチュウ")],
        accuracy=100,
    )
    attacker = battle.actives[0]
