...
def test_なげつける_fling_powerゼロで失敗してもごりむちゅうによる技ロックはかかる():
    """なげつける: fling_power=0のアイテムで技が不発してもPP消費後の不発として扱われ、
    ごりむちゅう（こだわり系アイテムと同様に技を固定する特性）によるロックはかかる。
    一方でアイテムは投げられなかったため消費されない。
    """
    battle = t.start_battle(
        team0=[Pokemon(
            "カビゴン", item_name="だいこんごうだま", ability_name="ごりむちゅう",
            move_names=["なげつける"],
        )],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    ...
    assert attacker.volatiles["ごりむちゅう"].move_name == "なげつける"


def test_なげつける_no_flingアイテムで失敗してもごりむちゅうによる技ロックはかかる():
    """なげつける: no_fling=Trueの投げられないアイテム（ジュエル等）で技が不発しても
    PP消費後の不発として扱われ、ごりむちゅうによるロックはかかる。
    一方でアイテムは投げられなかったため消費されない。
    """
    battle = t.start_battle(
        team0=[Pokemon(
            "カビゴン", item_name="ノーマルジュエル", ability_name="ごりむちゅう",
            move_names=["なげつける"],
        )],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_なげつける_アイテムなしで失敗してもごりむちゅうによる技ロックはかかる():
    """なげつける: アイテムを持たず技が不発してもPP消費後の不発として扱われ、
    ごりむちゅうによるロックはかかる。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="ごりむちゅう", move_names=["なげつける"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_なげつける_オレンのみは瀕死になった相手には回復を適用しない():
    """seed=2911 (LogInconsistency@move_attack.py:なげつける_apply_item_effect:2283) の回帰テスト。

    なげつけるで投げたオレンのみの回復効果は、同じ一撃で相手が瀕死(HP0)になった場合には
    適用してはならない。修正前は瀕死判定を行わず無条件に回復しており、HP0のポケモンが
    「復活」してしまうログ不整合が発生していた。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", item_name="オレンのみ", move_names=["なげつける"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    defender = battle.actives[1]
...
def test_なげつける_チイラのみは瀕死になった相手には能力ランク上昇を適用しない():
    """seed=2911 (LogInconsistency@move_attack.py:なげつける_apply_item_effect:2283) の回帰テスト。

    なげつけるで投げたチイラのみの能力ランク上昇効果は、同じ一撃で相手が瀕死(HP0)になった
    場合には適用してはならない（battle.modify_statsに瀕死ガードがないため呼び出し側での
    ガードが必要）。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", item_name="チイラのみ", move_names=["なげつける"])],
...
def test_なげつける_フィラのみは瀕死になった相手には回復もこんらんも適用しない():
    """seed=2911 (LogInconsistency@move_attack.py:なげつける_apply_item_effect:2283) の回帰テスト。

    なげつけるで投げたフィラのみのHP回復・こんらん付与効果は、同じ一撃で相手が瀕死(HP0)に
    なった場合には適用してはならない。こんらんが発動しやすい性格（ひかえめ）にしていても、
    瀕死の場合は回復・こんらんいずれも発動しない。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", item_name="フィラのみ", move_names=["なげつける"])],
        team1=[Pokemon("カビゴン", nature="ひかえめ")],
...
def test_なげつける_ぶきようでは持っていたこだわり系アイテムのロックもかからない():
    """なげつける: 使用者の特性がぶきようの場合、こだわり系アイテムの効果自体が
    無効化されているため、なげつけるが不発してもこだわりロックはかからない
    （PP消費後の不発でも一律ロックする本修正の対象は、あくまで有効なアイテム/
    特性経由のロックのみであり、ぶきようで無効化されたアイテムはロックの起点にならない）。
    """
    battle = t.start_battle(
        team0=[Pokemon(
            "カビゴン", ability_name="ぶきよう", item_name="こだわりメガネ",
            move_names=["なげつける"],
        )],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_なげつける_マジックルーム中は持っていたこだわり系アイテムのロックもかからない():
    """なげつける: マジックルーム状態ではこだわり系アイテムの効果自体も無効化されて
    いるため、なげつけるが不発してもこだわりロックはかからない
    （ぶきようと同様、アイテムが無効化されている間はロックの起点にならない）。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", item_name="こだわりメガネ", move_names=["なげつける"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
        field={"マジックルーム": 5},
    )
...
def test_ネコにこばん_相手にダメージを与える():
    """ネコにこばん: 通常攻撃技で相手にダメージを与える。"""
    battle = t.start_battle(
        team0=[Pokemon("ニャース", move_names=["ネコにこばん"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    defender = battle.actives[1]
...
def test_ネコにこばん_追加効果が発生しない():
    """ネコにこばん: 追加効果なしのため、状態異常や能力ランク変化が発生しない。"""
    battle = t.start_battle(
        team0=[Pokemon("ニャース", move_names=["ネコにこばん"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
