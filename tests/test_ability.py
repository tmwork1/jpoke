...
def test_タイプ依存攻撃補正特性(ability_name: str, move_name: str, expected: int):
    ...
    # 技実行に依存せず攻撃補正を直接確認する
    ctx = t.build_context(battle, 0)
    atk_modifier = battle.events.emit(Event.ON_CALC_ATK_MODIFIER, ctx, 4096)
    assert expected == atk_modifier
...
def test_こんじょう_全ての状態異常で攻撃1_5倍(ailment_name: str):
    battle = t.start_battle(
