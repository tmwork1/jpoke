...
def test_おうごんのからだ_相手の変化技を無効化():
    ...
    immune = t.check_event_result(battle, Event.ON_CHECK_MOVE_IMMUNE, value=False)
    assert immune
...
def test_がんじょう_一撃必殺技を無効化する():
    ...
    immune = battle.events.emit(Event.ON_CHECK_MOVE_IMMUNE, ctx, False)
...
def test_すりぬけ_みがわり越しに変化技が通る():
    ...
    assert not t.check_event_result(battle, Event.ON_CHECK_MOVE_IMMUNE, value=False)
