...
def test_すなのちから_すなあらし中に岩地面鋼技の威力が1_3倍になる(move_name: str):
    battle = t.start_battle(
...
def test_すなのちから_すなあらし中でもタイプ不一致技は威力補正なし():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="すなのちから", move_names=["でんきショック"])],
        team1=[Pokemon("ピカチュウ")],
        weather=("すなあらし", 5),
    )
    t.run_move(battle, 0)
...
def test_すなのちから_すなあらしのターン終了ダメージを受けない():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="すなのちから")],
        team1=[Pokemon("ピカチュウ")],
        weather=("すなあらし", 5),
    )
    mon = battle.actives[0]
...
def test_すなのちから_通常ポケモンはすなあらしでダメージを受ける():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ")],
        team1=[Pokemon("ピカチュウ")],
        weather=("すなあらし", 5),
