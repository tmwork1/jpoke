...
def test_アイスフェイス_かたやぶり状態でゆき発生してもフォルムチェンジしない():
    battle = t.start_battle(
        team0=[Pokemon("コオリッポ(ナイス)", ability_name="アイスフェイス")],
        team1=[Pokemon("ピカチュウ")],
    )
...
def test_アイスフェイス_物理技のダメージが0になる():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["たいあたり"])],
        team1=[Pokemon("コオリッポ(アイス)", ability_name="アイスフェイス")],
    )
    _, defender = battle.actives
    before_hp = defender.hp
    battle.roll_damage = lambda *_args, **_kwargs: 50
    t.run_move(battle, 0)
    assert defender.hp == before_hp


def test_アイスフェイス_物理技を受けるとナイスフェイスにフォルムチェンジする():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["たいあたり"])],
        team1=[Pokemon("コオリッポ(アイス)", ability_name="アイスフェイス")],
    )
...
def test_アイスフェイス_特殊技のダメージを防がない():
    ...
    _, defender = battle.actives
    before_hp = defender.hp
    battle.roll_damage = lambda *_args, **_kwargs: 40
    t.run_move(battle, 0)
    assert defender.hp == before_hp - 40
    assert defender.name == "コオリッポ(アイス)"
...
def test_うのミサイル_うのみのすがたで攻撃を受けると通常に戻りぼうぎょが下がる():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["たいあたり"])],
        team1=[Pokemon("ウッウ(うのみのすがた)", ability_name="うのミサイル")],
        accuracy=100,
    )
...
def test_うのミサイル_みがわりで防いだ場合は吐き出しが発動しない():
    ...
    t.run_move(battle, 0)
    assert defender.name == "ウッウ(うのみのすがた)"
    assert attacker.hp == before_attacker_hp


def test_うのミサイル_交代するとフォルムが通常のすがたに戻る():
    battle = t.start_battle(
        team0=[Pokemon("ウッウ(うのみのすがた)", ability_name="うのミサイル"), Pokemon("ピカチュウ")],
        team1=[Pokemon("ピカチュウ")],
    )
    mon = battle.actives[0]
    assert mon.name == "ウッウ(うのみのすがた)"
...
def test_スワームチェンジ_フォルムチェンジ後にHPが増加する():
    battle = t.start_battle(
...
def test_ゼロフォーミング_テラスタル時にフィールドを消去する():
    """ゼロフォーミング: テラスタル時に場のフィールドを消去する"""
    from jpoke.enums import Event
    from jpoke.core import AttackContext, EventContext
    battle = t.start_battle(
    ...
    attacker, _ = battle.actives
    assert battle.terrain.name == "グラスフィールド"
    ctx = EventContext(source=attacker)
    battle.events.emit(Event.ON_TERASTALLIZE, ctx=ctx, value=None)
    assert battle.terrain.name == "", "ゼロフォーミングでフィールドが消去される"


def test_ゼロフォーミング_テラスタル時に天候を消去する():
    """ゼロフォーミング: テラスタル時に場の天候を消去する"""
    from jpoke.enums import Event
    from jpoke.core import AttackContext, EventContext
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="ゼロフォーミング")],
...
def test_そうだいしょう_味方5体ひんしで補正率上限0_5():
    ...
    battle = t.start_battle(team0=team, team1=[Pokemon("フシギダネ")])
    player0 = battle.players[0]
...
def test_ちどりあし_こんらん中命中率が半減する():
    ...
    battle.volatile_manager.apply(battle.actives[0], "こんらん")
    t.run_move(battle, 1)
    assert battle.move_executor.accuracy == 100 * 2048 // 4096  # 50
...
def test_てんきや_エアロック中は天気変化でもフォルムチェンジしない():
    battle = t.start_battle(
...
def test_てんきや_天気に対応したフォルムになる(weather: str, form: str):
    battle = t.start_battle(
        team0=[Pokemon("ポワルン", ability_name="てんきや")],
        team1=[Pokemon("ピカチュウ")],
        weather=(weather, 5),
...
def test_てんきや_天気変化で即座にフォルムが変わる():
    battle = t.start_battle(
        team0=[Pokemon("ポワルン", ability_name="てんきや")],
        team1=[Pokemon("ピカチュウ")],
    )
