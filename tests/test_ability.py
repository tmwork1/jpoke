...
def test_ARシステム_フェアリーメモリでフェアリータイプになる():
    ...
    battle = t.start_battle(
        team0=[Pokemon("シルヴァディ", ability_name="ARシステム", item_name="フェアリーメモリ")],
        team1=[Pokemon("ピカチュウ")],
    )
...
def test_query_先攻後攻を問い合わせできる():
    battle = t.start_battle(
        team0=[Pokemon("コイル", move_names=["でんきショック"])],
        team1=[Pokemon("ピカチュウ", move_names=["でんきショック"])],
    )
    player0, player1 = battle.players
    state0 = battle.player_states[player0]
    state1 = battle.player_states[player1]

    state0.action_order_index = 0
    state1.action_order_index = 1

    assert battle.query.is_first_actor(player0) is True
    assert battle.query.is_second_actor(player0) is False
    assert battle.query.acted_before(player0) is True


def test_query_行動順未確定ならNoneを返す():
    battle = t.start_battle(
        team0=[Pokemon("コイル", move_names=["でんきショック"])],
        team1=[Pokemon("ピカチュウ", move_names=["でんきショック"])],
    )
    ...
    assert battle.query.is_first_actor(player0) is None
    assert battle.query.is_second_actor(player0) is None
    assert battle.query.acted_before(player0) is None
...
def test_あめふらし_通常天候を上書きする(initial_weather: str):
    ...
    battle.switch_manager.run_switch(
        battle.players[0],
        battle.player_states[battle.players[0]].team[1],
    )
...
def test_ありじごく_交代不可():
    battle = t.start_battle(
        ...
        team1=[Pokemon("ピカチュウ", ability_name="ありじごく")],
    )
...
def test_かげふみ_かげふみ持ちは交代可能():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="かげふみ"), Pokemon("ピカチュウ")],
        team1=[Pokemon("ピカチュウ", ability_name="かげふみ"), Pokemon("ピカチュウ")],
    )
    assert t.can_switch(battle, 0)
...
def test_かいりきバサミ_こうげき低下のみ防ぐ():
    battle = t.start_battle(
        team0=[Pokemon("カイリキー", ability_name="かいりきバサミ")],
        team1=[Pokemon("ピカチュウ")],
    )
...
def test_かがくへんかガス_解除後は特性が再び有効化される():
    ...
    battle.switch_manager.run_switch(player, battle.player_states[player].team[1])
...
def test_かるわざ_アイテムを失ってから再入場しても発動しない():
    ...
    mon = battle.actives[0]
    battle.consume_item(mon)
    battle.run_switch(battle.players[0], battle.player_states[battle.players[0]].team[1])
    battle.run_switch(battle.players[0], battle.player_states[battle.players[0]].team[0])
    assert battle.calc_effective_speed(mon) == mon.stats["S"]
...
def test_じしんかじょう_相手を倒すと攻撃1段階上昇する():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="じしんかじょう", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker, defender = battle.actives
    defender.hp = 1
...
def test_すなかき_すなあらしで素早さ2倍():
    battle = t.start_battle(team1=[Pokemon("ピカチュウ")],
                            team0=[Pokemon("ピカチュウ", ability_name="すなかき")],
                            weather=("すなあらし", 999),
                            )
    mon = battle.actives[0]
    assert battle.calc_effective_speed(mon) == mon.stats["S"] * 2
...
def test_ゆきかき_ゆきで素早さ2倍():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="ゆきかき")],
        team1=[Pokemon("ピカチュウ")],
        weather=("ゆき", 5),
    )
    mon = battle.actives[0]
...
def test_スカイスキン_ノーマル技をひこうタイプに変換する():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="スカイスキン", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    move = attacker.moves[0]
...
def test_スカイスキン_元からひこうタイプの技は威力補正なし():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="スカイスキン", move_names=["アクロバット"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    defender = battle.actives[1]
...
def test_フェアリースキン_ノーマル技をフェアリータイプに変換する():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="フェアリースキン", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    move = attacker.moves[0]
...
def test_フェアリースキン_元からフェアリータイプの技は威力補正なし():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="フェアリースキン", move_names=["じゃれつく"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    defender = battle.actives[1]
...
def test_フリーズスキン_ノーマル技をこおりタイプに変換する():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="フリーズスキン", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    move = attacker.moves[0]
...
def test_フリーズスキン_元からこおりタイプの技は威力補正なし():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="フリーズスキン", move_names=["アイススピナー"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    defender = battle.actives[1]
...
def test_ばけのかわ_交代しても再有効化されない():
    ...
    battle.switch_manager.run_switch(
        battle.players[1],
        battle.player_states[battle.players[1]].team[1],
    )
    battle.switch_manager.run_switch(
        battle.players[1],
        battle.player_states[battle.players[1]].team[0],
    )
    assert battle.actives[1].ability.enabled is False
...
def test_ぼうおん_音技を無効化する():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="ぼうおん")],
        team1=[Pokemon("ピカチュウ", move_names=["バークアウト"])],
    )
    defender, attacker = battle.actives
    battle.run_move(attacker, attacker.moves[0])
    assert defender.hp == defender.max_hp
...
def test_ぼうおん_かたやぶりで無効():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="ぼうおん")],
        team1=[Pokemon("ピカチュウ", ability_name="かたやぶり", move_names=["バークアウト"])],
    )
    defender, attacker = battle.actives
    battle.run_move(attacker, attacker.moves[0])
    assert defender.hp < defender.max_hp
...
def test_よわき_HP半分以下で攻撃補正0_5倍():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="よわき", move_names=["でんきショック"])],
        team1=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    attacker.hp = 1
