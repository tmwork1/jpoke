...
def _activate_mold_breaker(battle: Battle, atk_idx: int):
    ...
    battle.events.emit(Event.ON_ACTIVATE_MOLD_BREAKER, ctx, True)
...
def test_あとだし_高優先度技は先攻():
    ...
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="あとだし", moves=["でんこうせっか"])],
        foe=[Pokemon("ピカチュウ")],
    )
    t.reserve_command(battle)
    order = battle.calc_action_order()
...
def test_あめふらし_通常天候を上書きする(initial_weather: str):
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("ピカチュウ", ability="あめふらし")],
        foe=[Pokemon("ピカチュウ")],
...
def test_おわりのだいち_らんきりゅう以外上書きする(weather_name: str):
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("ピカチュウ", ability="おわりのだいち")],
        foe=[Pokemon("ピカチュウ")],
...
def test_らんきりゅう_すべての天候を上書きする(weather_name: str):
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("ピカチュウ", ability="デルタストリーム")],
        foe=[Pokemon("ピカチュウ")],
        weather=(weather_name, 99)
    )
    battle.switch_manager.run_switch(battle.players[0], battle.players[0].team[1])
...
def test_強天候始動特性_退場時に解除される(ability_name: str, weather_name: str):
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability=ability_name), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ")],
    )
    battle.switch_manager.run_switch(battle.players[0], battle.players[0].team[1])
...
def test_ありじごく_交代不可():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("ピカチュウ")],
...
def test_かげふみ_かげふみ持ちは交代可能():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="かげふみ"), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ", ability="かげふみ"), Pokemon("ピカチュウ")],
    )
    assert t.can_switch(battle, 0)
...
def test_いたずらごころ_あくタイプ相手には変化技が無効化される():
    ...
    assert not battle.events.emit(Event.ON_APPLY_MOVE, ctx, True)
...
def test_タイプ依存攻撃補正特性(ability_name: str, move_name: str, expected: int):
    ...
    assert expected == t.calc_damage_modifier(battle, Event.ON_CALC_ATK_MODIFIER, atk_idx=0)
...
def test_地形始動特性_同一地形が有効時は継続ターンを更新しない(ability_name: str, terrain_name: str):
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("ピカチュウ", ability=ability_name)],
        foe=[Pokemon("ピカチュウ")],
...
def test_エレキメイカー_別フィールドを上書きする(initial_terrain: str):
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("ピカチュウ", ability="エレキメイカー")],
        foe=[Pokemon("ピカチュウ")],
...
def test_おうごんのからだ_相手の変化技を無効化():
    ...
    immune = t.check_event_result(battle, Event.ON_APPLY_MOVE, value=False)
    assert immune
...
def test_かがくへんかガス_解除後は特性が再び有効化される():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="かがくへんかガス"), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ", ability="いかく")],
    )
...
def test_かそく_交代直後のターンは発動しない():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("ピカチュウ", ability="かそく", moves=["でんきショック"])],
        foe=[Pokemon("ピカチュウ")],
    )
    # 交代したターンはかそくが発動しない
    t.reserve_command(
...
def test_かたいツメ_接触技のみ威力補正1_3倍():
    ...
    battle_non_contact = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="かたいツメ", moves=["でんきショック"])],
        foe=[Pokemon("ピカチュウ")],
    )
...
def test_かるわざ_アイテムを失ってから再入場しても発動しない():
    # TODO : battle.item_manager.lose_item()で直接アイテムを失わせるように修正
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="かるわざ", item="オボンのみ"), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ")],
    )
    mon = battle.actives[0]
    battle.consume_item(mon)
...
def test_がんじょう_HP満タン時の致死ダメージでHP1残る():
    battle = t.start_battle(
        ally=[Pokemon("コイル", ability="がんじょう")],
        foe=[Pokemon("ガブリアス", moves=["じしん"])],
    )
...
def test_がんじょう_一撃必殺技を無効化する():
    ...
    ctx = t.build_context(battle, atk_idx=1)
    assert not battle.events.emit(Event.ON_APPLY_MOVE, ctx, True)
...
def test_がんじょう_かたやぶり相手の一撃必殺は無効化されない():
    ...
    immune = battle.events.emit(Event.ON_APPLY_MOVE, ctx, False)
    assert not immune
...
def test_ききかいひ_被弾前HPが半分以下なら発動しない():
    battle = t.start_battle(
        ally=[Pokemon("コソクムシ", ability="ききかいひ"), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ", moves=["たいあたり"])],
    )
    defender = battle.actives[0]
    attacker = battle.actives[1]
...
def test_でんきエンジン_かたやぶりでは無効化される():
    battle = t.start_battle(
        ally=[Pokemon("イーブイ", ability="でんきエンジン")],
        foe=[Pokemon("ピカチュウ", ability="かたやぶり", moves=["でんきショック"])],
    )
    defender = battle.actives[0]
    attacker = battle.actives[1]
    before_hp = defender.hp
...
def test_さいせいりょく_交代で控えに戻ると回復する():
    battle = t.start_battle(
        ally=[Pokemon("ヤドン", ability="さいせいりょく"), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ")],
    )
    mon = battle.actives[0]
    mon.hp = 1
...
def test_しぜんかいふく_交代時に状態異常回復():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="しぜんかいふく"), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ")],
    )
    mon = battle.actives[0]
...
def test_しめりけ_じばくを失敗させる():
    ...
    result = t.check_event_result(battle, Event.ON_TRY_MOVE)
    assert not result
...
def test_ちからもち_特殊技では攻撃補正なし():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="ちからもち", moves=["でんきショック"])],
        foe=[Pokemon("ピカチュウ")],
    )
    assert 4096 == t.calc_damage_modifier(battle, Event.ON_CALC_ATK_MODIFIER)
...
def test_ちくでん_でんき技を無効化して4分の1回復する():
    ...
    battle.modify_hp(defender, v=-20, reason="")
    before = defender.hp
...
def test_てきおうりょく_元タイプ一致テラスタルで2_25倍になる():
    battle = t.start_battle(
        ally=[Pokemon("リザードン", ability="てきおうりょく", tera_type="ほのお", moves=["ひのこ"])],
        foe=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
    defender = battle.actives[1]
...
def test_テラスシェル_HP満タンでないと発動しない():
    ...
    assert 4096 == t.calc_damage_modifier(battle, Event.ON_CALC_DEF_TYPE_MODIFIER, atk_idx=0)
...
def test_ばけのかわ_交代しても再有効化されない():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", moves=["たいあたり"])],
        foe=[Pokemon("ピカチュウ", ability="ばけのかわ"), Pokemon("ピカチュウ")],
    )
    # 初回被弾でばけのかわを消費
    attacker = battle.players[0].active_mon
    battle.move_executor.run_move(attacker, attacker.moves[0])
...
def test_はらぺこスイッチ_テラスタル中はターン終了時に切り替わらない():
    battle = t.start_battle(
        ally=[Pokemon("モルペコ", ability="はらぺこスイッチ", tera_type="あく")],
...
def test_はらぺこスイッチ_テラスタル交代時はフォルム維持する():
    battle = t.start_battle(
        ally=[Pokemon("モルペコ", ability="はらぺこスイッチ", tera_type="あく"), Pokemon("ピカチュウ")],
        foe=[Pokemon("ピカチュウ")],
    )
    player = battle.players[0]
    mon = player.team[0]
...
def test_ぼうじん_粉技を無効化する():
    battle = t.start_battle(
        ...
        foe=[Pokemon("ピカチュウ", moves=["キノコのほうし"])],
    )
    defender, attacker = battle.actives
    battle.run_move(attacker, attacker.moves[0])
    assert not defender.ailment.is_active
...
def test_よわき_HP半分以下で攻撃補正0_5倍():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="よわき", moves=["でんきショック"])],
        foe=[Pokemon("ピカチュウ")],
    )
    attacker = battle.actives[0]
...
def test_リーフガード_かたやぶりと対面中のどくびしは毒にならない():
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ", ability="リーフガード"), Pokemon("ピカチュウ", ability="リーフガード")],
        foe=[Pokemon("ピカチュウ", ability="かたやぶり")],
        weather=("はれ", 5),
