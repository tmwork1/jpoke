...
def test_かちき_自分由来の能力低下では発動しない():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="かちき")])
...
def test_ミラーアーマー_自己デメリットは反射しない():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="ミラーアーマー")])
    mon = battle.actives[0]
...
def test_パラドックス特性_かがくへんかガスで無効化される(ability_name: str, field: tuple[str, str]):
    ...
    battle = t.start_default_battle(**kwargs)
...
def test_すなかき_すなあらし以外では素早さ据え置き():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="すなかき")])
    mon = battle.actives[0]
...
def test_すいすい_あめ以外では素早さ据え置き():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="すいすい")])
    mon = battle.actives[0]
    assert battle.calc_effective_speed(mon) == mon.stats["S"]
...
def test_ようりょくそ_はれ以外では素早さ据え置き():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="ようりょくそ")])
    mon = battle.actives[0]
    assert battle.calc_effective_speed(mon) == mon.stats["S"]
...
def test_ぜったいねむり_登場時にねむり状態になる():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="ぜったいねむり")])
    assert battle.actives[0].ailment.name == "ねむり"


def test_グラスメイカー_登場時にグラスフィールド展開():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="グラスメイカー")])
...
def test_どんかん_指定状態をイベント段階で無効化():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="どんかん")])
    mon = battle.actives[0]
...
def test_状態異常無効系特性_対応状態異常を防ぐ(ability_name: str, ailment_name: str):
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability=ability_name)])
    mon = battle.actives[0]
...
def test_マイペース_状態異常は防がない_現実装確認():
    battle = t.start_default_battle(ally=[Pokemon("ピカチュウ", ability="マイペース")])
    mon = battle.actives[0]
