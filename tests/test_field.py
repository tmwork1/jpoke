...
def test_すなあらし_いわタイプ特防上昇():
    ...
    assert_def_modifier(battle, POWER_BOOST, "すなあらしでいわタイプの防御補正が上がらない")
...
def test_ゆき_こおりタイプ防御上昇():
    ...
    assert_def_modifier(battle, POWER_BOOST, "ゆきでこおりタイプの防御補正が上がらない")
...
def test_エレキフィールド_ねむり回復():
    """エレキフィールド: 出場時のねむり回復"""
    battle = t.start_battle(
        ally=[Pokemon("ピカチュウ"), Pokemon("フシギダネ")],
        terrain=("エレキフィールド", DEFAULT_DURATION),
    )
    sleeper = battle.players[0].team[1]
    applied = sleeper.apply_ailment(battle.events, "ねむり")
    assert applied, "ねむりの付与に失敗した"

    player = battle.players[0]
    battle.run_switch(player, sleeper)
    assert not battle.actives[0].ailment.is_active, "エレキフィールドでねむりが回復しない"
...
def test_じゅうりょく_命中補正():
    ...
    result = battle.events.emit(Event.ON_CALC_ACCURACY, ctx, 60)
    assert result == 100, "じゅうりょくで命中率補正が正しく適用されない"
