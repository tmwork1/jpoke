...
def test_ねばねばネット():
    ...
    before_rank = battle.players[0].team[1].boosts["spe"]
    active = t.run_switch(battle, 0, 1)
    after_rank = active.boosts["spe"]
...
def test_ねばねばネット_あつぞこブーツで無効():
    ...
    active = t.run_switch(battle, 0, 1)
    assert active.boosts["spe"] == 0, "あつぞこブーツ持ちはねばねばネットの影響を受けない"
...
def test_ねばねばネット_あまのじゃくは1段階上がる():
    ...
    active = t.run_switch(battle, 0, 1)
    assert active.boosts["spe"] == 1, "あまのじゃく持ちはすばやさが上がる"
...
def test_ねばねばネット_たんじゅんは2段階下がる():
    ...
    active = t.run_switch(battle, 0, 1)
    assert active.boosts["spe"] == -2, "たんじゅん持ちはすばやさが2段階下がる"
...
def test_みかづきのまい_いやしのねがいと重複した場合先に設置された方が優先発動する():
    """みかづきのまい: いやしのねがい状態と重複した場合、先に設置されたいやしのねがいが優先して
    発動する。回復対象が残っていなければみかづきのまいは保留され続ける"""
    battle = t.start_battle(
        team0=[
            Pokemon("クレセリア"),
    ...
    # いやしのねがいを先に設置し、後からみかづきのまいを設置する
    battle.side_managers[0].activate("いやしのねがい", 1)
    battle.side_managers[0].activate("みかづきのまい", 1)
    ...
    assert active.hp == active.max_hp
    assert not active.ailment.is_active
    side = battle.get_side(active)
    # いやしのねがいが先に発動してHPと状態異常を回復し、フィールドが消費される
    assert not side.get("いやしのねがい").is_active
    # みかづきのまいは回復対象（PP）が残っていないため保留され続ける
    assert side.get("みかづきのまい").is_active


def test_みかづきのまい_かいふくふうじ状態でもPPと状態異常は回復するがHPは回復しない():
    """みかづきのまい: かいふくふうじ状態のポケモンにも効果は発動し、
    状態異常とPPは回復するがHPは回復しない"""
    battle = t.start_battle(
        team0=[
            Pokemon("クレセリア"),
            Pokemon("カビゴン", move_names=["たいあたり"]),
    ...
    battle.ailment_manager.apply(bench, "まひ")
    bench.moves[0].pp = 0
    battle.volatile_manager.apply(bench, "かいふくふうじ", count=3)

    active = t.run_switch(battle, 0, 1)

    assert active is bench
    assert active.hp == 1, "かいふくふうじ状態ではHPは回復しない"
    assert not active.ailment.is_active, "かいふくふうじ状態でも状態異常は回復する"
    assert active.moves[0].pp == active.moves[0].data.pp, "かいふくふうじ状態でもPPは回復する"
...
def test_ワンダールーム_物理技のランク補正はぼうぎょランクのまま():
    ...
    def _final_defense(def_rank: int, spd_rank: int) -> int:
        ...
        defender.boosts["def"] = def_rank
        defender.boosts["spd"] = spd_rank
