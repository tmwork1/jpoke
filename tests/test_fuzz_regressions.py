...
def test_ききかいひ_通常の交代コマンドで場に出た直後の入場時ダメージによる緊急交代が同一ターン内で解決される():
    ...
    修正後は_process_events_after_switchが_resolve_ejectpack_after_switchと同様に
    _resolve_emergency_after_switchでInterrupt.EMERGENCYを解決してから呼び出し元へ
    処理を戻すため、同一ターン内でもう一方のプレイヤーの技が正しく実行され、
    次ターンも問題なく進行することを確認する。
    """
    battle = t.start_battle(
