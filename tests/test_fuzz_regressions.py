...
from framework import TreeSearchPlayer  # noqa: E402
...
def test_木探索_観測用盤面をコピーしてもobserverが引き継がれ瀕死交代の無限再帰が起きない():
    ...
    scripts/tree_search/framework.py の TreeSearchPlayer._worst_case_over_opponent は
    ...
    victim = VictimPlayer(name="Victim")
    victim.team = [
        Pokemon("コイキング", move_names=["はねる"]),
        Pokemon("ヒトカゲ", move_names=["たいあたり"]),
    ]

    searcher = TreeSearchPlayer(
        name="Searcher",
        max_plies=1,
        fallback=lambda battle, player: battle.get_available_commands(player)[0],
    )
    searcher.team = [
