...
def test_まねっこ_わるあがきコピー中にレッドカードで攻撃者が強制交代してもValueErrorが発生しない():
    """seed=430 (ValueError@battle.py:foe:473) の回帰テスト。

    まねっこでコピーしたわるあがきが、コピー元の攻撃対象（レッドカード持ち）に
    実ダメージを与えると、レッドカードが発動して攻撃者（まねっこ使用者）が
    強制交代させられる。この強制交代が Event.ON_HIT の時点（本来より早い
    タイミング。詳細は test_わるあがき_反動ダメージがON_HITイベントの実ダメージ量を
    破壊せずレッドカードが正しいタイミングで発動する を参照）で発生すると、
    ...
    ON_HIT の実ダメージ量を破壊しなくなり、まねっこ経由でも例外なく完了する
    ことを確認する。
    """
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", item_name="レッドカード")],
        team1=[
            Pokemon("アブリボン", move_names=["まねっこ"]),
            Pokemon("ニドクイン"),
        ],
    )
    # 直前のターンで「わるあがき」が使われた状況を再現する
    battle.last_used_move_name = "わるあがき"
    t.fix_damage(battle, 10)

    t.run_move(battle, 1, 0)  # 修正前はここでValueErrorになっていた

    # コピーしたわるあがきのダメージは正しく相手(カビゴン)に通っている
    assert battle.actives[0].hp == battle.actives[0].max_hp - 10
    # レッドカードでまねっこ使用者(アブリボン)が控え(ニドクイン)へ強制交代させられている
