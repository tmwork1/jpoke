...
# subprocessで起動する子プロセス用のインタプリタ。sys.executableに papermill/jpoke が
# 入っていない環境（Bashセッションによって `python` の解決先がぶれる等）でも、
# リポジトリ直下の .venv に papermill が入っていればそちらを優先して使う。
PYTHON = resolve_subprocess_python("papermill", "jpoke")
...
def test_notebook_examples_エラーなく実行できる(notebook_name, tmp_path):
    ...
    result = subprocess.run(
        [PYTHON, "-m", "papermill", str(notebook_path), str(output_path), *extra_params],
        cwd=ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
