...
def test_observationのdecision_randomは共有されrandomは独立している():
    ...
    """
    p0 = RandomPlayer("p0")
    p0.add_pokemon("ピカチュウ", moves=["でんきショック"])
    p1 = RandomPlayer("p1")
    p1.add_pokemon("ゼニガメ", moves=["みずでっぽう"])
...
def test_randomplayerが選ぶコマンドは常にget_available_commandsに含まれる():
    ...
    p0.add_pokemon("ピカチュウ", moves=["でんきショック", "でんこうせっか", "アイアンテール"])
    p1 = _RecordingRandomPlayer("p1")
    p1.add_pokemon("ゼニガメ", moves=["みずでっぽう", "たいあたり"])
...
def test_無限交代ループの再現条件下でもコマンドが技を含んで決着する():
    ...
    for seed in (1, 2, 3, 5):
        ...
        p0.add_pokemon("ピカチュウ", moves=["かみなり"])
        p0.add_pokemon("ヒトカゲ", moves=["かえんほうしゃ"])
        p0.add_pokemon("フシギダネ", moves=["ギガドレイン"])

        p1 = RandomPlayer("Team B")
        p1.add_pokemon("ゼニガメ", moves=["なみのり"])
        p1.add_pokemon("コラッタ", moves=["すてみタックル"])
        p1.add_pokemon("ピッピ", moves=["ムーンフォース"])
