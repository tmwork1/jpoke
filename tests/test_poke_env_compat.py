...
def test_observation_randomは本体のbattle_randomと同じ参照を共有する():
    """`build_observation()` が返す観測用コピー（sim）の `random` が、deepcopyされた
    独立インスタンスではなく本体 `battle.random` と同一オブジェクトであることを確認する。

    これが崩れていた旧実装では、RandomPlayer（`battle.random.choice(...)` で選ぶ）が
    `sim.random` を消費しても本体側の `battle.random` の状態が一切進まなかった。
    技を使わず交代だけが選ばれ続けるターンでは本体のrandomがどこからも消費されないため、
    次のターンも全く同じ乱数状態から観測用コピーが作られ、同じ選択が繰り返される
    無限交代ループに陥っていた（`examples/02_team_battle.py` の seed=1,2,3,5 で再現していた
    重大バグ）。
    """
    p0 = RandomPlayer("p0")
    ...
    assert sim.random is battle.random
