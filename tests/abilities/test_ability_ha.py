...
def test_はらぺこスイッチ_かがくへんかガス中はターン終了時に切り替わらない():
    ...
    battle = t.start_battle(
        team0=[Pokemon("モルペコ(まんぷく)", ability_name="はらぺこスイッチ")],
...
def test_はらぺこスイッチ_ダルマモード等のフォルムチェンジより後の優先度で登録されている():
    ...
    .internal/spec/turn.md の ON_TURN_END では両者とも priority=160 の同グループとして
...
def test_はらぺこスイッチ_テラスタル中にひんしになるとまんぷくもように戻る():
    ...
    battle = t.start_battle(
        team0=[Pokemon("モルペコ(まんぷく)", ability_name="はらぺこスイッチ", tera_type="あく"), Pokemon("ピカチュウ")],
        team1=[Pokemon("ピカチュウ")],
    )
...
def test_はらぺこスイッチ_テラスタル中はターン終了時に切り替わらない():
    battle = t.start_battle(
        team0=[Pokemon("モルペコ(まんぷく)", ability_name="はらぺこスイッチ", tera_type="あく")],
        team1=[Pokemon("ピカチュウ")],
    )
...
def test_はらぺこスイッチ_交代時は通常まんぷくへ戻る():
    battle = t.start_battle(
        team0=[Pokemon("モルペコ(まんぷく)", ability_name="はらぺこスイッチ"), Pokemon("ピカチュウ")],
        team1=[Pokemon("ピカチュウ")],
    )
    player = battle.players[0]
    mon = battle.player_states[player].team[0]
...
def test_ばけのかわ_こんらんの自傷ダメージのブロックに伴う消費が致死の場合発動アナウンスログが勝敗確定ログより先に記録される():
    """ばけのかわ_block_confusion_damage は、こんらんの自傷ダメージを肩代わりする際に
    自身が最大HPの1/8を消費するが、この消費ダメージが致死の場合でも
    battle.modify_hp() より先に特性発動アナウンスログ（LogCode.ABILITY_TRIGGERED）を
    記録するため、勝敗確定ログ（GAME_WON/GAME_LOST）より先にアナウンスログが
    記録されることを確認する。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="ばけのかわ")],
        team1=[Pokemon("カビゴン")],
    ...
    assert ability_idx < lost_idx


def test_ばけのかわ_こんらんの自傷ダメージも防ぐ():
    """こんらんの自傷ダメージ（ON_MODIFY_NON_MOVE_DAMAGE 経由）もばけのかわで防ぎ、
    最大HPの1/8のみ消費する（.internal/spec/abilities/ばけのかわ.md「こんらん時の自分への
    攻撃には発動し、ダメージを防ぐ」参照）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="ばけのかわ")],
        team1=[Pokemon("カビゴン")],
...
def test_ふうりょくでんき_致命打でも発動する():
    """ふうりょくでんき: 風の技でHPが0になったときも、特性が発動してからひんしになる
    （seed=2225 fuzz_log 回帰テスト。.internal/spec/abilities/ふうりょくでんき.md）。

    瀕死になった当のポケモン自身にじゅうでん状態そのものを付与することは
    VolatileManager.apply() が意図的にブロックする（瀕死のポケモンには揮発性状態を
    付与しない実機仕様）ため、ここでは特性発動アナウンス自体が記録されることを確認する。
    """
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["かぜおこし"])],
        team1=[Pokemon("カビゴン", ability_name="ふうりょくでんき")],
    ...
    logs = [
        ...
        if log.log == LogCode.ABILITY_TRIGGERED
        and log.payload is not None
        and getattr(log.payload, "ability", None) == "ふうりょくでんき"
    ]
    assert len(logs) == 1
...
def test_プレッシャー_ねばねばネットは相手の場が対象だがPP消費が増えない():
    ...
    唯一プレッシャーの影響を受けない例外（.internal/spec/abilities/プレッシャー.md参照。
...
def test_へんげんじざい_かがくへんかガスの効果中は発動しない():
    ...
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="へんげんじざい", move_names=["ほのおのちかい"])],
...
def test_へんげんじざい_テラスタル中は発動しない():
    ...
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="へんげんじざい", move_names=["ほのおのちかい"], tera_type="みず")],
        team1=[Pokemon("カビゴン")],
    )
    attacker = battle.actives[0]
...
def test_へんげんじざい_もりののろいで追加されたタイプは発動時にリセットされる():
    ...
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="へんげんじざい", move_names=["もりののろい", "ほのおのちかい"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_へんげんじざい_交代でリセットされ再発動できる():
    battle = t.start_battle(
        team0=[
            Pokemon("ピカチュウ", ability_name="へんげんじざい", move_names=["たいあたり", "ほのおのちかい"]),
            Pokemon("ピカチュウ"),
        ],
        team1=[Pokemon("カビゴン")],
    )
...
def test_へんげんじざい_単タイプで技タイプと一致する場合は発動しない():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="へんげんじざい", move_names=["でんげきは", "ほのおのちかい"])],
        team1=[Pokemon("カビゴン")],
    )
    attacker = battle.actives[0]
...
def test_へんげんじざい_同一滞在で1回のみ発動():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="へんげんじざい", move_names=["たいあたり", "ほのおのちかい"])],
        team1=[Pokemon("カビゴン")],
    )
    attacker = battle.actives[0]
...
def test_へんしょく_ちからずくの技を受けてもタイプ変化しない():
    ...
    受けた場合はタイプが変化しない（.internal/spec/abilities/へんしょく.md参照）。"""
    battle = t.start_battle(
...
def test_ほうし_攻撃側がくさタイプなら発動しない():
    """ほうし: 攻撃側がくさタイプを持つ場合は無効（フシギダネはどくタイプも持つため、
    どくタイプ自体の免疫と区別するためまひが選ばれる乱数値で検証する）。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="ほうし")],
...
def test_ほうし_攻撃側がぼうじんゴーグル所持なら発動しない():
    """ほうし: 攻撃側がぼうじんゴーグルを持つ場合は無効。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="ほうし")],
        team1=[Pokemon("コラッタ", item_name="ぼうじんゴーグル", move_names=["たいあたり"])],
        accuracy=100,
    )
...
def test_ほうし_攻撃側がぼうじん特性なら発動しない():
    """ほうし: 攻撃側の特性がぼうじんの場合は無効。"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="ほうし")],
        team1=[Pokemon("コラッタ", ability_name="ぼうじん", move_names=["たいあたり"])],
        accuracy=100,
    )
    battle.random.random = lambda: 0.0
...
def test_ほうし_連続攻撃技でねむりが発動すると中断される():
    ...
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="ほうし")],
        team1=[Pokemon("コラッタ", move_names=["みだれづき"])],
        accuracy=100,
    )
    battle.random.random = lambda: 0.19  # ねむりが選ばれる乱数値
    ...
    # 1ヒット目でねむりになり中断されるため、ダメージは1回分のみ
...
def test_ほのおのからだ_致命打でも発動する():
    """ほのおのからだ: 攻撃技でひんしになったときも発動する
    （seed=2225 fuzz_log 回帰テスト。.internal/spec/abilities/ほのおのからだ.md）"""
    battle = t.start_battle(
        team0=[Pokemon("カビゴン", ability_name="ほのおのからだ")],
        team1=[Pokemon("ピカチュウ", move_names=["たいあたり"])],
        accuracy=100,
