...
def test_クォークチャージ_しろいハーブより先に発動判定する():
    ...
    (.internal/spec/turn.md ON_SWITCH_IN: 140 クォークチャージ、160 しろいハーブ)。
...
def test_こだいかっせい_しろいハーブより先に発動判定する():
    ...
    (.internal/spec/turn.md ON_SWITCH_IN: 140 こだいかっせい、160 しろいハーブ)。
    バトンタッチで引き継いだ下降ランクを参照して能力を選ぶため、
    しろいハーブがランクを戻す前に『とくこう』が選ばれる。"""
    battle = t.start_battle(
        team0=[
...
def test_こだいかっせい_にほんばれ自然消滅でブーストエナジーへ切り替わる():
    ...
    あれば即座に消費して再発動する
    （.internal/spec/abilities/こだいかっせい.md「にほんばれ状態が消滅すると一旦特性の
...
def test_こだいかっせい_にほんばれ自然消滅時に効果終了アナウンスが記録される():
    """こだいかっせい: にほんばれ状態が自然消滅して補正が一旦解除される際、
    「こだいかっせいの効果が切れた」という終了専用ログ（ABILITY_EFFECT_ENDED）が
    記録される。修正前はこの終了アナウンス自体が完全に欠落していた
    （fuzzログ回帰: seed=3987, LogInconsistency@ability.py:スロースタート_tick:2269の
    修正と同根の欠落。ability_paradox.py refresh_paradox_charge_state内の
    _deactivate_paradox_boost呼び出し）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="こだいかっせい")],
        team1=[Pokemon("ピカチュウ")],
        weather=("はれ", 1),
    ...
    ended = [
        ...
        and getattr(log.payload, "ability", None) == "こだいかっせい"
    ]
    assert len(ended) == 1
    assert getattr(ended[0].payload, "message", None) == "こだいかっせいの効果が切れた"
...
def test_はじまりのうみ_とくせいなし状態になると強制天候を解除する():
    """いえき等ではじまりのうみ持ち自身が「とくせいなし」状態になった直後も、
    ON_ABILITY_DISABLEDハンドラが自己言及的な無効化理由（とくせいなし）で
    フィルタされず発動し、おおあめが正しく解除される（fuzz_log seed=1204参照）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="はじまりのうみ")],
...
def test_強制天候特性_とくせいなし状態になると強制天候を解除する(ability_name: str, weather_name: WeatherName):
    """おわりのだいち・デルタストリームも、はじまりのうみと同様にとくせいなし
    状態への遷移で強制天候を解除する（fuzz_log seed=1204参照）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name=ability_name)],
        team1=[Pokemon("ライチュウ", ability_name="せいでんき")],
        volatile0={"とくせいなし": 3},
