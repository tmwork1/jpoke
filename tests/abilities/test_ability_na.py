...
def test_のろわれボディ_ダメージがない技では発動しない():
    """変化技などダメージが発生しない技を受けても発動しない。"""
    battle = t.start_battle(
...
def test_のろわれボディ_相手が既にかなしばり状態のときは発動ログが出ない():
    """のろわれボディ: 相手が既にかなしばり状態のときは揮発性状態の再付与に失敗し、
    発動ログ（ABILITY_TRIGGERED）自体が出ない（fuzzログ seed=519 の回帰）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="のろわれボディ")],
        team1=[Pokemon("カビゴン", move_names=["たいあたり"])],
    )
    ...
    triggered = [
        ...
        and getattr(log.payload, "ability", None) == "のろわれボディ"
    ]
    assert triggered == []


def test_のろわれボディ_確率を外れた場合は発動しない():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="のろわれボディ")],
        team1=[Pokemon("カビゴン", move_names=["たいあたり"])],
    )
...
def test_のろわれボディ_非接触技でも発動する():
    """のろわれボディは接触技かどうかを問わず、攻撃技のダメージを受ければ発動する
    （一次情報: 非直接攻撃に対しても発動する）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="のろわれボディ")],
        team1=[Pokemon("カビゴン", move_names=["でんきショック"])],
...
def test_ノーてんき_かたやぶりでも天候効果無効化は貫通されない():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ヒトカゲ", ability_name="かたやぶり", move_names=["ほのおのちかい"])],
...
def test_ノーてんき_場から退場すると天候効果が復活する():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ヒトカゲ", move_names=["ほのおのちかい"])],
