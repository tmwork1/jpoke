...
def test_ダルマモード_HP1_2以下で場に出た直後は変化しない():
    ...
    darmanitan = Pokemon("ヒヒダルマ", ability_name="ダルマモード")
...
def test_ダルマモード_ターン終了時にHP1_2以下ならダルマのすがたになる():
    battle = t.start_battle(
        team0=[Pokemon("ヒヒダルマ", ability_name="ダルマモード")],
        team1=[Pokemon("ピカチュウ")],
    )
    mon = battle.actives[0]
    mon.hp = mon.max_hp // 2
...
def test_テラスチェンジ_かがくへんかガスが発動中でも発動する():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ"), Pokemon("テラパゴス", ability_name="テラスチェンジ")],
...
def test_テラスチェンジ_登場時にテラスタルフォルムになる():
    battle = t.start_battle(
        team0=[Pokemon("テラパゴス", ability_name="テラスチェンジ")],
        team1=[Pokemon("ピカチュウ")],
    )
    mon = battle.actives[0]
    assert mon.name == "テラパゴス(テラスタル)"
...
def test_てんねん_攻撃側は相手の回避率ランク補正を無視する(evasion_rank):
    ...
    battle.actives[1].boosts["evasion"] = evasion_rank
...
def test_てんねん_防御側は相手の命中率ランク補正を無視する(accuracy_rank):
    ...
    battle.actives[0].boosts["accuracy"] = accuracy_rank
    move = t.run_move(battle, 0)
    assert battle.move_executor.accuracy == move.accuracy
...
def test_てんのめぐみ_発動率が50パーセント以上の技は100パーセントになる():
    ...
    t.run_move(battle, 0)
    assert battle.actives[0].boosts["spa"] == 1
...
def test_とびだすハバネロ_ひんしになっても発動する():
    """とびだすハバネロ: 攻撃技を受けて自身がひんしになったときも攻撃者をやけど状態にする
    （.internal/spec/abilities/とびだすハバネロ.md「攻撃技でひんしになったときも発動する」）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["たいあたり"])],
...
def test_とびだすハバネロ_みがわりに阻まれたときは発動しない():
    """とびだすハバネロ: 攻撃技をみがわり状態で防いだとき（実HPダメージ0）は発動しない。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ", ability_name="とびだすハバネロ")],
    )
    ...
    def count_triggered() -> int:
        return sum(
            ...
            if log.log == LogCode.ABILITY_TRIGGERED
            and log.payload is not None
            and getattr(log.payload, "ability", None) == "とびだすハバネロ"
        )

    assert count_triggered() == 0


def test_とびだすハバネロ_やけど付与時に発動ログが記録される():
    """とびだすハバネロ: やけど付与に成功したとき特性発動アナウンスログが記録される。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["たいあたり"])],
        team1=[Pokemon("ピカチュウ", ability_name="とびだすハバネロ")],
    )
...
def test_とれないにおい_接触した相手の特性を上書き(move_name: str, result: bool):
    """とれないにおい: 直接攻撃でダメージを受けたとき攻撃者の特性をとれないにおいにする（非接触技では発動しない）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=[move_name])],
        team1=[Pokemon("カビゴン", ability_name="とれないにおい")],
        accuracy=100,
