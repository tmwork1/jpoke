...
def test_リベロ_へんげんじざいと同様に技実行直前にタイプが変化する():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="リベロ", move_names=["たいあたり", "ほのおのちかい"])],
        team1=[Pokemon("カビゴン")],
    )
    attacker = battle.actives[0]
...
def test_リベロ_交代でリセットされ再発動できる():
    battle = t.start_battle(
        team0=[
            Pokemon("ピカチュウ", ability_name="リベロ", move_names=["たいあたり", "ほのおのちかい"]),
            Pokemon("ピカチュウ"),
        ],
        team1=[Pokemon("カビゴン")],
    )
...
def test_わたげ_攻撃者がみがわり状態のときは発動しない():
    ...
    （ぬめぬめ/カーリーヘアーとの違い。一次情報: .internal/wiki/abilities/わたげ.html 特性の仕様節）。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="わたげ")],
        team1=[Pokemon("カビゴン", move_names=["たいあたり"])],
        volatile1={"みがわり": 1},
...
def test_わるいてぐせ_さまようたましいで渡った直後に発動する():
    ...
    待たずに交換直後に発動し、道具を奪える（.internal/spec/abilities/わるいてぐせ.md
...
def test_わるいてぐせ_ねんちゃくの相手が反動でひんしになった場合は奪える():
    ...
    わるいてぐせで道具を奪える（.internal/spec/abilities/わるいてぐせ.md「効果の処理順」
...
def test_わるいてぐせ_レッドカードで攻撃者が強制交代させられた場合は奪わない():
    """fuzz seed=15996 (ValueError@battle.py:foe:601) の回帰テスト。

    レッドカード（Event.ON_DAMAGE_HIT priority=150）は、わるいてぐせ（同イベント
    priority=180）より先に発動するため、わるいてぐせの発動判定時には攻撃者が既に
    レッドカードで強制交代させられ場を離れている場合がある。この場合ctx.attackerは
    もう場のポケモンではないため、battle.item_manager.take_item内のbattle.foe解決が
    ValueErrorになっていた。修正後は攻撃者が場を離れている（かつひんしでもない）場合
    何もせず、道具も奪わない。"""
    battle = t.start_battle(
        team0=[
        ...
        team1=[Pokemon("カビゴン", ability_name="わるいてぐせ", item_name="レッドカード")],
        accuracy=100,
    )
    attacker, defender = battle.actives
    t.run_move(battle, 0)
    ...
    assert attacker.has_item("たべのこし")
    assert not defender.has_item()
...
def test_わるいてぐせ_自分がひんしになった場合は発動しない():
    ...
    なった場合、道具は奪えない（.internal/spec/abilities/わるいてぐせ.md「効果」）"""
    battle = t.start_battle(
