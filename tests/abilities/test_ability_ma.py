...
def test_マジシャン_なげつけるでダメージを与えても発動しない():
    ...
    ダメージを与えても相手のアイテムは奪えない（.internal/spec/abilities/マジシャン.md）
    """
    battle = t.start_battle(
...
def test_ミラーアーマー_しろいきりで無効化されたときは反射しない():
    ...
    （.internal/spec/abilities/ミラーアーマー.md「自身のみがわり/しろいきり状態...により
...
def test_メガソーラー_あめ中でもほのお技が1_5倍():
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", ability_name="メガソーラー", move_names=["ほのおのちかい"])],
        team1=[Pokemon("カビゴン")],
        weather=("あめ", 5),
    )
    t.run_move(battle, 0)
...
def test_メガソーラー_まねっこでネストした技使用中の天候変化も維持される():
    ...
    battle = t.start_battle(
        ...
        team1=[Pokemon("カビゴン", ability_name="すなはき", move_names=["ほのおのちかい"])],
        accuracy=100,
    )
    t.run_move(battle, 1)  # カビゴン: ほのおのちかい（まねっこのコピー対象を作る。まだすなはきは発動しない）
    t.run_move(battle, 0)  # ピカチュウ: まねっこ（内部でほのおのちかいがネストして実行され、カビゴンのすなはきが発動する）
...
def test_メガソーラー_相手が攻撃するときは天候補正なし():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["ほのおのちかい"])],
...
def test_メロメロボディ_致命打でも発動する():
    """メロメロボディ: 攻撃技でひんしになったときも発動する
    （fuzz_log横断監査。.internal/spec/abilities/メロメロボディ.md
...
def test_ものひろい_ターン終了時の毒ダメージで消費されたアイテムも同ターンに拾う():
    """ものひろい: .internal/spec/turn.md の ON_TURN_END priority表どおり、どく等のダメージ
