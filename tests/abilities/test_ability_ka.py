...
def test_ごりむちゅう_ためわざの2ターン目をコマンド経由で強制続行してもロックされる():
    """ごりむちゅう: ためわざ（ソーラービーム）の2ターン目の強制続行を
    コマンド解決経由（battle.get_available_commands→battle.step）で行った場合でも、
    技が正しく発動し、実際に使った技（ソーラービーム）でごりむちゅうロックが付与される。

    揮発状態にmove_nameが未設定でわるあがきにフォールバックし、ロックが誤った技
    （わるあがき）で付与されていた不具合の回帰。t.run_move() はCommand解決層を
    経由しないため、この不具合は検出できない。
    溜めターン中はON_MOVE_ENDが発火しないため、ごりむちゅうロックは
    実際に技が発動した2ターン目にのみ付与される点に注意。
    """
    battle = t.start_battle(
        team0=[Pokemon(
    ...
    # 1ターン目: 溜め。ためわざ中はON_MOVE_ENDが発火しないため、ごりむちゅうロックは
    # まだ付与されない
    battle.step({
    ...
    # 実際にソーラービームが発動し、その技でごりむちゅうロックが付与される
    assert mon.has_volatile("ごりむちゅう")
    with battle.phase_context("action"):
        commands = battle.get_available_commands(player0)
    move_commands = [cmd for cmd in commands if cmd.is_type("move")]
