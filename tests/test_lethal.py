...
def test_Vジェネレート_ランクダウン_secondary有り():
    ...
    assert results[0].attacker.boosts["spd"] == -1
    assert results[0].attacker.boosts["spe"] == -1
...
def test_Vジェネレート_ランクダウン_secondary無し():
    ...
    assert results[0].attacker.boosts.get("def", 0) == 0
    assert results[0].attacker.boosts.get("spd", 0) == 0
    assert results[0].attacker.boosts.get("spe", 0) == 0
...
def test_グロウパンチ_secondary無しなら発動しない():
    ...
    assert results[0].attacker.boosts.get("atk", 0) == 0
...
def test_しっとのほのお_やけど付与_ランク上昇時_secondary有り():
    ...
    assert results[0].defender.ailment == "やけど"
...
def test_しっとのほのお_ランク上昇なしなら発動しない():
    ...
    results = t.calc_lethal(battle, player_idx=0, moves=Move("しっとのほのお"), max_attack=1, secondary=True)
    assert results[0].defender.ailment == ""
...
def test_なげつける_しろいハーブ_下がったランクをリセット():
    ...
    assert results[0].defender.boosts["atk"] == 0
...
def test_なげつける_チイラのみ_相手のこうげきランク上昇():
    ...
    results = t.calc_lethal(battle, player_idx=0, moves=Move("なげつける"), max_attack=1, secondary=True)
    assert results[0].defender.boosts["atk"] == 1
...
def test_なげつける_でんきだま_まひ付与_secondary有り():
    ...
    results = t.calc_lethal(battle, player_idx=0, moves=Move("なげつける"), max_attack=1, secondary=True)
    assert results[0].defender.ailment == "まひ"
...
def test_はきだす_たくわえるぶんのランクダウン():
    ...
    move.base_power = 200
    results = t.calc_lethal(battle, player_idx=0, moves=move, max_attack=1)
    assert results[0].attacker.boosts["def"] == -2
    assert results[0].attacker.boosts["spd"] == -2
...
def test_ホイールスピン_すばやさランクダウン():
    ...
    assert results[0].attacker.boosts["spe"] == -2
