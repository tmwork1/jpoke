...
def test_なやみのタネ_かたやぶりでもなまけふみんの相手には失敗する(d_ability):
...
def test_なりきり_かがくへんかガス保持者が使うと元の特性がコピーされガスが解除される():
    """なりきり: かがくへんかガス保持者自身が使用した場合、相手の特性が
    ガスで無効化されていてもとくせいなしとは異なり本来の特性がコピーされる。
    使用者自身の特性が変わることでガスの効果が切れ、相手の特性も再有効化される。"""
    battle = t.start_battle(
        team0=[Pokemon("ドガース", ability_name="かがくへんかガス", move_names=["なりきり"])],
    ...
    assert attacker.ability.base_name == "せいでんき"
