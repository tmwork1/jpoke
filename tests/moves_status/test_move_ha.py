...
def test_バトンタッチ_控えがいる場合はPIVOTが設定される():
    """バトンタッチ: 控えにポケモンがいる場合は PIVOT 割り込みが設定される"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["バトンタッチ"]), Pokemon("ライチュウ")],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_ほえる_soundラベルを持つ():
    """ほえる: sound ラベルを持つことを確認する"""
    battle = t.start_battle(
...
def test_ほろびのうた_soundラベルを持つ():
    """ほろびのうた: sound ラベルを持つことを確認する"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["ほろびのうた"])],
        team1=[Pokemon("カビゴン")],
    )
