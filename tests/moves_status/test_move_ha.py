...
def test_はいすいのじん_2回目の使用で失敗する():
    ...
    t.run_move(battle, 0)
    assert mon.boosts["atk"] == 1  # ランクは変化しない
...
def test_ハバネロエキス_PPは16():
    """ハバネロエキス: チャンピオンズでのPPは16（.internal/champions/move_list.txt準拠）。"""
...
def test_ハバネロエキス_発動前後のランク変化(atk_init, def_init, atk_exp, def_exp):
    ...
    defender = battle.actives[1]
    defender.boosts["atk"] = atk_init
    defender.boosts["def"] = def_init
    t.run_move(battle, 0)

    assert defender.boosts["atk"] == atk_exp
    assert defender.boosts["def"] == def_exp
...
def test_バトンタッチ_引き継ぎ対象volatileが交代先に引き継がれる(volatile_name):
    ...
    .internal/wiki/moves/バトンタッチ.html「状態変化の引き継ぎ」表の第9世代列が○のもの。
    """
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["バトンタッチ"]), Pokemon("ライチュウ")],
        team1=[Pokemon("カビゴン")],
...
def test_パワーシフト_2回使用で元の値に戻る():
    """パワーシフト: 2回使用するとこうげきとぼうぎょの実数値が元に戻ること"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["パワーシフト"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    mon = battle.actives[0]
...
def test_パワーシフト_PPは12():
    """パワーシフト: チャンピオンズでのPPは12（src/jpoke/data/ps-champ-ja/moves.json準拠）。"""
    assert MOVES["パワーシフト"].pp == 12


def test_パワーシフト_こうげきとぼうぎょが入れ替わる():
    """パワーシフト: 使用後にこうげきとぼうぎょの実数値が入れ替わること"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["パワーシフト"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_パワーシフト_タイプはノーマル():
    """パワーシフト: タイプはノーマル（src/jpoke/data/ps-champ-ja/moves.json準拠）。"""
    assert MOVES["パワーシフト"].type == "ノーマル"


def test_パワーシフト_パワートリックと入れ替え操作を共有する():
    """パワーシフト: パワートリックと入れ替え操作を共有するため、
    パワーシフト→パワートリックの順で使用すると実数値が元に戻ること"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["パワーシフト", "パワートリック"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    )
    mon = battle.actives[0]
    ...
    t.run_move(battle, 0, move_idx=0)  # パワーシフト
    t.run_move(battle, 0, move_idx=1)  # パワートリック

    assert mon._stats[1] == atk_original
    assert mon._stats[2] == def_original


def test_パワーシフト_ランクは変化しない():
    """パワーシフト: 実数値のみを入れ替え、こうげき・ぼうぎょのランクは変化しないこと"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["パワーシフト"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_パワーシフト_対象はself():
    """パワーシフト: 対象は自分（target="self"）。foeのままだとまもる・マジックコートに誤って阻害される"""
    move_data = MOVES["パワーシフト"]
    assert move_data.target == "self"


def test_パワーシフト_相手の実数値は変化しない():
    """パワーシフト: 使用者の入れ替えのみで、相手のこうげき・ぼうぎょ実数値は変化しないこと"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["パワーシフト"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
...
def test_パワースワップ_ACランクが双方で入れ替わる():
    ...
    defender.boosts["atk"] = -3
    defender.boosts["spa"] = 1
    t.run_move(battle, 0)

    # 入れ替わった後のランクを確認
    assert attacker.boosts["atk"] == -3
...
def test_パワースワップ_BDランクは変化しない():
    ...
    attacker = battle.actives[0]
    defender = battle.actives[1]
    attacker.boosts["def"] = 3
    attacker.boosts["spd"] = 2
    defender.boosts["def"] = -1
    defender.boosts["spd"] = -2
    t.run_move(battle, 0)

    # ぼうぎょ・とくぼうは変化しない
    assert attacker.boosts["def"] == 3
    assert attacker.boosts["spd"] == 2
...
def test_パワートリック_PPは12():
    """パワートリック: チャンピオンズでのPPは12（.internal/champions/move_list.txt準拠）。"""
...
def test_ひかりのかべ_PPは20():
    """ひかりのかべ: チャンピオンズでのPPは20（.internal/champions/move_list.txt準拠）。"""
...
def test_ひっくりかえす_全ランク0なら失敗する():
    ...
    for v in defender.boosts.values():
        assert v == 0
...
def test_ビルドアップ_発動前後のランク変化(atk_init, def_init, atk_exp, def_exp):
    ...
    attacker = battle.actives[0]
    attacker.boosts["atk"] = atk_init
    attacker.boosts["def"] = def_init
    t.run_move(battle, 0)

    assert attacker.boosts["atk"] == atk_exp
    assert attacker.boosts["def"] == def_exp
...
def test_フェザーダンス_PPは16():
    """フェザーダンス: チャンピオンズでのPPは16（.internal/champions/move_list.txt準拠）。"""
...
def test_ふきとばし_かぜのりで無効化される():
    ...
    assert defender_before.boosts["atk"] == 1
...
def test_へびにらみ_PPは20():
    """へびにらみ: チャンピオンズでのPPは20（.internal/champions/move_list.txt準拠）。"""
...
def test_ほおばる_あまのじゃくでぼうぎょ最大でも成功する():
    ...
    attacker = battle.actives[0]
    attacker.boosts["def"] = 6
    t.run_move(battle, 0)

    assert attacker.boosts["def"] == 4
...
def test_ほろびのうた_PPは8():
    """ほろびのうた: チャンピオンズでのPPは8（.internal/champions/move_list.txt準拠）。"""
...
def test_ほろびのうた_マジックコートで跳ね返されない():
    ...
    （.internal/spec/moves/ほろびのうた.md 技の仕様節）。target="foe"のままだと
