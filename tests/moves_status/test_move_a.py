...
def test_いちゃもん_直前に使用した技が選択肢から除外される():
    ...
    with battle.phase_context("action"):
        commands = battle.available_commands(battle.players[1])
...
def test_いやしのねがい_控えがいないと失敗時のログはMOVE_FAILEDが1行のみ():
    """いやしのねがい: 控え不在による失敗時、_execute_status_hitの汎用フォールバックログが
    ...
    assert failed_logs[0].payload.display_reason == "いやしのねがい_交代不可"
