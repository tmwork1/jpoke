...
def test_サイコフィールド_すでに同じ地形なら失敗():
    """サイコフィールド: すでにサイコフィールドが有効なら失敗（再設置されない）"""
    battle = t.start_battle(
        team0=[Pokemon("ケーシィ", move_names=["サイコフィールド"])],
        team1=[Pokemon("カビゴン")],
        terrain=("サイコフィールド", 3),
    )
    t.run_move(battle, 0)
...
def test_サイコフィールド_地形がサイコフィールドになる():
    """サイコフィールド: 使用後に地形がサイコフィールドになり5ターン継続する"""
    battle = t.start_battle(
        team0=[Pokemon("ケーシィ", move_names=["サイコフィールド"])],
        team1=[Pokemon("カビゴン")],
    )
...
def test_さいはい_ねごと経由ではねごとが対象になり失敗する():
    """さいはい: 相手がねごとでサブ技を実行した場合、指示対象はサブ技ではなく
    最後にPPを消費したねごと自身になるため、指示不可技として失敗する
    （サブ技はPPを消費しないため対象にならない）"""
    battle = t.start_battle(
    ...
    # ねむり状態による行動不能でさいはいの成否が曖昧にならないよう、ねむりを治しておく
    battle.ailment_manager.remove(defender)

    t.run_move(battle, 0)  # ピカチュウ: さいはい

    # さいはいは失敗し、たいあたりの再実行によるダメージ・PP消費は発生しない
    assert attacker.hp == hp_after_first_hit
    assert negoto.pp == 11, "ねごとのPPは最初の1消費のまま変化しない"
...
def test_シンプルビーム_うのミサイルには失敗():
    """シンプルビーム: うのミサイル（上書きできない特性のためprotectedフラグ持ち）の相手には失敗する"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["シンプルビーム"])],
...
def test_じこあんじ_相手のランクは変化しない():
    ...
    defender = battle.actives[1]
    defender.boosts["atk"] = 2
    defender.boosts["spa"] = -3
    t.run_move(battle, 0)

    assert defender.boosts["atk"] == 2
    assert defender.boosts["spa"] == -3
...
def test_じゅうりょく_PPは8():
    """じゅうりょく: チャンピオンズでのPPは8（.internal/champions/move_list.txt準拠）。"""
...
def test_スキルスワップ_PPは12():
    """スキルスワップ: チャンピオンズでのPPは12（.internal/champions/move_list.txt準拠）。"""
    assert MOVES["スキルスワップ"].pp == 12
...
def test_スキルスワップ_うのミサイルは例外的に交換できる():
    """スキルスワップ: うのミサイルはprotectedフラグを持つが、SV Ver.3.0.0以降は
    スキルスワップでの交換が可能になったため例外的に成功する。"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["スキルスワップ"], ability_name="うのミサイル")],
        team1=[Pokemon("カビゴン", ability_name="めんえき")],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_スキルスワップ_まもるで防がれる():
    """スキルスワップ: 対象がまもる状態のときは特性が入れ替わらない"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["スキルスワップ"], ability_name="せいでんき")],
        team1=[Pokemon("カビゴン", ability_name="めんえき")],
        volatile1={"まもる": 1},
...
def test_スキルスワップ_めんえき取得ですでにどく状態なら即座に回復する():
    """スキルスワップ: すでにどく状態のポケモンがめんえきを取得すると即座に回復する"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["スキルスワップ"], ability_name="せいでんき")],
        team1=[Pokemon("カビゴン", ability_name="めんえき")],
        ailment0=("どく", None),
...
def test_スキルスワップ_命中判定を行わず必ず命中する():
    """スキルスワップ: 必中技であり、命中判定自体を行わない（回避ランクの影響を受けない）"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["スキルスワップ"])],
        team1=[Pokemon("カビゴン")],
    )
    t.run_move(battle, 0)
...
def test_せいちょう_たんじゅんで晴れ中4段階上がる():
    ...
    assert attacker.boosts["atk"] == 4
    assert attacker.boosts["spa"] == 4
