...
def test_マジックルーム_PPは12():
    """マジックルーム: チャンピオンズでのPPは12（.internal/champions/move_list.txt準拠）。"""
...
def test_まねっこ_pp_consumed_moveはまねっこ自身になりアンコールがまねっこをロックする():
    """まねっこ: コピー実行中はON_MODIFY_PP_CONSUMEDでPP消費が抑制されるため、
    pp_consumed_moveはコピーした技ではなくまねっこ自身のまま残る。

    これにより直後のアンコールは「コピーした技」ではなく「まねっこ自身」を
    ロックする（アンコール_apply が参照する pp_consumed_move の食い違いを防ぐ回帰テスト）。
    """
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["まねっこ"])],
        team1=[Pokemon("カビゴン", move_names=["たいあたり", "アンコール"])],
        accuracy=100,
    )
    mon = battle.actives[0]

    t.run_move(battle, 1, 0)  # カビゴン: たいあたり（コピー対象を用意）
    t.run_move(battle, 0)  # ピカチュウ: まねっこ（たいあたりをコピー）

    assert mon.pp_consumed_move.name == "まねっこ"

    t.run_move(battle, 1, 1)  # カビゴン: アンコール → まねっこ自身をロックするはず
...
def test_まねっこ_コピー技のPPが0でも成功する():
    """まねっこ: コピー対象の技のPPが0でもまねっこ自体は成功し、
    コピー技側のPPが負にならない（まねっこ_can_useはPP残量を判定条件にしないため）"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["まねっこ"])],
        team1=[Pokemon("コラッタ", move_names=["でんこうせっか"])],
        accuracy=100,
    ...
    t.run_move(battle, 1)  # コラッタ: でんこうせっか（コピー対象として記録される）
    defender.moves[0].pp = 0  # コピー対象の技のPPを0にする

    t.run_move(battle, 0)  # ピカチュウ: まねっこ（PP0のでんこうせっかをコピー）

    assert battle.move_executor.move_applied
    assert defender.moves[0].pp == 0, "コピー技のPPは消費されないため0のまま変化しない"
...
def test_まねっこ_自分が使った技もコピー対象になる():
    ...
    コピー元の技（でんこうせっか）自身のPPスロットがまねっこ実行によって
    追加で消費されないことも合わせて確認する。
    """
    battle = t.start_battle(
    ...
    denkousekka_pp_after_own_use = attacker.moves[0].pp

    t.run_move(battle, 0, 1)  # まねっこ（でんこうせっかをコピー）

    assert defender.hp < hp_after_first_hit
    assert attacker.moves[0].pp == denkousekka_pp_after_own_use, (
        "自分自身の技をコピーしても、コピー元の技スロットのPPはそれ以上消費されない"
    )
...
def test_みかづきのいのり_HP満タンかつ無状態異常だと失敗する():
    ...
    回復対象がいない場合に失敗する（.internal/spec/turn.md 「ジャングルヒール: HP満タン・状態異常無」）。
    """
    battle = t.start_battle(
        team0=[Pokemon("クレセリア", move_names=["みかづきのいのり"])],
        team1=[Pokemon("カビゴン")],
