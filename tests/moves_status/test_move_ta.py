...
def test_たくわえる_N回使用でカウントがNになる(count):
    ...
    assert attacker.boosts["def"] == count
    assert attacker.boosts["spd"] == count
...
def test_つぼをつく_選ばれない能力のランクは変化しない():
    ...
    assert attacker.boosts["evasion"] == 0
...
def test_テクスチャー２_フリーズドライに対してみずタイプとして判定される():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ポリゴン2", move_names=["テクスチャー２"])],
        team1=[Pokemon("カビゴン", move_names=["フリーズドライ"])],
        accuracy=100,
    )
    attacker, defender = battle.actives
    t.run_move(battle, 1)
    assert defender.last_move is not None
    assert defender.last_move.type == "こおり"
...
def test_テクスチャー２_交代でタイプが元に戻る():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ポリゴン2", move_names=["テクスチャー２"]), Pokemon("コイキング")],
        team1=[Pokemon("カビゴン", move_names=["かえんほうしゃ"])],
        accuracy=100,
    )
    attacker = battle.actives[0]
...
def test_テクスチャー２_自分がテラスタル中の場合失敗する():
    ...
    battle = t.start_battle(
        team0=[Pokemon("ポリゴン2", move_names=["テクスチャー２"], tera_type="ほのお")],
        team1=[Pokemon("カビゴン", move_names=["かえんほうしゃ"])],
        accuracy=100,
    )
    attacker = battle.actives[0]
