...
def test_アンコール_non_encoreラベルの技を使った相手には失敗する():
    ...
    defender.last_pp_consumed_move = Move("わるあがき")
    t.run_move(battle, 0)
...
def test_アンコール_バーンアクセルを使った相手には失敗する():
    ...
    defender = battle.actives[1]
    defender.last_pp_consumed_move = Move("バーンアクセル")
    t.run_move(battle, 0)
    assert not defender.has_volatile("アンコール")
...
def test_アンコール_未行動の相手には失敗する():
    ...
    （last_pp_consumed_moveがNone）はアンコール状態が付与されない"""
    battle = t.start_battle(
        team0=[Pokemon("ピカチュウ", move_names=["アンコール"])],
        team1=[Pokemon("カビゴン")],
        accuracy=100,
    ...
    defender = battle.actives[1]
    # last_pp_consumed_move は None のまま（デフォルト）
    assert defender.last_pp_consumed_move is None
    t.run_move(battle, 0)
    assert not defender.has_volatile("アンコール")
