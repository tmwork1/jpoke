...
def test_やどりぎのタネ_回復先が同ターン中に瀕死なら回復しない():
    """seed=443 (LogInconsistency@volatile.py:やどりぎのタネ_drain_hp:1517) の回帰テスト。

    やどりぎのタネの吸収先（種を設置した側 = to_mon）が、同ターン中の別要因（直接攻撃等）で
    既に瀕死になっている場合、ターン終了時のやどりぎのタネの吸収による回復を適用してはならない。
    修正前は to_mon の瀕死判定を行わず無条件に回復しており、HP0のポケモンが「復活」してしまう
    ログ不整合が発生していた。
    """
    battle = t.start_battle(
        team1=[Pokemon("ピカチュウ")],
    ...
    from_mon, to_mon = battle.actives
    to_mon.hp = 1
    # 同ターン中の別要因（直接攻撃等）で回復先が先に瀕死になったケースを再現する
    battle.modify_hp(to_mon, v=-1, source=from_mon, reason="move_damage")
    assert to_mon.fainted

    t.end_turn(battle)

    # やどりぎのタネによるダメージ自体は通常通り発生する
    damage = from_mon.max_hp - from_mon.hp
    assert damage == from_mon.max_hp // 8
    # 回復先は既に瀕死のため、回復は適用されず瀕死のまま
    assert to_mon.hp == 0
    assert to_mon.fainted
...
def test_リチャージ_中はテラスタルを選択できない():
    ...
    （行動不能ターン）中は、.internal/spec/terastal.md「発動条件・制限」の通り
